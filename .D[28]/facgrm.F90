...
ILPREF = MIN(LEN (CLPREF),LEN (CDPREF))
ILSUFF = LEN (CLSUFF)
...
IF ((KNIVAU > 0).AND.(CDPREF(1:4) /= 'SURF').AND.(CDPREF(1:4) /= 'ATMO').AND.(CDPREF(1:3) /= 'TOP')) THEN
...
CALL IGRIB_SET_VALUE (KGRIBH, "biFourierTruncationType", 88) 
...
INTEGER (KIND=JPLIKB) :: ILONS, ILATS, IILONW, IILATS, IILONE, IILATN, IILOND, IILATD
INTEGER (KIND=JPLIKB) :: IDLOND, IDLATD
...
IILONW = IROUNDM4(ZLONW)
IILATS = IROUNDM4(ZLATS)
IILONE = IROUNDM4(ZLONE)
IILATN = IROUNDM4(ZLATN)
!IILOND = IROUNDM4(ZLOND)
!IILATD = IROUNDM4(ZLATD)
IDLOND=MODULO(IILONE-IILONW,360000000)
IDLATD=MODULO(IILATN-IILATS,180000000)
IF (IDLOND==0) IDLOND=360000000
IF (IDLATD==0) IDLATD=180000000
IILOND = IDLOND/(ILONS-1)
IILATD = IDLATD/(ILATS-1)

IF (MODULO(IDLOND,IILOND).NE.0) THEN
END IF
IF (MODULO(IDLATD,IILATD).NE.0) THEN
END IF
IF (ABS(IDLOND/IILOND).NE.(ILONS-1)) THEN
END IF
IF (ABS(IDLATD/IILATD).NE.(ILATS-1)) THEN
END IF


CALL IGRIB_SET_VALUE (KGRIBH, 'Ni', ILONS)
CALL IGRIB_SET_VALUE (KGRIBH, 'Nj', ILATS)
CALL IGRIB_SET_VALUE (KGRIBH, 'longitudeOfFirstGridPoint', IILONW)
CALL IGRIB_SET_VALUE (KGRIBH, 'latitudeOfLastGridPoint',   IILATS)
CALL IGRIB_SET_VALUE (KGRIBH, 'longitudeOfLastGridPoint',  IILONE)
CALL IGRIB_SET_VALUE (KGRIBH, 'latitudeOfFirstGridPoint',  IILATN)
CALL IGRIB_SET_VALUE (KGRIBH, 'iScansNegatively', 0)
CALL IGRIB_SET_VALUE (KGRIBH, 'jScansPositively', 0)
CALL IGRIB_SET_VALUE (KGRIBH, 'iDirectionIncrement', IILOND)
CALL IGRIB_SET_VALUE (KGRIBH, 'jDirectionIncrement', IILATD)
...
IF (CDPREF == 'S') THEN
  CALL IGRIB_SET_VALUE (KGRIBH, 'PVPresent', 1)
...
INTEGER (KIND=JPLIKB) :: ITRI, IRANGE,IUNITRANGE, ITYPGP,IUNITFORRANGE
LOGICAL :: LTEST
...
ELSE
  ...
  ELSEIF (ITRI /= 0 .AND. ISTCUM == 1) THEN
    ...
    ELSEIF (IUNITRANGE==0) THEN
      CALL IGRIB_SET_VALUE (KGRIBH, 'lengthOfTimeRange', IDATEF(15)/60)
      CALL IGRIB_SET_VALUE (KGRIBH, 'numberOfTimeRange', 1)
      CALL IGRIB_SET_VALUE (KGRIBH, 'indicatorOfUnitForTimeRange', 0)
    END IF
  ELSEIF (ITRI /= 0 .AND. ISTCUM == 2) THEN
    ...
    !hours
      ...
      CALL FAGRIB_SETTIME ('endStep',   IDATEF (15), CTERMU)
    ELSEIF (IUNITRANGE==0) THEN
      CALL IGRIB_GET_VALUE (KGRIBH, 'indicatorOfUnitForTimeRange', IUNITFORRANGE)
      CALL IGRIB_GET_VALUE (KGRIBH, 'lengthOfTimeRange', IRANGE)
        IF (IUNITFORRANGE==0) THEN
         CALL IGRIB_SET_VALUE (KGRIBH, 'lengthOfTimeRange', IRANGE)
         CALL IGRIB_SET_VALUE (KGRIBH, 'numberOfTimeRange', 1)
         CALL FAGRIB_SETTIME ('startStep', IDATEF(15)-IRANGE*60 , CTERMU)
         CALL FAGRIB_SETTIME ('endStep',   IDATEF (15), CTERMU)
        ELSE
         CALL IGRIB_SET_VALUE (KGRIBH, 'indicatorOfUnitForTimeRange', 0)
         CALL IGRIB_SET_VALUE (KGRIBH, 'lengthOfTimeRange', IRANGE*60)
         CALL IGRIB_SET_VALUE (KGRIBH, 'numberOfTimeRange', 1)
         CALL FAGRIB_SETTIME ('startStep', IDATEF(15)-IRANGE*3600 , CTERMU)
         CALL FAGRIB_SETTIME ('endStep',   IDATEF (15), CTERMU)
        END IF
...
SELECT CASE (CDUNIT)
  ...
  CASE ('C')
    IVALUE = KSECS / (100_JPLIKB * 365_JPLIKB * 24_JPLIKB * 60_JPLIKB * 60_JPLIKB)
  CASE ('h')
...
IF (LLCOSP) THEN
  ...
  ELSEIF (LLMLAM) THEN
    ...
    IF (INBITS == INBITSMAX) THEN
      ...
      CALL IGRIB_SET_VALUE (KGRIBH, "biFourierSubTruncationType", 88) 
    ELSE
    ...
    CALL IGRIB_SET_VALUE (KGRIBH, "biFourierPackingModeForAxes", 1)
...
FUNCTION IROUNDM2(PVA) RESULT(IVA)
!the REAL grid parameter (lat,lon,deltalat,deltalon....)
!is converted into grib2 integer value with an arbitrary
!round off of 0.01 degres in  accuracy
REAL (KIND=JPDBLR)    :: PVA
INTEGER (KIND=JPLIKB) :: IVA
IVA=NINT(PVA*1000000.,SELECTED_INT_KIND(16))
IF (MODULO(IVA,10000)>5000) THEN
 IVA=(IVA/10000+1)*10000
ELSE
 IVA=(IVA/10000)*10000
END IF
END FUNCTION IROUNDM2

FUNCTION IROUNDM4(PVA) RESULT(IVA)
!the REAL grid parameter (lat,lon,deltalat,deltalon....)
!is converted into grib2 integer value with an arbitrary
!round off of 0.0001 degres in  accuracy
REAL (KIND=JPDBLR)    :: PVA
INTEGER (KIND=JPLIKB) :: IVA,ISG,IRO
IRO=10000
ISG=INT(SIGN(1._8,PVA))
IVA=NINT(ABS(PVA)*1000000.,SELECTED_INT_KIND(16))
IF (MODULO(IVA,IRO)>IRO/2) THEN
 IVA=(IVA/IRO+1)*IRO
ELSE
 IVA=(IVA/IRO)*IRO
END IF
IF (IVA==360000000) THEN
 IVA=0
END IF
IVA=IVA*ISG
END FUNCTION IROUNDM4
