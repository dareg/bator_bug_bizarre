...
&                    KLNOMC, KGARDE)
USE FA_MOD, ONLY : FA_COM, NEW_CADRE, FREE_CADRE, JPNIIL, JNGEOM, JNEXPL, NUNDEF
...
LDREDF=.FALSE.
LDMODC=.FALSE.
LLSURF=FA%NCODGRI (13) /= 0
ZEPS = 10._JPDBLR * EPSILON (1._JPDBLR)

IF ((FA%NCODGRI (14) /= NUNDEF) .AND. (FA%NCODGRI (14) /= 0)) THEN
  ZEPS = 10._JPDBLR**FA%NCODGRI (14)
ENDIF
!
IF (KTYPTR .LE. 0 ) THEN
   LLMLAM = .TRUE.
...
CALL NEW_CADRE (FA%CADRE(KRANGC), KTYPTR, KNLATI, KTRONC, KNIVER, LLSURF)
...
!
IF (LLSURF) THEN
  FA%CADRE(KRANGC)%SFOHYB = 0._JPDBLR
ELSE
  DO J=0,KNIVER
...
IF (FA%CADRE(KRANGC)%NNIVER.NE.KNIVER .AND. .NOT. LLSURF) GOTO 505
...
ELSE
   DO J=1,JNEXPL
      IF (FA%CADRE(KRANGC)%NLOPAR(J).NE.KNLOPA(J))THEN
         IF (.NOT. LLSURF .AND. J/=2)  GOTO 505
      ENDIF
   ENDDO
   DO J=1,JNGEOM
   IF (ABS(REAL (FA%CADRE(KRANGC)%SINLAT(J), JPDBLR)-REAL (PSINLA(J), JPDBLR))>ZEPS) GOTO 505
...
&  KRANGC,KLNOMC,KGARDE
  CALL FAIPAR_MT64                                      &
&                 (FA, INUMER,INIMES,KREP,LLFATA,CLMESS, &
