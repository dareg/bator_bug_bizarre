...
PUBLIC void 
RSTBIAS_print_flags_file(void)
{
  int rc = 0, io = -1;
  FILE *fp = NULL;
  cma_open_(&io, "RSTBIAS.flags", "w", &rc, strlen("RSTBIAS.flags"), strlen("w"));
...
Create_Funcs(ODB_Pool *pool, int is_new, int io_method, int it)
{
  ...
  ODBMAC_CREATE_TABLE(RSTBIAS, desc, 0, 1);
  ODBMAC_CREATE_TABLE(RSTBIAS, hdr, 1, 1);
  ODBMAC_CREATE_TABLE(RSTBIAS, body, 2, 1);
  DRHOOK_END(0);
  return nfuncs;
}
...
PUBLIC ODB_Funcs *
Anchor2RSTBIAS(void *V, ODB_Pool *pool, int *ntables, int it, int add_vars)
{
  ODB_Anchor_Funcs *func = V;
  ODB_Pool *p = pool;
  DRHOOK_START(Anchor2RSTBIAS);
  ...
  if (add_vars) {
    p->add_var(p->dbname, "$mdi", NULL, it, USD_mdi_RSTBIAS);
  } /* if (add_vars) */
  finish:
  DRHOOK_END(0);
  return NULL;
