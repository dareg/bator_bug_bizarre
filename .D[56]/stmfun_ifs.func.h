...
!     ISET0(KVAR,KBP) = KVAR .AND. COMPL( SHIFTL( 1B,KBP))
REAL(KIND=JPRB) :: PCONF
INTEGER(KIND=JPIM) :: ICN2FL
ICN2FL(PCONF) = MIN(3, (92 - MIN(92, INT(PCONF+EPSMAC)))/23 )

REAL(KIND=JPRB) :: FOEALFA,PTARG
FOEALFA (PTARG) = MIN(1.0_JPRB,((MAX(RTICE,MIN(RTWAT,PTARG))-RTICE)&
 &/(RTWAT-RTICE))**2)
...
FOEEWM ( PTARG ) = R2ES *&
  ...
  &(1.0_JPRB-FOEALFA(PTARG))*EXP(R3IES*(PTARG-RTT)/(PTARG-R4IES)))
