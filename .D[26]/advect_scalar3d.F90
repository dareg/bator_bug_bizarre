...
real(RR) u(dimx1_u:dimx2_u, dimy1_u:dimy2_u, nzm)
real(RR) v(dimx1_v:dimx2_v, dimy1_v:dimy2_v, nzm)
real(RR) w(dimx1_w:dimx2_w, dimy1_w:dimy2_w, nz )
real(RR) rho(nzm)
real(RR) rhow(nz)
real(RR) flux(nz)

real(RR) mx (0:nxp1,0:nyp1,nzm)
real(RR) mn (0:nxp1,0:nyp1,nzm)
real(RR) uuu(-1:nxp3,-1:nyp2,nzm)
real(RR) vvv(-1:nxp2,-1:nyp3,nzm)
real(RR) www(-1:nxp2,-1:nyp2,nz)

real(RR) eps, dd
real(RR) iadz(nzm),irho(nzm),irhow(nzm)
integer i,j,k,ic,ib,jc,jb,kc,kb
logical nonos

real(RR) x1, x2, a, b, a1, a2, y
real(RR) andiff,across,pp,pn
