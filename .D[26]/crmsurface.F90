...
!************** LICENSE END ******************
     ...
     real(RR), intent (in) :: bflx
     real(RR) u_h0, tau00, tauxm, tauym
     real(RR) diag_ustar
     integer i,j
...
! so now used as reciprocal of obukhov length)
      ...
      real(RR) function diag_ustar(z,bflx,wnd,z0)

      use real_size
      implicit none
      real(RR), parameter      :: vonk =  0.4   ! von Karmans constant
      real(RR), parameter      :: g    = 9.81   ! gravitational acceleration
      real(RR), parameter      :: am   =  4.8   !   "          "         "
      real(RR), parameter      :: bm   = 19.3   !   "          "         "
      real(RR), parameter      :: eps  = 1.e-10 ! non-zero, small number

      real(RR), intent (in)    :: z             ! height where u locates
      real(RR), intent (in)    :: bflx          ! surface buoyancy flux (m^2/s^3)
      real(RR), intent (in)    :: wnd           ! wind speed at z
      real(RR), intent (in)    :: z0            ! momentum roughness height

      integer :: iterate
      real(RR)    :: lnz, klnz, c1, x, psi1, zeta, rlmo, ustar
...
! ----------------------------------------------------------------------
      ...
      real(RR) function z0_est(z,bflx,wnd,ustar)
...
!
      ...
      real(RR), intent (in)    :: bflx          ! surface buoyancy flux (m^2/s^3)
      real(RR), intent (in)    :: wnd           ! wind speed at z
      real(RR), intent (in)    :: ustar         ! friction velocity

      real(RR)    :: lnz, klnz, c1, x, psi1, zeta, rlmo
