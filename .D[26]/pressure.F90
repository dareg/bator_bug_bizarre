...
real(RR) f(nx2,ny2,nzslab) ! global rhs and array for FTP coefficeients
real(RR) ff(nx+1,ny+2*YES3D,nzm)  ! local (subdomain's) version of f
real(RR) buff_slabs(nxp1,nyp2,nzslab,npressureslabs)
real(RR) buff_subs(nxp1,nyp2,nzslab,nsubdomains) 
real(RR) bufp_slabs(0:nx,1-YES3D:ny,nzslab,npressureslabs)  
real(RR) bufp_subs(0:nx,1-YES3D:ny,nzslab,nsubdomains)  

real(RR) work(nx2,ny2),trigxi(n3i),trigxj(n3j) ! FFT stuff
