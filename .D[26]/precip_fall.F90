...
real(RR) qp(dimx1_s:dimx2_s, dimy1_s:dimy2_s, nzm) ! falling hydrometeor
integer hydro_type   ! 0 - all liquid, 1 - all ice, 2 - mixed
real(RR) omega(nx,ny,nzm)   !  = 1: liquid, = 0: ice;  = 0-1: mixed : used only when hydro_type=2
integer ind

! Terminal velocity fnction 

real(RR), external :: term_vel  ! terminal velocity function


! Local:

real(RR) mx(nzm),mn(nzm), lfac(nz)
real(RR) www(nz),fz(nz)
real(RR) df(dimx1_s:dimx2_s, dimy1_s:dimy2_s, nzm)
real(RR) f0(nzm),df0(nzm)
real(RR) eps
integer i,j,k,kc,kb
logical nonos
...
real(RR) lat_heat, wmax

real(RR) wp(nzm), tmp_qp(nzm), irhoadz(nzm), iwmax(nzm), rhofac(nzm), prec_cfl
integer nprec, iprec
real(RR)  flagstat

!--------------------------------------------------------

! call t_startf ('precip_fall') ! commented by Phani
...
! call t_stopf ('precip_fall') ! commented by Phani
