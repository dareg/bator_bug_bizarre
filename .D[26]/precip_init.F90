...
real(RR) pratio, coef1, coef2,estw,esti,rrr1,rrr2
real(4) gammafff   ! calling C module
external gammafff
integer k 
...
! accretion by snow:
  ...
  coef1 = 0.25 * pi * nzeros * a_snow * REAL(gams1,RR) * pratio/ &
...
! evaporation of snow:
  ...
  evaps2(k)  =  0.49*4.*nzeros*REAL(gams2,RR)*sqrt(a_snow/(muelq*rrr1))/ & 
       (pi*rhos*nzeros)**((5+b_snow)/8.) / (coef1+coef2) &
               * rho(k)**((1+b_snow)/8.)*sqrt(pratio)  

! accretion by graupel:

  coef1 = 0.25*pi*nzerog*a_grau*REAL(gamg1,RR)*pratio/&
...
! evaporation of graupel:
  ...
  evapg2(k)  = 0.49*4.*nzerog*REAL(gamg2,RR)*sqrt(a_grau/(muelq*rrr1))/ &
...
! accretion by rain:
  ...
  accrrc(k)=  0.25 * pi * nzeror * a_rain * REAL(gamr1,RR) * pratio/ &
...
! evaporation of rain:
  ...
  evapr2(k)  =  0.31 * 2. * pi  * nzeror * REAL(gamr2,RR) * &
