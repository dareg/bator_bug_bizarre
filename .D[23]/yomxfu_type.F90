...
TYPE (FIELD_2D), POINTER :: F_RMWINDCALC => NULL ()
...
TYPE (FIELD_2D), POINTER :: F_RMNWINDCALC => NULL ()
...
TYPE (FIELD_2D), POINTER :: F_CLWC2 => NULL ()
TYPE (FLUXES_DESCRIPTOR) :: YCLWC2

! maximum of CLWC
REAL (KIND=JPRB), POINTER :: CLWC (:) => NULL ()
TYPE (FIELD_2D), POINTER :: F_CLWC => NULL ()
...
TYPE (FIELD_2D), POINTER :: F_NEBT => NULL ()
...
TYPE (FIELD_2D), POINTER :: F_PTYPESEV2 => NULL ()
TYPE (FLUXES_DESCRIPTOR) :: YPTYPESEV2

! severe precipitations type
REAL (KIND=JPRB), POINTER :: PTYPESEV (:) => NULL ()
TYPE (FIELD_2D), POINTER :: F_PTYPESEV => NULL ()
...
TYPE (FIELD_3D), POINTER :: F_RSO => NULL ()
TYPE (FLUXES_DESCRIPTOR) :: YRSO

! surface radiation
REAL (KIND=JPRB), POINTER :: RTH (:,:) => NULL ()
TYPE (FIELD_3D), POINTER :: F_RTH => NULL ()
...
TYPE (FIELD_2D), POINTER :: F_SIC => NULL ()
...
TYPE (FIELD_2D), POINTER :: F_UGST2 => NULL ()
...
TYPE (FIELD_2D), POINTER :: F_VGST2 => NULL ()
...
TYPE (FIELD_2D), POINTER :: F_VISICLD2 => NULL ()
...
TYPE (FIELD_2D), POINTER :: F_VISIHYD2 => NULL ()
...
TYPE (FIELD_2D), POINTER :: F_XDIAGH => NULL ()
...
IF (ASSOCIATED (SELF%RMWINDCALC_B)) THEN
  SELF%F_RMWINDCALC => CREATE_FIELD_WRAP (SELF%RMWINDCALC_B, PERSISTENT=.TRUE.)
ENDIF

IF (ASSOCIATED (SELF%RMNWINDCALC_B)) THEN
  SELF%F_RMNWINDCALC => CREATE_FIELD_WRAP (SELF%RMNWINDCALC_B, PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXCAPE
IF (IOFF > 0) THEN
  SELF%F_CAPE => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXCLPH
IF (IOFF > 0) THEN
  SELF%F_CLPH => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXCLWC2
IF (IOFF > 0) THEN
  SELF%F_CLWC2 => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXCLWC
IF (IOFF > 0) THEN
  SELF%F_CLWC => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXCTOP
IF (IOFF > 0) THEN
  SELF%F_CTOP => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXDICQ
IF (IOFF > 0) THEN
  ILEV = SELF%YDICQ%ILEV
  SELF%F_DICQ => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXDICS
IF (IOFF > 0) THEN
  ILEV = SELF%YDICS%ILEV
  SELF%F_DICS => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXDITQ
IF (IOFF > 0) THEN
  ILEV = SELF%YDITQ%ILEV
  SELF%F_DITQ => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXDITS
IF (IOFF > 0) THEN
  ILEV = SELF%YDITS%ILEV
  SELF%F_DITS => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXFCHSP
IF (IOFF > 0) THEN
  SELF%F_FCHSP => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXFEVL
IF (IOFF > 0) THEN
  SELF%F_FEVL => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXFEVV
IF (IOFF > 0) THEN
  SELF%F_FEVV => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXFLWSP
IF (IOFF > 0) THEN
  SELF%F_FLWSP => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXFONTE
IF (IOFF > 0) THEN
  SELF%F_FONTE => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXFTR
IF (IOFF > 0) THEN
  SELF%F_FTR => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXHUN
IF (IOFF > 0) THEN
  SELF%F_HUN => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXHUX
IF (IOFF > 0) THEN
  SELF%F_HUX => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXMOCON
IF (IOFF > 0) THEN
  SELF%F_MOCON => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXMRT
IF (IOFF > 0) THEN
  SELF%F_MRT => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXNBBAS
IF (IOFF > 0) THEN
  SELF%F_NBBAS => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXNBCON
IF (IOFF > 0) THEN
  SELF%F_NBCON => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXNBHAU
IF (IOFF > 0) THEN
  SELF%F_NBHAU => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXNBMOY
IF (IOFF > 0) THEN
  SELF%F_NBMOY => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXNEB
IF (IOFF > 0) THEN
  ILEV = SELF%YNEB%ILEV
  SELF%F_NEB => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXNEBT
IF (IOFF > 0) THEN
  SELF%F_NEBT => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXNUCLS
IF (IOFF > 0) THEN
  SELF%F_NUCLS => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXNVCLS
IF (IOFF > 0) THEN
  SELF%F_NVCLS => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXPLCG
IF (IOFF > 0) THEN
  ILEV = SELF%YPLCG%ILEV
  SELF%F_PLCG => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXPLCH
IF (IOFF > 0) THEN
  ILEV = SELF%YPLCH%ILEV
  SELF%F_PLCH => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXPLCL
IF (IOFF > 0) THEN
  ILEV = SELF%YPLCL%ILEV
  SELF%F_PLCL => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXPLCN
IF (IOFF > 0) THEN
  ILEV = SELF%YPLCN%ILEV
  SELF%F_PLCN => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXPLSG
IF (IOFF > 0) THEN
  ILEV = SELF%YPLSG%ILEV
  SELF%F_PLSG => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXPLSH
IF (IOFF > 0) THEN
  ILEV = SELF%YPLSH%ILEV
  SELF%F_PLSH => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXPLSL
IF (IOFF > 0) THEN
  ILEV = SELF%YPLSL%ILEV
  SELF%F_PLSL => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXPLSN
IF (IOFF > 0) THEN
  ILEV = SELF%YPLSN%ILEV
  SELF%F_PLSN => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXPTYPE2
IF (IOFF > 0) THEN
  SELF%F_PTYPE2 => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXPTYPE
IF (IOFF > 0) THEN
  SELF%F_PTYPE => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXPTYPESEV2
IF (IOFF > 0) THEN
  SELF%F_PTYPESEV2 => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXPTYPESEV
IF (IOFF > 0) THEN
  SELF%F_PTYPESEV => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXQCLS
IF (IOFF > 0) THEN
  SELF%F_QCLS => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXQICE
IF (IOFF > 0) THEN
  ILEV = SELF%YQICE%ILEV
  SELF%F_QICE => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXQLI
IF (IOFF > 0) THEN
  ILEV = SELF%YQLI%ILEV
  SELF%F_QLI => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXRHCLS
IF (IOFF > 0) THEN
  SELF%F_RHCLS => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXRSO
IF (IOFF > 0) THEN
  ILEV = SELF%YRSO%ILEV
  SELF%F_RSO => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXRTH
IF (IOFF > 0) THEN
  ILEV = SELF%YRTH%ILEV
  SELF%F_RTH => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXRUISL
IF (IOFF > 0) THEN
  SELF%F_RUISL => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXRUISP
IF (IOFF > 0) THEN
  SELF%F_RUISP => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXRUISS
IF (IOFF > 0) THEN
  SELF%F_RUISS => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXSIC
IF (IOFF > 0) THEN
  SELF%F_SIC => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXTCLS
IF (IOFF > 0) THEN
  SELF%F_TCLS => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXTHW
IF (IOFF > 0) THEN
  SELF%F_THW => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXTN
IF (IOFF > 0) THEN
  SELF%F_TN => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXTPWCLS
IF (IOFF > 0) THEN
  SELF%F_TPWCLS => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXTRCU
IF (IOFF > 0) THEN
  ILEV = SELF%YTRCU%ILEV
  SELF%F_TRCU => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXTRCV
IF (IOFF > 0) THEN
  ILEV = SELF%YTRCV%ILEV
  SELF%F_TRCV => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXTRDU
IF (IOFF > 0) THEN
  ILEV = SELF%YTRDU%ILEV
  SELF%F_TRDU => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXTRDV
IF (IOFF > 0) THEN
  ILEV = SELF%YTRDV%ILEV
  SELF%F_TRDV => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXTRTU
IF (IOFF > 0) THEN
  ILEV = SELF%YTRTU%ILEV
  SELF%F_TRTU => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXTRTV
IF (IOFF > 0) THEN
  ILEV = SELF%YTRTV%ILEV
  SELF%F_TRTV => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF:IOFF+ILEV-1, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXTX
IF (IOFF > 0) THEN
  SELF%F_TX => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXUCLS
IF (IOFF > 0) THEN
  SELF%F_UCLS => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXUGST2
IF (IOFF > 0) THEN
  SELF%F_UGST2 => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXUGST
IF (IOFF > 0) THEN
  SELF%F_UGST => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXVCLS
IF (IOFF > 0) THEN
  SELF%F_VCLS => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXVEIN
IF (IOFF > 0) THEN
  SELF%F_VEIN => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXVGST2
IF (IOFF > 0) THEN
  SELF%F_VGST2 => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXVGST
IF (IOFF > 0) THEN
  SELF%F_VGST => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXVISICLD2
IF (IOFF > 0) THEN
  SELF%F_VISICLD2 => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXVISICLD
IF (IOFF > 0) THEN
  SELF%F_VISICLD => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXVISIHYD2
IF (IOFF > 0) THEN
  SELF%F_VISIHYD2 => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXVISIHYD
IF (IOFF > 0) THEN
  SELF%F_VISIHYD => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF

IOFF = SELF%YXFUPT%MXXDIAGH
IF (IOFF > 0) THEN
  SELF%F_XDIAGH => CREATE_FIELD_WRAP (SELF%XFUBUF_B (:, IOFF, :), PERSISTENT=.TRUE.)
ENDIF
...
IF (ASSOCIATED (SELF%F_CLWC2)) THEN
  CALL DELETE_TEMPORARY (SELF%F_CLWC2)
  SELF%F_CLWC2 => NULL () 
  SELF%CLWC2 => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_CLWC)) THEN
  CALL DELETE_TEMPORARY (SELF%F_CLWC)
  SELF%F_CLWC => NULL () 
  SELF%CLWC => NULL ()
ENDIF
...
IF (ASSOCIATED (SELF%F_DICQ)) THEN
  CALL DELETE_TEMPORARY (SELF%F_DICQ)
  SELF%F_DICQ => NULL () 
  SELF%DICQ => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_DICS)) THEN
  CALL DELETE_TEMPORARY (SELF%F_DICS)
  SELF%F_DICS => NULL () 
  SELF%DICS => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_DITQ)) THEN
  CALL DELETE_TEMPORARY (SELF%F_DITQ)
  SELF%F_DITQ => NULL () 
  SELF%DITQ => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_DITS)) THEN
  CALL DELETE_TEMPORARY (SELF%F_DITS)
  SELF%F_DITS => NULL () 
  SELF%DITS => NULL ()
ENDIF
...
IF (ASSOCIATED (SELF%F_HUN)) THEN
  CALL DELETE_TEMPORARY (SELF%F_HUN)
  SELF%F_HUN => NULL () 
  SELF%HUN => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_HUX)) THEN
  CALL DELETE_TEMPORARY (SELF%F_HUX)
  SELF%F_HUX => NULL () 
  SELF%HUX => NULL ()
ENDIF
...
IF (ASSOCIATED (SELF%F_NBBAS)) THEN
  CALL DELETE_TEMPORARY (SELF%F_NBBAS)
  SELF%F_NBBAS => NULL () 
  SELF%NBBAS => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_NBCON)) THEN
  CALL DELETE_TEMPORARY (SELF%F_NBCON)
  SELF%F_NBCON => NULL () 
  SELF%NBCON => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_NBHAU)) THEN
  CALL DELETE_TEMPORARY (SELF%F_NBHAU)
  SELF%F_NBHAU => NULL () 
  SELF%NBHAU => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_NBMOY)) THEN
  CALL DELETE_TEMPORARY (SELF%F_NBMOY)
  SELF%F_NBMOY => NULL () 
  SELF%NBMOY => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_NEB)) THEN
  CALL DELETE_TEMPORARY (SELF%F_NEB)
  SELF%F_NEB => NULL () 
  SELF%NEB => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_NEBT)) THEN
  CALL DELETE_TEMPORARY (SELF%F_NEBT)
  SELF%F_NEBT => NULL () 
  SELF%NEBT => NULL ()
ENDIF
...
IF (ASSOCIATED (SELF%F_PLCG)) THEN
  CALL DELETE_TEMPORARY (SELF%F_PLCG)
  SELF%F_PLCG => NULL () 
  SELF%PLCG => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_PLCH)) THEN
  CALL DELETE_TEMPORARY (SELF%F_PLCH)
  SELF%F_PLCH => NULL () 
  SELF%PLCH => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_PLCL)) THEN
  CALL DELETE_TEMPORARY (SELF%F_PLCL)
  SELF%F_PLCL => NULL () 
  SELF%PLCL => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_PLCN)) THEN
  CALL DELETE_TEMPORARY (SELF%F_PLCN)
  SELF%F_PLCN => NULL () 
  SELF%PLCN => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_PLSG)) THEN
  CALL DELETE_TEMPORARY (SELF%F_PLSG)
  SELF%F_PLSG => NULL () 
  SELF%PLSG => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_PLSH)) THEN
  CALL DELETE_TEMPORARY (SELF%F_PLSH)
  SELF%F_PLSH => NULL () 
  SELF%PLSH => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_PLSL)) THEN
  CALL DELETE_TEMPORARY (SELF%F_PLSL)
  SELF%F_PLSL => NULL () 
  SELF%PLSL => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_PLSN)) THEN
  CALL DELETE_TEMPORARY (SELF%F_PLSN)
  SELF%F_PLSN => NULL () 
  SELF%PLSN => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_PTYPE2)) THEN
  CALL DELETE_TEMPORARY (SELF%F_PTYPE2)
  SELF%F_PTYPE2 => NULL () 
  SELF%PTYPE2 => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_PTYPE)) THEN
  CALL DELETE_TEMPORARY (SELF%F_PTYPE)
  SELF%F_PTYPE => NULL () 
  SELF%PTYPE => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_PTYPESEV2)) THEN
  CALL DELETE_TEMPORARY (SELF%F_PTYPESEV2)
  SELF%F_PTYPESEV2 => NULL () 
  SELF%PTYPESEV2 => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_PTYPESEV)) THEN
  CALL DELETE_TEMPORARY (SELF%F_PTYPESEV)
...
IF (ASSOCIATED (SELF%F_RSO)) THEN
  CALL DELETE_TEMPORARY (SELF%F_RSO)
  SELF%F_RSO => NULL () 
  SELF%RSO => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_RTH)) THEN
  CALL DELETE_TEMPORARY (SELF%F_RTH)
  SELF%F_RTH => NULL () 
  SELF%RTH => NULL ()
ENDIF
...
IF (ASSOCIATED (SELF%F_SIC)) THEN
  CALL DELETE_TEMPORARY (SELF%F_SIC)
  SELF%F_SIC => NULL () 
  SELF%SIC => NULL ()
ENDIF
...
IF (ASSOCIATED (SELF%F_THW)) THEN
  CALL DELETE_TEMPORARY (SELF%F_THW)
  SELF%F_THW => NULL () 
  SELF%THW => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_TN)) THEN
  CALL DELETE_TEMPORARY (SELF%F_TN)
  SELF%F_TN => NULL () 
  SELF%TN => NULL ()
ENDIF
...
IF (ASSOCIATED (SELF%F_TRCU)) THEN
  CALL DELETE_TEMPORARY (SELF%F_TRCU)
  SELF%F_TRCU => NULL () 
  SELF%TRCU => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_TRCV)) THEN
  CALL DELETE_TEMPORARY (SELF%F_TRCV)
  SELF%F_TRCV => NULL () 
  SELF%TRCV => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_TRDU)) THEN
  CALL DELETE_TEMPORARY (SELF%F_TRDU)
  SELF%F_TRDU => NULL () 
  SELF%TRDU => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_TRDV)) THEN
  CALL DELETE_TEMPORARY (SELF%F_TRDV)
  SELF%F_TRDV => NULL () 
  SELF%TRDV => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_TRTU)) THEN
  CALL DELETE_TEMPORARY (SELF%F_TRTU)
  SELF%F_TRTU => NULL () 
  SELF%TRTU => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_TRTV)) THEN
  CALL DELETE_TEMPORARY (SELF%F_TRTV)
  SELF%F_TRTV => NULL () 
  SELF%TRTV => NULL ()
ENDIF

IF (ASSOCIATED (SELF%F_TX)) THEN
  CALL DELETE_TEMPORARY (SELF%F_TX)
  SELF%F_TX => NULL () 
  SELF%TX => NULL ()
ENDIF
...
IF (ASSOCIATED (SELF%F_UGST2)) THEN
  CALL DELETE_TEMPORARY (SELF%F_UGST2)
  SELF%F_UGST2 => NULL () 
  SELF%UGST2 => NULL ()
ENDIF
...
IF (ASSOCIATED (SELF%F_VGST2)) THEN
  CALL DELETE_TEMPORARY (SELF%F_VGST2)
  SELF%F_VGST2 => NULL () 
  SELF%VGST2 => NULL ()
ENDIF
...
IF (ASSOCIATED (SELF%F_VISICLD2)) THEN
  CALL DELETE_TEMPORARY (SELF%F_VISICLD2)
...
IF (ASSOCIATED (SELF%F_VISIHYD2)) THEN
  CALL DELETE_TEMPORARY (SELF%F_VISIHYD2)
...
IF (ASSOCIATED (SELF%F_XDIAGH)) THEN
  CALL DELETE_TEMPORARY (SELF%F_XDIAGH)
  SELF%F_XDIAGH => NULL () 
  SELF%XDIAGH => NULL ()
ENDIF
