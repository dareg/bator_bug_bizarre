MODULE Compiler_Features
  ...
  !
  ! NOTE: we currently have no machine to test this compiler
  !       so cannot be sure this version really works
  !---------------------------------------------------
...
CONTAINS 
  ...
  subroutine flush_aeolus(fileunit)
    ...
    ! flush() seems not available in NAGware f95, so do nothing
    !call flush(fileunit)
