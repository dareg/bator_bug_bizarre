...
if (numargs < 4) then
  ...
  call odb_abort('odb2mysql(main)',&
                 'Usage: '//trim(a_out)//' dbname refcol massaged_wishlist_file minmax_file view(s)',-1)
endif
...
do jtbl=ntables,1,-1
  call MPL_getarg(4+jtbl,viewname)
  ...
  do jp=myproc,npools,nproc
    call process_table(h, jp, irefcol, &
  ...
  call system('chmod a+r '//trim(filename))
...
subroutine process_table(h, jp, irefcol, &
...
do jrow=1,nrows
  ...
  idx = idx + 1
  call compress_print(rowbuf(1:idx))
enddo
...
subroutine compress_print(rowbuf)
implicit none
