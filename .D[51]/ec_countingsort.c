...
/* ec_countingsort() : Fortran-callable counting-sort */
...
#define  ALLOC(x,size)    \
 { ll_t bytes = (ll_t)sizeof(*x) * (size); \
   ...
   if (!x) { fprintf(stderr, \
                     ...
                     #x, bytes, __FILE__, __LINE__); _exit(SIGABRT); } }
...
{ \
  ...
  ALLOC(A, n); \
  if (index) { \
   printf("index is NOT NULL %d\n", index_adj); \
  } else { \
   printf("index is NULL %d\n", index_adj); \
  }  \
...
void
ec_countingsort(const    int *Mode,
		 const    int *N,
		 const    int *Inc,
	                  ...
	                  int *retc)
{
  ...
  printf("mode = %d, method = %d, n = %d, rc = %d, inc = %d, addr = %d, nidx = %d, index_adj = %d, irev = %d\n",
          mode, method, n, rc, inc, addr, nidx, index_adj, irev);
  if (n <= 0) goto finish;
