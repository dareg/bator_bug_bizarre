...
USE ObsConvertGlobalDefinitions
USE ObsConvertBufrToOdb
USE ObsConvertNetcdfToOdb
USE ObsConvertHdfToOdb
USE ObsConvertWritingsToOdb
USE ObsConvertPrintings
USE ObsConvertInits
USE ObsConvertReadings
USE ObsConvertSaisies
USE ObsConvertTools
USE ObsConvertPoolBalance
USE YOMDB  
...
USE MPL_MODULE
USE ODB_MODULE
USE ObsConvertDateAndTime, ONLY : TDate, NewDate, PrintDate
...
!--------------------------------
 ...
 DO jfic = 1, inbfic_loc(myproc)
   ...
   CASE('BUFR')
     if (INbTypeBufr > 0) CALL BufrPrefetch(numfic)
   CASE('NETCDF')
...
!--------------------------
 ...
 DO jpool = 1, INBPOOL
   ...
   IF (ITNBOPO(jpool) == 0) THEN
     if (jpool == 1) WRITE(BATOUT,'("WARNING : bator - no data in pool",I4,/)') NUMPOOL
     CYCLE
   ENDIF
