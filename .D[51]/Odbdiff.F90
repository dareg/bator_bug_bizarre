...
implicit none
REAL(KIND=JPRD), parameter :: rscale = 1000000._JPRD
REAL(KIND=JPRD), parameter :: eps = tiny(rscale)
...
character(len=8) :: ztr
REAL(KIND=JPRD)   , allocatable :: x1(:,:), x2(:,:), z(:)
...
REAL(KIND=JPRD) :: ZHOOK_HANDLE, ZH1(3), ZH2(5)
REAL(KIND=JPRD) :: RAD2DEG, x, rlatinc, lat0, lat1, lat2
...
if (numargs /= 13) then
  ...
  if (numargs >= 13) then
    ...
    rlatinc = abs(rlatinc)
    rlatinc = max(rlatinc,0.1_JPRD) ! Minimum lat.band slice at least 0.1 degrees
  endif
