...
! Convert a set of odb2 input files into an IFS odb1 data base
...
! Add "@" to croot in case not present
  ...
  if (nodb2==0) then
     write(6,*) &
    & 'Usage: odb2_to_odb1_era -i odb2file [-l dbname -r @mytableroot -s npools -o iseqno_offset -w column@table=my_value ]'
     call abort()
  endif
...
! Open ODB-1, read schema as defined in IFS and place its contents in sch1
...
! Do the work
  ...
  do ipool=iproc,npools,nproc           ! my pools only
     if(ipool==1) call odb1_header(sch1)! Print summary of IFS schema only once
     ...
     ! Fill part of ODB1 schema sch1 that appears in the odb2 columns
       call odb22odb1(sch2,db2,sch1,croot)
       call odb1_header(sch2)  ! Print summary

     ! Merge the sch2 schema into the bigger IFS odb1 schema
