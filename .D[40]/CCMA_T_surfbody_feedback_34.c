#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_surfbody_feedback_34 1
#include "CCMA.h"
...
PUBLIC int
CCMA_Pack_T_surfbody_feedback_34(void *T)
{
  int Nbytes = 0;
  TABLE_surfbody_feedback_34 *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
...
PUBLIC int
CCMA_Unpack_T_surfbody_feedback_34(void *T)
{
  int Nbytes = 0;
  TABLE_surfbody_feedback_34 *P = T;
  if (P->Is_loaded) {
...
PUBLIC int
CCMA_Sel_T_surfbody_feedback_34(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_surfbody_feedback_34 *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(surfbody_feedback_34);
  return P->Nrows;
}


PreGetTable(CCMA, d, double, surfbody_feedback_34)
  Call_CopyGet_TABLE(CCMA, d, 1, surfbody_feedback_34, Bitfield, D, datum_status, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(CCMA, d, 2, surfbody_feedback_34, Bitfield, D, datum_sfc_event, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(CCMA, d, 3, surfbody_feedback_34, pk9real, D, an_depar, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 4, surfbody_feedback_34, pk9real, D, fg_depar, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 5, surfbody_feedback_34, pk9real, D, snow_depth, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 6, surfbody_feedback_34, pk9real, D, snow_density, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 7, surfbody_feedback_34, pk9real, D, lsm, Count, DATATYPE_REAL8);
PostGetTable(d, double, surfbody_feedback_34)


PrePutTable(CCMA, d, double, surfbody_feedback_34)
  Call_CopyPut_TABLE(CCMA, d, 1, surfbody_feedback_34, Bitfield, datum_status, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(CCMA, d, 2, surfbody_feedback_34, Bitfield, datum_sfc_event, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(CCMA, d, 3, surfbody_feedback_34, pk9real, an_depar, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 4, surfbody_feedback_34, pk9real, fg_depar, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 5, surfbody_feedback_34, pk9real, snow_depth, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 6, surfbody_feedback_34, pk9real, snow_density, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 7, surfbody_feedback_34, pk9real, lsm, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, surfbody_feedback_34)
...
PostLoadTable(surfbody_feedback_34)
...
PostStoreTable(surfbody_feedback_34)

DefineLookupTable(surfbody_feedback_34)

PUBLIC void
CCMA_Dim_T_surfbody_feedback_34(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_surfbody_feedback_34 *P = T;
  Call_LookupTable(surfbody_feedback_34, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
CCMA_Swapout_T_surfbody_feedback_34(void *T)
{
  TABLE_surfbody_feedback_34 *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  ODBMAC_TRACE_SWAPOUT(surfbody_feedback_34,7);
}

DefineRemoveTable(CCMA, surfbody_feedback_34)

PUBLIC int
CCMA_Sql_T_surfbody_feedback_34(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_surfbody_feedback_34(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_surfbody_feedback_34 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_surfbody_feedback_34);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 7);
  InitDS(Bitfield, DATATYPE_BITFIELD, datum_status, surfbody_feedback_34, 1);
  InitDS(Bitfield, DATATYPE_BITFIELD, datum_sfc_event, surfbody_feedback_34, 1);
  InitDS(pk9real, DATATYPE_REAL8, an_depar, surfbody_feedback_34, 9);
  InitDS(pk9real, DATATYPE_REAL8, fg_depar, surfbody_feedback_34, 9);
  InitDS(pk9real, DATATYPE_REAL8, snow_depth, surfbody_feedback_34, 9);
  InitDS(pk9real, DATATYPE_REAL8, snow_density, surfbody_feedback_34, 9);
  InitDS(pk9real, DATATYPE_REAL8, lsm, surfbody_feedback_34, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@surfbody_feedback_34"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 283;
    pfcom->wt = 358.000259;
    pfcom->tags = CCMA_Set_T_surfbody_feedback_34_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_surfbody_feedback_34_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_surfbody_feedback_34;
    pfcom->swapout = CCMA_Swapout_T_surfbody_feedback_34;
    pfcom->dim = CCMA_Dim_T_surfbody_feedback_34;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_surfbody_feedback_34;
    pfcom->remove = CCMA_Remove_T_surfbody_feedback_34;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_surfbody_feedback_34; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_surfbody_feedback_34; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_surfbody_feedback_34;
    pfcom->store = CCMA_Store_T_surfbody_feedback_34;
    pfcom->pack = CCMA_Pack_T_surfbody_feedback_34;
    pfcom->unpack = CCMA_Unpack_T_surfbody_feedback_34;
    pfcom->sql = CCMA_Sql_T_surfbody_feedback_34;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
...
/* *************** End of TABLE "surfbody_feedback_34" *************** */
