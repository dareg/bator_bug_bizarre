...
PUBLIC int
CCMA_Sel_T_enkf_39(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_enkf_39 *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(enkf_39);
  return P->Nrows;
}


PreGetTable(CCMA, d, double, enkf_39)
  Call_CopyGet_TABLE(CCMA, d, 1, enkf_39, pk1int, D, member, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 2, enkf_39, pk9real, D, hprior, Count, DATATYPE_REAL8);
PostGetTable(d, double, enkf_39)


PrePutTable(CCMA, d, double, enkf_39)
  Call_CopyPut_TABLE(CCMA, d, 1, enkf_39, pk1int, member, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 2, enkf_39, pk9real, hprior, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, enkf_39)
...
PostStoreTable(enkf_39)

DefineLookupTable(enkf_39)

PUBLIC void
CCMA_Dim_T_enkf_39(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_enkf_39 *P = T;
  Call_LookupTable(enkf_39, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
CCMA_Swapout_T_enkf_39(void *T)
{
  TABLE_enkf_39 *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  ODBMAC_TRACE_SWAPOUT(enkf_39,2);
}

DefineRemoveTable(CCMA, enkf_39)

PUBLIC int
CCMA_Sql_T_enkf_39(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_enkf_39(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_enkf_39 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_enkf_39);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 2);
  InitDS(pk1int, DATATYPE_INT4, member, enkf_39, 1);
  InitDS(pk9real, DATATYPE_REAL8, hprior, enkf_39, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@enkf_39"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 68;
    pfcom->wt = 358.000044;
    pfcom->tags = CCMA_Set_T_enkf_39_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_enkf_39_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_enkf_39;
    pfcom->swapout = CCMA_Swapout_T_enkf_39;
    pfcom->dim = CCMA_Dim_T_enkf_39;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_enkf_39;
    pfcom->remove = CCMA_Remove_T_enkf_39;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_enkf_39; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_enkf_39; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_enkf_39;
    pfcom->store = CCMA_Store_T_enkf_39;
    pfcom->pack = CCMA_Pack_T_enkf_39;
    pfcom->unpack = CCMA_Unpack_T_enkf_39;
    pfcom->sql = CCMA_Sql_T_enkf_39;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
