...
PUBLIC int
CCMA_Pack_T_auxiliary_body(void *T)
{
  int Nbytes = 0;
  TABLE_auxiliary_body *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk9real, datum_aux_1); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk9real, datum_aux_2); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk9real, datum_aux_3); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk9real, datum_aux_4); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, pk9real, datum_aux_5); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, pk9real, datum_aux_6); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, pk9real, datum_aux_7); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, CCMA, pk9real, datum_aux_8); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, CCMA, pk9real, datum_aux_9); CHECK_PDS_ERROR(9);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_auxiliary_body(void *T)
{
  int Nbytes = 0;
  TABLE_auxiliary_body *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk9real, datum_aux_1); Nbytes += BYTESIZE(P->datum_aux_1.d);
    UseDS(P, CCMA, pk9real, datum_aux_2); Nbytes += BYTESIZE(P->datum_aux_2.d);
    UseDS(P, CCMA, pk9real, datum_aux_3); Nbytes += BYTESIZE(P->datum_aux_3.d);
    UseDS(P, CCMA, pk9real, datum_aux_4); Nbytes += BYTESIZE(P->datum_aux_4.d);
    UseDS(P, CCMA, pk9real, datum_aux_5); Nbytes += BYTESIZE(P->datum_aux_5.d);
    UseDS(P, CCMA, pk9real, datum_aux_6); Nbytes += BYTESIZE(P->datum_aux_6.d);
    UseDS(P, CCMA, pk9real, datum_aux_7); Nbytes += BYTESIZE(P->datum_aux_7.d);
    UseDS(P, CCMA, pk9real, datum_aux_8); Nbytes += BYTESIZE(P->datum_aux_8.d);
    UseDS(P, CCMA, pk9real, datum_aux_9); Nbytes += BYTESIZE(P->datum_aux_9.d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_auxiliary_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_auxiliary_body *P = T;
...
PreGetTable(CCMA, d, double, auxiliary_body)
  Call_CopyGet_TABLE(CCMA, d, 1, auxiliary_body, pk9real, D, datum_aux_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, auxiliary_body, pk9real, D, datum_aux_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, auxiliary_body, pk9real, D, datum_aux_3, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 4, auxiliary_body, pk9real, D, datum_aux_4, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 5, auxiliary_body, pk9real, D, datum_aux_5, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 6, auxiliary_body, pk9real, D, datum_aux_6, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 7, auxiliary_body, pk9real, D, datum_aux_7, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 8, auxiliary_body, pk9real, D, datum_aux_8, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 9, auxiliary_body, pk9real, D, datum_aux_9, Count, DATATYPE_REAL8);
PostGetTable(d, double, auxiliary_body)


PrePutTable(CCMA, d, double, auxiliary_body)
  Call_CopyPut_TABLE(CCMA, d, 1, auxiliary_body, pk9real, datum_aux_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, auxiliary_body, pk9real, datum_aux_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, auxiliary_body, pk9real, datum_aux_3, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 4, auxiliary_body, pk9real, datum_aux_4, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 5, auxiliary_body, pk9real, datum_aux_5, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 6, auxiliary_body, pk9real, datum_aux_6, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 7, auxiliary_body, pk9real, datum_aux_7, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 8, auxiliary_body, pk9real, datum_aux_8, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 9, auxiliary_body, pk9real, datum_aux_9, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, auxiliary_body)

PreLoadTable(CCMA, auxiliary_body);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_1);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_2);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_3);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_4);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_5);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_6);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_7);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_8);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_9);
PostLoadTable(auxiliary_body)

PreStoreTable(CCMA, auxiliary_body);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_1);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_2);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_3);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_4);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_5);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_6);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_7);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_8);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_9);
PostStoreTable(auxiliary_body)

DefineLookupTable(auxiliary_body)

PUBLIC void
CCMA_Dim_T_auxiliary_body(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_auxiliary_body(void *T)
{
  TABLE_auxiliary_body *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, auxiliary_body)

PUBLIC int
CCMA_Sql_T_auxiliary_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_auxiliary_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_auxiliary_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_auxiliary_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 9);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 376;
    pfcom->wt = 358.000363;
    pfcom->tags = CCMA_Set_T_auxiliary_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_auxiliary_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_auxiliary_body;
    pfcom->swapout = CCMA_Swapout_T_auxiliary_body;
    pfcom->dim = CCMA_Dim_T_auxiliary_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_auxiliary_body;
    pfcom->remove = CCMA_Remove_T_auxiliary_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_auxiliary_body; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_auxiliary_body; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_auxiliary_body;
    pfcom->store = CCMA_Store_T_auxiliary_body;
    pfcom->pack = CCMA_Pack_T_auxiliary_body;
    pfcom->unpack = CCMA_Unpack_T_auxiliary_body;
    pfcom->sql = CCMA_Sql_T_auxiliary_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
