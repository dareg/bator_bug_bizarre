...
PUBLIC int
CCMA_Pack_T_enda_100(void *T)
{
  int Nbytes = 0;
  TABLE_enda_100 *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk1int, member); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, Bitfield, report_status); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, Bitfield, report_event1); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk9real, obsvalue); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, Bitfield, datum_anflag); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, Bitfield, datum_status); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, Bitfield, datum_event1); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, CCMA, pk9real, biascorr); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, CCMA, pk9real, biascorr_fg); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, CCMA, pk9real, an_depar); CHECK_PDS_ERROR(10);
    PDS = PackDS(P, CCMA, pk9real, fg_depar); CHECK_PDS_ERROR(11);
    PDS = PackDS(P, CCMA, pk9real, qc_pge); CHECK_PDS_ERROR(12);
    PDS = PackDS(P, CCMA, pk9real, final_obs_error); CHECK_PDS_ERROR(13);
    PDS = PackDS(P, CCMA, pk9real, obs_error); CHECK_PDS_ERROR(14);
    PDS = PackDS(P, CCMA, pk9real, fg_error); CHECK_PDS_ERROR(15);
    PDS = PackDS(P, CCMA, pk9real, skintemp_1); CHECK_PDS_ERROR(16);
    PDS = PackDS(P, CCMA, pk9real, skintemp_2); CHECK_PDS_ERROR(17);
    PDS = PackDS(P, CCMA, pk9real, skintemp_3); CHECK_PDS_ERROR(18);
    PDS = PackDS(P, CCMA, pk9real, skintemp_4); CHECK_PDS_ERROR(19);
    PDS = PackDS(P, CCMA, pk9real, skintemp_5); CHECK_PDS_ERROR(20);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_enda_100(void *T)
{
  int Nbytes = 0;
  TABLE_enda_100 *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk1int, member); Nbytes += BYTESIZE(P->member.d);
    ...
    UseDS(P, CCMA, pk9real, final_obs_error); Nbytes += BYTESIZE(P->final_obs_error.d);
    UseDS(P, CCMA, pk9real, obs_error); Nbytes += BYTESIZE(P->obs_error.d);
    UseDS(P, CCMA, pk9real, fg_error); Nbytes += BYTESIZE(P->fg_error.d);
...
PUBLIC int
CCMA_Sel_T_enda_100(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_enda_100 *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(enda_100);
  return P->Nrows;
}


PreGetTable(CCMA, d, double, enda_100)
  Call_CopyGet_TABLE(CCMA, d, 1, enda_100, pk1int, D, member, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 2, enda_100, Bitfield, D, report_status, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(CCMA, d, 3, enda_100, Bitfield, D, report_event1, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(CCMA, d, 4, enda_100, pk9real, D, obsvalue, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 5, enda_100, Bitfield, D, datum_anflag, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(CCMA, d, 6, enda_100, Bitfield, D, datum_status, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(CCMA, d, 7, enda_100, Bitfield, D, datum_event1, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(CCMA, d, 8, enda_100, pk9real, D, biascorr, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 9, enda_100, pk9real, D, biascorr_fg, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 10, enda_100, pk9real, D, an_depar, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 11, enda_100, pk9real, D, fg_depar, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 12, enda_100, pk9real, D, qc_pge, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 13, enda_100, pk9real, D, final_obs_error, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 14, enda_100, pk9real, D, obs_error, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 15, enda_100, pk9real, D, fg_error, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 16, enda_100, pk9real, D, skintemp_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 17, enda_100, pk9real, D, skintemp_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 18, enda_100, pk9real, D, skintemp_3, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 19, enda_100, pk9real, D, skintemp_4, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 20, enda_100, pk9real, D, skintemp_5, Count, DATATYPE_REAL8);
PostGetTable(d, double, enda_100)


PrePutTable(CCMA, d, double, enda_100)
  Call_CopyPut_TABLE(CCMA, d, 1, enda_100, pk1int, member, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 2, enda_100, Bitfield, report_status, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(CCMA, d, 3, enda_100, Bitfield, report_event1, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(CCMA, d, 4, enda_100, pk9real, obsvalue, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 5, enda_100, Bitfield, datum_anflag, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(CCMA, d, 6, enda_100, Bitfield, datum_status, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(CCMA, d, 7, enda_100, Bitfield, datum_event1, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(CCMA, d, 8, enda_100, pk9real, biascorr, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 9, enda_100, pk9real, biascorr_fg, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 10, enda_100, pk9real, an_depar, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 11, enda_100, pk9real, fg_depar, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 12, enda_100, pk9real, qc_pge, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 13, enda_100, pk9real, final_obs_error, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 14, enda_100, pk9real, obs_error, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 15, enda_100, pk9real, fg_error, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 16, enda_100, pk9real, skintemp_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 17, enda_100, pk9real, skintemp_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 18, enda_100, pk9real, skintemp_3, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 19, enda_100, pk9real, skintemp_4, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 20, enda_100, pk9real, skintemp_5, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, enda_100)

PreLoadTable(CCMA, enda_100);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, member);
  ...
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, final_obs_error);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_error);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, fg_error);
...
PostLoadTable(enda_100)

PreStoreTable(CCMA, enda_100);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, member);
  ...
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, final_obs_error);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_error);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, fg_error);
...
PostStoreTable(enda_100)

DefineLookupTable(enda_100)

PUBLIC void
CCMA_Dim_T_enda_100(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_enda_100 *P = T;
  Call_LookupTable(enda_100, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
CCMA_Swapout_T_enda_100(void *T)
{
  TABLE_enda_100 *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  ODBMAC_TRACE_SWAPOUT(enda_100,20);
}

DefineRemoveTable(CCMA, enda_100)

PUBLIC int
CCMA_Sql_T_enda_100(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_enda_100(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_enda_100 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_enda_100);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 20);
  InitDS(pk1int, DATATYPE_INT4, member, enda_100, 1);
  InitDS(Bitfield, DATATYPE_BITFIELD, report_status, enda_100, 1);
  InitDS(Bitfield, DATATYPE_BITFIELD, report_event1, enda_100, 1);
  InitDS(pk9real, DATATYPE_REAL8, obsvalue, enda_100, 9);
  InitDS(Bitfield, DATATYPE_BITFIELD, datum_anflag, enda_100, 1);
  InitDS(Bitfield, DATATYPE_BITFIELD, datum_status, enda_100, 1);
  InitDS(Bitfield, DATATYPE_BITFIELD, datum_event1, enda_100, 1);
  InitDS(pk9real, DATATYPE_REAL8, biascorr, enda_100, 9);
  InitDS(pk9real, DATATYPE_REAL8, biascorr_fg, enda_100, 9);
  InitDS(pk9real, DATATYPE_REAL8, an_depar, enda_100, 9);
  InitDS(pk9real, DATATYPE_REAL8, fg_depar, enda_100, 9);
  InitDS(pk9real, DATATYPE_REAL8, qc_pge, enda_100, 9);
  InitDS(pk9real, DATATYPE_REAL8, final_obs_error, enda_100, 9);
  InitDS(pk9real, DATATYPE_REAL8, obs_error, enda_100, 9);
  InitDS(pk9real, DATATYPE_REAL8, fg_error, enda_100, 9);
  InitDS(pk9real, DATATYPE_REAL8, skintemp_1, enda_100, 9);
  InitDS(pk9real, DATATYPE_REAL8, skintemp_2, enda_100, 9);
  InitDS(pk9real, DATATYPE_REAL8, skintemp_3, enda_100, 9);
  InitDS(pk9real, DATATYPE_REAL8, skintemp_4, enda_100, 9);
  InitDS(pk9real, DATATYPE_REAL8, skintemp_5, enda_100, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@enda_100"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 249;
    pfcom->wt = 358.000225;
    pfcom->tags = CCMA_Set_T_enda_100_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_enda_100_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_enda_100;
    pfcom->swapout = CCMA_Swapout_T_enda_100;
    pfcom->dim = CCMA_Dim_T_enda_100;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_enda_100;
    pfcom->remove = CCMA_Remove_T_enda_100;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_enda_100; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_enda_100; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_enda_100;
    pfcom->store = CCMA_Store_T_enda_100;
    pfcom->pack = CCMA_Pack_T_enda_100;
    pfcom->unpack = CCMA_Unpack_T_enda_100;
    pfcom->sql = CCMA_Sql_T_enda_100;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
