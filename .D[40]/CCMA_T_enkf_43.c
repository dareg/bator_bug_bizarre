...
PUBLIC int
CCMA_Sel_T_enkf_43(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_enkf_43 *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(enkf_43);
  return P->Nrows;
}


PreGetTable(CCMA, d, double, enkf_43)
  Call_CopyGet_TABLE(CCMA, d, 1, enkf_43, pk1int, D, member, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 2, enkf_43, pk9real, D, hprior, Count, DATATYPE_REAL8);
PostGetTable(d, double, enkf_43)


PrePutTable(CCMA, d, double, enkf_43)
  Call_CopyPut_TABLE(CCMA, d, 1, enkf_43, pk1int, member, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 2, enkf_43, pk9real, hprior, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, enkf_43)
...
PostStoreTable(enkf_43)

DefineLookupTable(enkf_43)

PUBLIC void
CCMA_Dim_T_enkf_43(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_enkf_43 *P = T;
  Call_LookupTable(enkf_43, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
CCMA_Swapout_T_enkf_43(void *T)
{
  TABLE_enkf_43 *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  ODBMAC_TRACE_SWAPOUT(enkf_43,2);
}

DefineRemoveTable(CCMA, enkf_43)

PUBLIC int
CCMA_Sql_T_enkf_43(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_enkf_43(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_enkf_43 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_enkf_43);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 2);
  InitDS(pk1int, DATATYPE_INT4, member, enkf_43, 1);
  InitDS(pk9real, DATATYPE_REAL8, hprior, enkf_43, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@enkf_43"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 72;
    pfcom->wt = 358.000048;
    pfcom->tags = CCMA_Set_T_enkf_43_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_enkf_43_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_enkf_43;
    pfcom->swapout = CCMA_Swapout_T_enkf_43;
    pfcom->dim = CCMA_Dim_T_enkf_43;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_enkf_43;
    pfcom->remove = CCMA_Remove_T_enkf_43;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_enkf_43; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_enkf_43; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_enkf_43;
    pfcom->store = CCMA_Store_T_enkf_43;
    pfcom->pack = CCMA_Pack_T_enkf_43;
    pfcom->unpack = CCMA_Unpack_T_enkf_43;
    pfcom->sql = CCMA_Sql_T_enkf_43;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
