...
PUBLIC int
CCMA_Pack_T_aeolus_l2c(void *T)
{
  int Nbytes = 0;
  TABLE_aeolus_l2c *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk9real, hlos_ob_err); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk9real, hlos_fg); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk9real, u_fg); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk9real, u_fg_err); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, pk9real, v_fg); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, pk9real, v_fg_err); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, pk9real, hlos_fg_err); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, CCMA, pk9real, hlos_an); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, CCMA, pk9real, hlos_an_err); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, CCMA, pk9real, u_an); CHECK_PDS_ERROR(10);
    PDS = PackDS(P, CCMA, pk9real, v_an); CHECK_PDS_ERROR(11);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_aeolus_l2c(void *T)
{
  int Nbytes = 0;
  TABLE_aeolus_l2c *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk9real, hlos_ob_err); Nbytes += BYTESIZE(P->hlos_ob_err.d);
    UseDS(P, CCMA, pk9real, hlos_fg); Nbytes += BYTESIZE(P->hlos_fg.d);
    UseDS(P, CCMA, pk9real, u_fg); Nbytes += BYTESIZE(P->u_fg.d);
    UseDS(P, CCMA, pk9real, u_fg_err); Nbytes += BYTESIZE(P->u_fg_err.d);
    UseDS(P, CCMA, pk9real, v_fg); Nbytes += BYTESIZE(P->v_fg.d);
    UseDS(P, CCMA, pk9real, v_fg_err); Nbytes += BYTESIZE(P->v_fg_err.d);
    UseDS(P, CCMA, pk9real, hlos_fg_err); Nbytes += BYTESIZE(P->hlos_fg_err.d);
    UseDS(P, CCMA, pk9real, hlos_an); Nbytes += BYTESIZE(P->hlos_an.d);
    UseDS(P, CCMA, pk9real, hlos_an_err); Nbytes += BYTESIZE(P->hlos_an_err.d);
    UseDS(P, CCMA, pk9real, u_an); Nbytes += BYTESIZE(P->u_an.d);
    UseDS(P, CCMA, pk9real, v_an); Nbytes += BYTESIZE(P->v_an.d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_aeolus_l2c(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_aeolus_l2c *P = T;
...
PreGetTable(CCMA, d, double, aeolus_l2c)
  Call_CopyGet_TABLE(CCMA, d, 1, aeolus_l2c, pk9real, D, hlos_ob_err, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, aeolus_l2c, pk9real, D, hlos_fg, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, aeolus_l2c, pk9real, D, u_fg, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 4, aeolus_l2c, pk9real, D, u_fg_err, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 5, aeolus_l2c, pk9real, D, v_fg, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 6, aeolus_l2c, pk9real, D, v_fg_err, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 7, aeolus_l2c, pk9real, D, hlos_fg_err, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 8, aeolus_l2c, pk9real, D, hlos_an, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 9, aeolus_l2c, pk9real, D, hlos_an_err, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 10, aeolus_l2c, pk9real, D, u_an, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 11, aeolus_l2c, pk9real, D, v_an, Count, DATATYPE_REAL8);
PostGetTable(d, double, aeolus_l2c)


PrePutTable(CCMA, d, double, aeolus_l2c)
  Call_CopyPut_TABLE(CCMA, d, 1, aeolus_l2c, pk9real, hlos_ob_err, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, aeolus_l2c, pk9real, hlos_fg, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, aeolus_l2c, pk9real, u_fg, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 4, aeolus_l2c, pk9real, u_fg_err, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 5, aeolus_l2c, pk9real, v_fg, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 6, aeolus_l2c, pk9real, v_fg_err, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 7, aeolus_l2c, pk9real, hlos_fg_err, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 8, aeolus_l2c, pk9real, hlos_an, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 9, aeolus_l2c, pk9real, hlos_an_err, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 10, aeolus_l2c, pk9real, u_an, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 11, aeolus_l2c, pk9real, v_an, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, aeolus_l2c)

PreLoadTable(CCMA, aeolus_l2c);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hlos_ob_err);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hlos_fg);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, u_fg);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, u_fg_err);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, v_fg);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, v_fg_err);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hlos_fg_err);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hlos_an);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hlos_an_err);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, u_an);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, v_an);
PostLoadTable(aeolus_l2c)

PreStoreTable(CCMA, aeolus_l2c);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hlos_ob_err);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hlos_fg);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, u_fg);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, u_fg_err);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, v_fg);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, v_fg_err);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hlos_fg_err);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hlos_an);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hlos_an_err);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, u_an);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, v_an);
PostStoreTable(aeolus_l2c)

DefineLookupTable(aeolus_l2c)

PUBLIC void
CCMA_Dim_T_aeolus_l2c(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_aeolus_l2c(void *T)
{
  TABLE_aeolus_l2c *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, aeolus_l2c)

PUBLIC int
CCMA_Sql_T_aeolus_l2c(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_aeolus_l2c(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_aeolus_l2c *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_aeolus_l2c);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 11);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 28;
    pfcom->wt = 3.003004;
    pfcom->tags = CCMA_Set_T_aeolus_l2c_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_aeolus_l2c_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_aeolus_l2c;
    pfcom->swapout = CCMA_Swapout_T_aeolus_l2c;
    pfcom->dim = CCMA_Dim_T_aeolus_l2c;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_aeolus_l2c;
    pfcom->remove = CCMA_Remove_T_aeolus_l2c;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_aeolus_l2c; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_aeolus_l2c; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_aeolus_l2c;
    pfcom->store = CCMA_Store_T_aeolus_l2c;
    pfcom->pack = CCMA_Pack_T_aeolus_l2c;
    pfcom->unpack = CCMA_Unpack_T_aeolus_l2c;
    pfcom->sql = CCMA_Sql_T_aeolus_l2c;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
