...
PUBLIC int
CCMA_Pack_T_allsky_body(void *T)
{
  int Nbytes = 0;
  TABLE_allsky_body *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk9real, tbvalue); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk9real, tbvaluetl); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk9real, tbvaluead); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk1int, datum_tbflag); CHECK_PDS_ERROR(4);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_allsky_body(void *T)
{
  int Nbytes = 0;
  TABLE_allsky_body *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk9real, tbvalue); Nbytes += BYTESIZE(P->tbvalue.d);
    UseDS(P, CCMA, pk9real, tbvaluetl); Nbytes += BYTESIZE(P->tbvaluetl.d);
    UseDS(P, CCMA, pk9real, tbvaluead); Nbytes += BYTESIZE(P->tbvaluead.d);
    UseDS(P, CCMA, pk1int, datum_tbflag); Nbytes += BYTESIZE(P->datum_tbflag.d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_allsky_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_allsky_body *P = T;
...
PreGetTable(CCMA, d, double, allsky_body)
  Call_CopyGet_TABLE(CCMA, d, 1, allsky_body, pk9real, D, tbvalue, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, allsky_body, pk9real, D, tbvaluetl, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, allsky_body, pk9real, D, tbvaluead, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 4, allsky_body, pk1int, D, datum_tbflag, Count, DATATYPE_INT4);
PostGetTable(d, double, allsky_body)


PrePutTable(CCMA, d, double, allsky_body)
  Call_CopyPut_TABLE(CCMA, d, 1, allsky_body, pk9real, tbvalue, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, allsky_body, pk9real, tbvaluetl, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, allsky_body, pk9real, tbvaluead, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 4, allsky_body, pk1int, datum_tbflag, D, Count, DATATYPE_INT4);
...
PreLoadTable(CCMA, allsky_body);
  ...
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, tbvaluetl);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, tbvaluead);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, datum_tbflag);
...
PreStoreTable(CCMA, allsky_body);
  ...
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, tbvaluetl);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, tbvaluead);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, datum_tbflag);
PostStoreTable(allsky_body)

DefineLookupTable(allsky_body)

PUBLIC void
CCMA_Dim_T_allsky_body(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_allsky_body(void *T)
{
  TABLE_allsky_body *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, allsky_body)

PUBLIC int
CCMA_Sql_T_allsky_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_allsky_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_allsky_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_allsky_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 4);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 375;
    pfcom->wt = 358.000359;
    pfcom->tags = CCMA_Set_T_allsky_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_allsky_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_allsky_body;
    pfcom->swapout = CCMA_Swapout_T_allsky_body;
    pfcom->dim = CCMA_Dim_T_allsky_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_allsky_body;
    pfcom->remove = CCMA_Remove_T_allsky_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_allsky_body; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_allsky_body; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_allsky_body;
    pfcom->store = CCMA_Store_T_allsky_body;
    pfcom->pack = CCMA_Pack_T_allsky_body;
    pfcom->unpack = CCMA_Unpack_T_allsky_body;
    pfcom->sql = CCMA_Sql_T_allsky_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
