...
PUBLIC int
CCMA_Pack_T_aeolus_auxmet(void *T)
{
  int Nbytes = 0;
  TABLE_aeolus_auxmet *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk9real, lev); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk9real, ptop); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk9real, pnom); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk9real, ztop); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, pk9real, znom); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, pk9real, u); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, pk9real, v); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, CCMA, pk9real, t); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, CCMA, pk9real, rh); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, CCMA, pk9real, q); CHECK_PDS_ERROR(10);
    PDS = PackDS(P, CCMA, pk9real, cc); CHECK_PDS_ERROR(11);
    PDS = PackDS(P, CCMA, pk9real, clwc); CHECK_PDS_ERROR(12);
    PDS = PackDS(P, CCMA, pk9real, ciwc); CHECK_PDS_ERROR(13);
    PDS = PackDS(P, CCMA, pk9real, error_t); CHECK_PDS_ERROR(14);
    PDS = PackDS(P, CCMA, pk9real, error_rh); CHECK_PDS_ERROR(15);
    PDS = PackDS(P, CCMA, pk9real, error_p); CHECK_PDS_ERROR(16);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_aeolus_auxmet(void *T)
{
  int Nbytes = 0;
  TABLE_aeolus_auxmet *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk9real, lev); Nbytes += BYTESIZE(P->lev.d);
    UseDS(P, CCMA, pk9real, ptop); Nbytes += BYTESIZE(P->ptop.d);
    UseDS(P, CCMA, pk9real, pnom); Nbytes += BYTESIZE(P->pnom.d);
    UseDS(P, CCMA, pk9real, ztop); Nbytes += BYTESIZE(P->ztop.d);
    UseDS(P, CCMA, pk9real, znom); Nbytes += BYTESIZE(P->znom.d);
    UseDS(P, CCMA, pk9real, u); Nbytes += BYTESIZE(P->u.d);
    UseDS(P, CCMA, pk9real, v); Nbytes += BYTESIZE(P->v.d);
    UseDS(P, CCMA, pk9real, t); Nbytes += BYTESIZE(P->t.d);
    UseDS(P, CCMA, pk9real, rh); Nbytes += BYTESIZE(P->rh.d);
    UseDS(P, CCMA, pk9real, q); Nbytes += BYTESIZE(P->q.d);
    UseDS(P, CCMA, pk9real, cc); Nbytes += BYTESIZE(P->cc.d);
    UseDS(P, CCMA, pk9real, clwc); Nbytes += BYTESIZE(P->clwc.d);
    UseDS(P, CCMA, pk9real, ciwc); Nbytes += BYTESIZE(P->ciwc.d);
    UseDS(P, CCMA, pk9real, error_t); Nbytes += BYTESIZE(P->error_t.d);
    UseDS(P, CCMA, pk9real, error_rh); Nbytes += BYTESIZE(P->error_rh.d);
    UseDS(P, CCMA, pk9real, error_p); Nbytes += BYTESIZE(P->error_p.d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_aeolus_auxmet(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_aeolus_auxmet *P = T;
...
PreGetTable(CCMA, d, double, aeolus_auxmet)
  Call_CopyGet_TABLE(CCMA, d, 1, aeolus_auxmet, pk9real, D, lev, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, aeolus_auxmet, pk9real, D, ptop, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, aeolus_auxmet, pk9real, D, pnom, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 4, aeolus_auxmet, pk9real, D, ztop, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 5, aeolus_auxmet, pk9real, D, znom, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 6, aeolus_auxmet, pk9real, D, u, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 7, aeolus_auxmet, pk9real, D, v, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 8, aeolus_auxmet, pk9real, D, t, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 9, aeolus_auxmet, pk9real, D, rh, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 10, aeolus_auxmet, pk9real, D, q, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 11, aeolus_auxmet, pk9real, D, cc, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 12, aeolus_auxmet, pk9real, D, clwc, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 13, aeolus_auxmet, pk9real, D, ciwc, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 14, aeolus_auxmet, pk9real, D, error_t, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 15, aeolus_auxmet, pk9real, D, error_rh, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 16, aeolus_auxmet, pk9real, D, error_p, Count, DATATYPE_REAL8);
PostGetTable(d, double, aeolus_auxmet)


PrePutTable(CCMA, d, double, aeolus_auxmet)
  Call_CopyPut_TABLE(CCMA, d, 1, aeolus_auxmet, pk9real, lev, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, aeolus_auxmet, pk9real, ptop, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, aeolus_auxmet, pk9real, pnom, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 4, aeolus_auxmet, pk9real, ztop, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 5, aeolus_auxmet, pk9real, znom, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 6, aeolus_auxmet, pk9real, u, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 7, aeolus_auxmet, pk9real, v, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 8, aeolus_auxmet, pk9real, t, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 9, aeolus_auxmet, pk9real, rh, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 10, aeolus_auxmet, pk9real, q, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 11, aeolus_auxmet, pk9real, cc, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 12, aeolus_auxmet, pk9real, clwc, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 13, aeolus_auxmet, pk9real, ciwc, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 14, aeolus_auxmet, pk9real, error_t, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 15, aeolus_auxmet, pk9real, error_rh, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 16, aeolus_auxmet, pk9real, error_p, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, aeolus_auxmet)

PreLoadTable(CCMA, aeolus_auxmet);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, lev);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ptop);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, pnom);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ztop);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, znom);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, u);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, v);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, t);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rh);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, q);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, cc);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, clwc);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ciwc);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, error_t);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, error_rh);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, error_p);
PostLoadTable(aeolus_auxmet)

PreStoreTable(CCMA, aeolus_auxmet);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, lev);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ptop);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, pnom);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ztop);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, znom);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, u);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, v);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, t);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rh);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, q);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, cc);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, clwc);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ciwc);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, error_t);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, error_rh);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, error_p);
PostStoreTable(aeolus_auxmet)

DefineLookupTable(aeolus_auxmet)

PUBLIC void
CCMA_Dim_T_aeolus_auxmet(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_aeolus_auxmet(void *T)
{
  TABLE_aeolus_auxmet *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, aeolus_auxmet)

PUBLIC int
CCMA_Sql_T_aeolus_auxmet(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_aeolus_auxmet(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_aeolus_auxmet *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_aeolus_auxmet);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 16);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 27;
    pfcom->wt = 3.003004;
    pfcom->tags = CCMA_Set_T_aeolus_auxmet_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_aeolus_auxmet_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_aeolus_auxmet;
    pfcom->swapout = CCMA_Swapout_T_aeolus_auxmet;
    pfcom->dim = CCMA_Dim_T_aeolus_auxmet;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_aeolus_auxmet;
    pfcom->remove = CCMA_Remove_T_aeolus_auxmet;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_aeolus_auxmet; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_aeolus_auxmet; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_aeolus_auxmet;
    pfcom->store = CCMA_Store_T_aeolus_auxmet;
    pfcom->pack = CCMA_Pack_T_aeolus_auxmet;
    pfcom->unpack = CCMA_Unpack_T_aeolus_auxmet;
    pfcom->sql = CCMA_Sql_T_aeolus_auxmet;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
