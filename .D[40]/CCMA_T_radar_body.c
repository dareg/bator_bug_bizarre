...
PUBLIC int
CCMA_Pack_T_radar_body(void *T)
{
  int Nbytes = 0;
  TABLE_radar_body *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk1int, flgdyn); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, hhmmss, time); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk9real, distance); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk9real, elevation); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, pk9real, polarisation); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, pk1int, anaprop); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, pk9real, reflcost); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, CCMA, pk9real, azimuth); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, CCMA, pk9real, press); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, CCMA, pk9real, temp_1); CHECK_PDS_ERROR(10);
    PDS = PackDS(P, CCMA, pk9real, temp_2); CHECK_PDS_ERROR(11);
    PDS = PackDS(P, CCMA, pk9real, q_1); CHECK_PDS_ERROR(12);
    PDS = PackDS(P, CCMA, pk9real, q_2); CHECK_PDS_ERROR(13);
    PDS = PackDS(P, CCMA, pk9real, temp_1dv); CHECK_PDS_ERROR(14);
    PDS = PackDS(P, CCMA, pk9real, q_1dv); CHECK_PDS_ERROR(15);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_radar_body(void *T)
{
  int Nbytes = 0;
  TABLE_radar_body *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk1int, flgdyn); Nbytes += BYTESIZE(P->flgdyn.d);
    UseDS(P, CCMA, hhmmss, time); Nbytes += BYTESIZE(P->time.d);
    UseDS(P, CCMA, pk9real, distance); Nbytes += BYTESIZE(P->distance.d);
    UseDS(P, CCMA, pk9real, elevation); Nbytes += BYTESIZE(P->elevation.d);
    UseDS(P, CCMA, pk9real, polarisation); Nbytes += BYTESIZE(P->polarisation.d);
    UseDS(P, CCMA, pk1int, anaprop); Nbytes += BYTESIZE(P->anaprop.d);
    UseDS(P, CCMA, pk9real, reflcost); Nbytes += BYTESIZE(P->reflcost.d);
    ...
    UseDS(P, CCMA, pk9real, temp_1dv); Nbytes += BYTESIZE(P->temp_1dv.d);
    UseDS(P, CCMA, pk9real, q_1dv); Nbytes += BYTESIZE(P->q_1dv.d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_radar_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_radar_body *P = T;
...
PreGetTable(CCMA, d, double, radar_body)
  Call_CopyGet_TABLE(CCMA, d, 1, radar_body, pk1int, D, flgdyn, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 2, radar_body, hhmmss, D, time, Count, DATATYPE_HHMMSS);
  Call_CopyGet_TABLE(CCMA, d, 3, radar_body, pk9real, D, distance, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 4, radar_body, pk9real, D, elevation, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 5, radar_body, pk9real, D, polarisation, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 6, radar_body, pk1int, D, anaprop, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 7, radar_body, pk9real, D, reflcost, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 8, radar_body, pk9real, D, azimuth, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 9, radar_body, pk9real, D, press, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 10, radar_body, pk9real, D, temp_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 11, radar_body, pk9real, D, temp_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 12, radar_body, pk9real, D, q_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 13, radar_body, pk9real, D, q_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 14, radar_body, pk9real, D, temp_1dv, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 15, radar_body, pk9real, D, q_1dv, Count, DATATYPE_REAL8);
PostGetTable(d, double, radar_body)


PrePutTable(CCMA, d, double, radar_body)
  Call_CopyPut_TABLE(CCMA, d, 1, radar_body, pk1int, flgdyn, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 2, radar_body, hhmmss, time, D, Count, DATATYPE_HHMMSS);
  Call_CopyPut_TABLE(CCMA, d, 3, radar_body, pk9real, distance, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 4, radar_body, pk9real, elevation, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 5, radar_body, pk9real, polarisation, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 6, radar_body, pk1int, anaprop, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 7, radar_body, pk9real, reflcost, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 8, radar_body, pk9real, azimuth, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 9, radar_body, pk9real, press, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 10, radar_body, pk9real, temp_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 11, radar_body, pk9real, temp_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 12, radar_body, pk9real, q_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 13, radar_body, pk9real, q_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 14, radar_body, pk9real, temp_1dv, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 15, radar_body, pk9real, q_1dv, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, radar_body)

PreLoadTable(CCMA, radar_body);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, flgdyn);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, hhmmss, DATATYPE_HHMMSS, time);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, distance);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, elevation);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, polarisation);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, anaprop);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, reflcost);
  ...
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, temp_1dv);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, q_1dv);
PostLoadTable(radar_body)

PreStoreTable(CCMA, radar_body);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, flgdyn);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, hhmmss, DATATYPE_HHMMSS, time);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, distance);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, elevation);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, polarisation);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, anaprop);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, reflcost);
  ...
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, temp_1dv);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, q_1dv);
PostStoreTable(radar_body)

DefineLookupTable(radar_body)

PUBLIC void
CCMA_Dim_T_radar_body(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_radar_body(void *T)
{
  TABLE_radar_body *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, radar_body)

PUBLIC int
CCMA_Sql_T_radar_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_radar_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_radar_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_radar_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 15);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 381;
    pfcom->wt = 358.000376;
    pfcom->tags = CCMA_Set_T_radar_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_radar_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_radar_body;
    pfcom->swapout = CCMA_Swapout_T_radar_body;
    pfcom->dim = CCMA_Dim_T_radar_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_radar_body;
    pfcom->remove = CCMA_Remove_T_radar_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_radar_body; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_radar_body; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_radar_body;
    pfcom->store = CCMA_Store_T_radar_body;
    pfcom->pack = CCMA_Pack_T_radar_body;
    pfcom->unpack = CCMA_Unpack_T_radar_body;
    pfcom->sql = CCMA_Sql_T_radar_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
