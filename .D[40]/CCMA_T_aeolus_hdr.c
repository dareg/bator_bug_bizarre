...
PUBLIC int
CCMA_Pack_T_aeolus_hdr(void *T)
{
  int Nbytes = 0;
  TABLE_aeolus_hdr *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, Bitfield, aeolus_hdrflag); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, linkoffset_t, LINKOFFSET(aeolus_auxmet)); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, linklen_t, LINKLEN(aeolus_auxmet)); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, linkoffset_t, LINKOFFSET(aeolus_l2c)); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, linklen_t, LINKLEN(aeolus_l2c)); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, linkoffset_t, LINKOFFSET(aeolus_l2b)); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, linklen_t, LINKLEN(aeolus_l2b)); CHECK_PDS_ERROR(7);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_aeolus_hdr(void *T)
{
  int Nbytes = 0;
  TABLE_aeolus_hdr *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, Bitfield, aeolus_hdrflag); Nbytes += BYTESIZE(P->aeolus_hdrflag.d);
    UseDS(P, CCMA, linkoffset_t, LINKOFFSET(aeolus_auxmet)); Nbytes += BYTESIZE(P->LINKOFFSET(aeolus_auxmet).d);
    UseDS(P, CCMA, linklen_t, LINKLEN(aeolus_auxmet)); Nbytes += BYTESIZE(P->LINKLEN(aeolus_auxmet).d);
    UseDS(P, CCMA, linkoffset_t, LINKOFFSET(aeolus_l2c)); Nbytes += BYTESIZE(P->LINKOFFSET(aeolus_l2c).d);
    UseDS(P, CCMA, linklen_t, LINKLEN(aeolus_l2c)); Nbytes += BYTESIZE(P->LINKLEN(aeolus_l2c).d);
    UseDS(P, CCMA, linkoffset_t, LINKOFFSET(aeolus_l2b)); Nbytes += BYTESIZE(P->LINKOFFSET(aeolus_l2b).d);
    UseDS(P, CCMA, linklen_t, LINKLEN(aeolus_l2b)); Nbytes += BYTESIZE(P->LINKLEN(aeolus_l2b).d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_aeolus_hdr(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_aeolus_hdr *P = T;
...
PreGetTable(CCMA, d, double, aeolus_hdr)
  Call_CopyGet_TABLE(CCMA, d, 1, aeolus_hdr, Bitfield, D, aeolus_hdrflag, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(CCMA, d, 2, aeolus_hdr, linkoffset_t, D, LINKOFFSET(aeolus_auxmet), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(CCMA, d, 3, aeolus_hdr, linklen_t, D, LINKLEN(aeolus_auxmet), Count, DATATYPE_LINKLEN);
  Call_CopyGet_TABLE(CCMA, d, 4, aeolus_hdr, linkoffset_t, D, LINKOFFSET(aeolus_l2c), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(CCMA, d, 5, aeolus_hdr, linklen_t, D, LINKLEN(aeolus_l2c), Count, DATATYPE_LINKLEN);
  Call_CopyGet_TABLE(CCMA, d, 6, aeolus_hdr, linkoffset_t, D, LINKOFFSET(aeolus_l2b), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(CCMA, d, 7, aeolus_hdr, linklen_t, D, LINKLEN(aeolus_l2b), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, aeolus_hdr)


PrePutTable(CCMA, d, double, aeolus_hdr)
  Call_CopyPut_TABLE(CCMA, d, 1, aeolus_hdr, Bitfield, aeolus_hdrflag, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(CCMA, d, 2, aeolus_hdr, linkoffset_t, LINKOFFSET(aeolus_auxmet), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(CCMA, d, 3, aeolus_hdr, linklen_t, LINKLEN(aeolus_auxmet), D, Count, DATATYPE_LINKLEN);
  Call_CopyPut_TABLE(CCMA, d, 4, aeolus_hdr, linkoffset_t, LINKOFFSET(aeolus_l2c), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(CCMA, d, 5, aeolus_hdr, linklen_t, LINKLEN(aeolus_l2c), D, Count, DATATYPE_LINKLEN);
  Call_CopyPut_TABLE(CCMA, d, 6, aeolus_hdr, linkoffset_t, LINKOFFSET(aeolus_l2b), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(CCMA, d, 7, aeolus_hdr, linklen_t, LINKLEN(aeolus_l2b), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, aeolus_hdr)

PreLoadTable(CCMA, aeolus_hdr);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, aeolus_hdrflag);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_auxmet));
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_auxmet));
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_l2c));
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_l2c));
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_l2b));
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_l2b));
PostLoadTable(aeolus_hdr)

PreStoreTable(CCMA, aeolus_hdr);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, aeolus_hdrflag);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_auxmet));
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_auxmet));
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_l2c));
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_l2c));
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_l2b));
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_l2b));
PostStoreTable(aeolus_hdr)

DefineLookupTable(aeolus_hdr)

PUBLIC void
CCMA_Dim_T_aeolus_hdr(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_aeolus_hdr(void *T)
{
  TABLE_aeolus_hdr *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, aeolus_hdr)

PUBLIC int
CCMA_Sql_T_aeolus_hdr(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_aeolus_hdr(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_aeolus_hdr *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_aeolus_hdr);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 7);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 23;
    pfcom->wt = 3.002004;
    pfcom->tags = CCMA_Set_T_aeolus_hdr_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_aeolus_hdr_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_aeolus_hdr;
    pfcom->swapout = CCMA_Swapout_T_aeolus_hdr;
    pfcom->dim = CCMA_Dim_T_aeolus_hdr;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_aeolus_hdr;
    pfcom->remove = CCMA_Remove_T_aeolus_hdr;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_aeolus_hdr; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_aeolus_hdr; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_aeolus_hdr;
    pfcom->store = CCMA_Store_T_aeolus_hdr;
    pfcom->pack = CCMA_Pack_T_aeolus_hdr;
    pfcom->unpack = CCMA_Unpack_T_aeolus_hdr;
    pfcom->sql = CCMA_Sql_T_aeolus_hdr;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
