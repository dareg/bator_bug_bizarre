...
PUBLIC int
CCMA_Pack_T_raingg(void *T)
{
  int Nbytes = 0;
  TABLE_raingg *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk1int, report_rrflag); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, linkoffset_t, LINKOFFSET(raingg_body)); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, linklen_t, LINKLEN(raingg_body)); CHECK_PDS_ERROR(3);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_raingg(void *T)
{
  int Nbytes = 0;
  TABLE_raingg *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk1int, report_rrflag); Nbytes += BYTESIZE(P->report_rrflag.d);
    UseDS(P, CCMA, linkoffset_t, LINKOFFSET(raingg_body)); Nbytes += BYTESIZE(P->LINKOFFSET(raingg_body).d);
    UseDS(P, CCMA, linklen_t, LINKLEN(raingg_body)); Nbytes += BYTESIZE(P->LINKLEN(raingg_body).d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_raingg(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_raingg *P = T;
...
PreGetTable(CCMA, d, double, raingg)
  Call_CopyGet_TABLE(CCMA, d, 1, raingg, pk1int, D, report_rrflag, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 2, raingg, linkoffset_t, D, LINKOFFSET(raingg_body), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(CCMA, d, 3, raingg, linklen_t, D, LINKLEN(raingg_body), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, raingg)


PrePutTable(CCMA, d, double, raingg)
  Call_CopyPut_TABLE(CCMA, d, 1, raingg, pk1int, report_rrflag, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 2, raingg, linkoffset_t, LINKOFFSET(raingg_body), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(CCMA, d, 3, raingg, linklen_t, LINKLEN(raingg_body), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, raingg)

PreLoadTable(CCMA, raingg);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, report_rrflag);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(raingg_body));
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(raingg_body));
PostLoadTable(raingg)

PreStoreTable(CCMA, raingg);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, report_rrflag);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(raingg_body));
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(raingg_body));
PostStoreTable(raingg)

DefineLookupTable(raingg)

PUBLIC void
CCMA_Dim_T_raingg(void *T, int *Nrows, int *Ncols,
...
DefineRemoveTable(CCMA, raingg)

PUBLIC int
CCMA_Sql_T_raingg(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_raingg(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_raingg *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_raingg);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 3);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 8;
    pfcom->wt = 3.001004;
    pfcom->tags = CCMA_Set_T_raingg_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_raingg_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_raingg;
    pfcom->swapout = CCMA_Swapout_T_raingg;
    pfcom->dim = CCMA_Dim_T_raingg;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_raingg;
    pfcom->remove = CCMA_Remove_T_raingg;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_raingg; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_raingg; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_raingg;
    pfcom->store = CCMA_Store_T_raingg;
    pfcom->pack = CCMA_Pack_T_raingg;
    pfcom->unpack = CCMA_Unpack_T_raingg;
    pfcom->sql = CCMA_Sql_T_raingg;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
