...
PUBLIC int
CCMA_Pack_T_cloud_sink(void *T)
{
  int Nbytes = 0;
  TABLE_cloud_sink *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk9real, ctop_1); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk9real, ctop_2); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk9real, ctop_3); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk9real, ctop_4); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, pk9real, camt_1); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, pk9real, camt_2); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, pk9real, camt_3); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, CCMA, pk9real, camt_4); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, CCMA, pk9real, ctopbg); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, CCMA, pk9real, ctoper); CHECK_PDS_ERROR(10);
    PDS = PackDS(P, CCMA, pk9real, ctopinc); CHECK_PDS_ERROR(11);
    PDS = PackDS(P, CCMA, pk9real, camtbg); CHECK_PDS_ERROR(12);
    PDS = PackDS(P, CCMA, pk9real, camter); CHECK_PDS_ERROR(13);
    PDS = PackDS(P, CCMA, pk9real, camtinc); CHECK_PDS_ERROR(14);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_cloud_sink(void *T)
{
  int Nbytes = 0;
  TABLE_cloud_sink *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk9real, ctop_1); Nbytes += BYTESIZE(P->ctop_1.d);
    UseDS(P, CCMA, pk9real, ctop_2); Nbytes += BYTESIZE(P->ctop_2.d);
    UseDS(P, CCMA, pk9real, ctop_3); Nbytes += BYTESIZE(P->ctop_3.d);
    UseDS(P, CCMA, pk9real, ctop_4); Nbytes += BYTESIZE(P->ctop_4.d);
    UseDS(P, CCMA, pk9real, camt_1); Nbytes += BYTESIZE(P->camt_1.d);
    UseDS(P, CCMA, pk9real, camt_2); Nbytes += BYTESIZE(P->camt_2.d);
    UseDS(P, CCMA, pk9real, camt_3); Nbytes += BYTESIZE(P->camt_3.d);
    UseDS(P, CCMA, pk9real, camt_4); Nbytes += BYTESIZE(P->camt_4.d);
    UseDS(P, CCMA, pk9real, ctopbg); Nbytes += BYTESIZE(P->ctopbg.d);
    UseDS(P, CCMA, pk9real, ctoper); Nbytes += BYTESIZE(P->ctoper.d);
    UseDS(P, CCMA, pk9real, ctopinc); Nbytes += BYTESIZE(P->ctopinc.d);
    UseDS(P, CCMA, pk9real, camtbg); Nbytes += BYTESIZE(P->camtbg.d);
    UseDS(P, CCMA, pk9real, camter); Nbytes += BYTESIZE(P->camter.d);
    UseDS(P, CCMA, pk9real, camtinc); Nbytes += BYTESIZE(P->camtinc.d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_cloud_sink(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_cloud_sink *P = T;
...
PreGetTable(CCMA, d, double, cloud_sink)
  Call_CopyGet_TABLE(CCMA, d, 1, cloud_sink, pk9real, D, ctop_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, cloud_sink, pk9real, D, ctop_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, cloud_sink, pk9real, D, ctop_3, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 4, cloud_sink, pk9real, D, ctop_4, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 5, cloud_sink, pk9real, D, camt_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 6, cloud_sink, pk9real, D, camt_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 7, cloud_sink, pk9real, D, camt_3, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 8, cloud_sink, pk9real, D, camt_4, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 9, cloud_sink, pk9real, D, ctopbg, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 10, cloud_sink, pk9real, D, ctoper, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 11, cloud_sink, pk9real, D, ctopinc, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 12, cloud_sink, pk9real, D, camtbg, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 13, cloud_sink, pk9real, D, camter, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 14, cloud_sink, pk9real, D, camtinc, Count, DATATYPE_REAL8);
PostGetTable(d, double, cloud_sink)


PrePutTable(CCMA, d, double, cloud_sink)
  Call_CopyPut_TABLE(CCMA, d, 1, cloud_sink, pk9real, ctop_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, cloud_sink, pk9real, ctop_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, cloud_sink, pk9real, ctop_3, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 4, cloud_sink, pk9real, ctop_4, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 5, cloud_sink, pk9real, camt_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 6, cloud_sink, pk9real, camt_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 7, cloud_sink, pk9real, camt_3, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 8, cloud_sink, pk9real, camt_4, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 9, cloud_sink, pk9real, ctopbg, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 10, cloud_sink, pk9real, ctoper, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 11, cloud_sink, pk9real, ctopinc, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 12, cloud_sink, pk9real, camtbg, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 13, cloud_sink, pk9real, camter, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 14, cloud_sink, pk9real, camtinc, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, cloud_sink)

PreLoadTable(CCMA, cloud_sink);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ctop_1);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ctop_2);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ctop_3);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ctop_4);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, camt_1);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, camt_2);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, camt_3);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, camt_4);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ctopbg);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ctoper);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ctopinc);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, camtbg);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, camter);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, camtinc);
PostLoadTable(cloud_sink)

PreStoreTable(CCMA, cloud_sink);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ctop_1);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ctop_2);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ctop_3);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ctop_4);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, camt_1);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, camt_2);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, camt_3);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, camt_4);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ctopbg);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ctoper);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ctopinc);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, camtbg);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, camter);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, camtinc);
PostStoreTable(cloud_sink)

DefineLookupTable(cloud_sink)

PUBLIC void
CCMA_Dim_T_cloud_sink(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_cloud_sink(void *T)
{
  TABLE_cloud_sink *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, cloud_sink)

PUBLIC int
CCMA_Sql_T_cloud_sink(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_cloud_sink(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_cloud_sink *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_cloud_sink);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 14);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 25;
    pfcom->wt = 3.003004;
    pfcom->tags = CCMA_Set_T_cloud_sink_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_cloud_sink_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_cloud_sink;
    pfcom->swapout = CCMA_Swapout_T_cloud_sink;
    pfcom->dim = CCMA_Dim_T_cloud_sink;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_cloud_sink;
    pfcom->remove = CCMA_Remove_T_cloud_sink;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_cloud_sink; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_cloud_sink; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_cloud_sink;
    pfcom->store = CCMA_Store_T_cloud_sink;
    pfcom->pack = CCMA_Pack_T_cloud_sink;
    pfcom->unpack = CCMA_Unpack_T_cloud_sink;
    pfcom->sql = CCMA_Sql_T_cloud_sink;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
