...
CCMA_Pack_T_gbrad(void *T)
{
  ...
  if (P->Is_loaded) {
    ...
    PDS = PackDS(P, CCMA, linkoffset_t, LINKOFFSET(gbrad_body)); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, linklen_t, LINKLEN(gbrad_body)); CHECK_PDS_ERROR(3);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_gbrad(void *T)
{
  int Nbytes = 0;
  TABLE_gbrad *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk1int, report_rrflag); Nbytes += BYTESIZE(P->report_rrflag.d);
    UseDS(P, CCMA, linkoffset_t, LINKOFFSET(gbrad_body)); Nbytes += BYTESIZE(P->LINKOFFSET(gbrad_body).d);
    UseDS(P, CCMA, linklen_t, LINKLEN(gbrad_body)); Nbytes += BYTESIZE(P->LINKLEN(gbrad_body).d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_gbrad(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_gbrad *P = T;
...
PreGetTable(CCMA, d, double, gbrad)
  Call_CopyGet_TABLE(CCMA, d, 1, gbrad, pk1int, D, report_rrflag, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 2, gbrad, linkoffset_t, D, LINKOFFSET(gbrad_body), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(CCMA, d, 3, gbrad, linklen_t, D, LINKLEN(gbrad_body), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, gbrad)


PrePutTable(CCMA, d, double, gbrad)
  Call_CopyPut_TABLE(CCMA, d, 1, gbrad, pk1int, report_rrflag, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 2, gbrad, linkoffset_t, LINKOFFSET(gbrad_body), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(CCMA, d, 3, gbrad, linklen_t, LINKLEN(gbrad_body), D, Count, DATATYPE_LINKLEN);
...
PreLoadTable(CCMA, gbrad);
  ...
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(gbrad_body));
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(gbrad_body));
PostLoadTable(gbrad)

PreStoreTable(CCMA, gbrad);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, report_rrflag);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(gbrad_body));
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(gbrad_body));
PostStoreTable(gbrad)

DefineLookupTable(gbrad)

PUBLIC void
CCMA_Dim_T_gbrad(void *T, int *Nrows, int *Ncols,
...
DefineRemoveTable(CCMA, gbrad)

PUBLIC int
CCMA_Sql_T_gbrad(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_gbrad(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_gbrad *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_gbrad);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 3);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 7;
    pfcom->wt = 3.001004;
    pfcom->tags = CCMA_Set_T_gbrad_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_gbrad_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_gbrad;
    pfcom->swapout = CCMA_Swapout_T_gbrad;
    pfcom->dim = CCMA_Dim_T_gbrad;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_gbrad;
    pfcom->remove = CCMA_Remove_T_gbrad;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_gbrad; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_gbrad; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_gbrad;
    pfcom->store = CCMA_Store_T_gbrad;
    pfcom->pack = CCMA_Pack_T_gbrad;
    pfcom->unpack = CCMA_Unpack_T_gbrad;
    pfcom->sql = CCMA_Sql_T_gbrad;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
