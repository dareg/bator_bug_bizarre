...
PUBLIC int
CCMA_Pack_T_scatt(void *T)
{
  int Nbytes = 0;
  TABLE_scatt *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk1int, cellno); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk1int, nretr_amb); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk1int, prodflag); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk1int, wvc_qf); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, linkoffset_t, LINKOFFSET(scatt_body)); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, linklen_t, LINKLEN(scatt_body)); CHECK_PDS_ERROR(6);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_scatt(void *T)
{
  int Nbytes = 0;
  TABLE_scatt *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk1int, cellno); Nbytes += BYTESIZE(P->cellno.d);
    UseDS(P, CCMA, pk1int, nretr_amb); Nbytes += BYTESIZE(P->nretr_amb.d);
    UseDS(P, CCMA, pk1int, prodflag); Nbytes += BYTESIZE(P->prodflag.d);
    UseDS(P, CCMA, pk1int, wvc_qf); Nbytes += BYTESIZE(P->wvc_qf.d);
    UseDS(P, CCMA, linkoffset_t, LINKOFFSET(scatt_body)); Nbytes += BYTESIZE(P->LINKOFFSET(scatt_body).d);
    UseDS(P, CCMA, linklen_t, LINKLEN(scatt_body)); Nbytes += BYTESIZE(P->LINKLEN(scatt_body).d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_scatt(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_scatt *P = T;
...
PreGetTable(CCMA, d, double, scatt)
  Call_CopyGet_TABLE(CCMA, d, 1, scatt, pk1int, D, cellno, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 2, scatt, pk1int, D, nretr_amb, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 3, scatt, pk1int, D, prodflag, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 4, scatt, pk1int, D, wvc_qf, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 5, scatt, linkoffset_t, D, LINKOFFSET(scatt_body), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(CCMA, d, 6, scatt, linklen_t, D, LINKLEN(scatt_body), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, scatt)


PrePutTable(CCMA, d, double, scatt)
  Call_CopyPut_TABLE(CCMA, d, 1, scatt, pk1int, cellno, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 2, scatt, pk1int, nretr_amb, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 3, scatt, pk1int, prodflag, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 4, scatt, pk1int, wvc_qf, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 5, scatt, linkoffset_t, LINKOFFSET(scatt_body), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(CCMA, d, 6, scatt, linklen_t, LINKLEN(scatt_body), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, scatt)

PreLoadTable(CCMA, scatt);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, cellno);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, nretr_amb);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, prodflag);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, wvc_qf);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(scatt_body));
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(scatt_body));
PostLoadTable(scatt)

PreStoreTable(CCMA, scatt);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, cellno);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, nretr_amb);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, prodflag);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, wvc_qf);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(scatt_body));
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(scatt_body));
PostStoreTable(scatt)

DefineLookupTable(scatt)

PUBLIC void
CCMA_Dim_T_scatt(void *T, int *Nrows, int *Ncols,
...
DefineRemoveTable(CCMA, scatt)

PUBLIC int
CCMA_Sql_T_scatt(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_scatt(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_scatt *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_scatt);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 6);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 17;
    pfcom->wt = 3.002004;
    pfcom->tags = CCMA_Set_T_scatt_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_scatt_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_scatt;
    pfcom->swapout = CCMA_Swapout_T_scatt;
    pfcom->dim = CCMA_Dim_T_scatt;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_scatt;
    pfcom->remove = CCMA_Remove_T_scatt;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_scatt; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_scatt; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_scatt;
    pfcom->store = CCMA_Store_T_scatt;
    pfcom->pack = CCMA_Pack_T_scatt;
    pfcom->unpack = CCMA_Unpack_T_scatt;
    pfcom->sql = CCMA_Sql_T_scatt;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
