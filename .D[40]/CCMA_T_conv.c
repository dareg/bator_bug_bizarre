...
PUBLIC int
CCMA_Pack_T_conv(void *T)
{
  int Nbytes = 0;
  TABLE_conv *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk1int, flight_phase); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk9real, flight_dp_o_dt); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk9real, anemoht); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk9real, baroht); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, pk1int, station_type); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, pk1int, sonde_type); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, pk1int, collection_identifier); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, CCMA, pk1int, country); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, CCMA, pk1int, unique_identifier); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, CCMA, pk1int, timeseries_index); CHECK_PDS_ERROR(10);
    PDS = PackDS(P, CCMA, pk9real, heading); CHECK_PDS_ERROR(11);
    PDS = PackDS(P, CCMA, string, aircraft_type); CHECK_PDS_ERROR(12);
    PDS = PackDS(P, CCMA, linkoffset_t, LINKOFFSET(conv_body)); CHECK_PDS_ERROR(13);
    PDS = PackDS(P, CCMA, linklen_t, LINKLEN(conv_body)); CHECK_PDS_ERROR(14);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_conv(void *T)
{
  int Nbytes = 0;
  TABLE_conv *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk1int, flight_phase); Nbytes += BYTESIZE(P->flight_phase.d);
    UseDS(P, CCMA, pk9real, flight_dp_o_dt); Nbytes += BYTESIZE(P->flight_dp_o_dt.d);
    UseDS(P, CCMA, pk9real, anemoht); Nbytes += BYTESIZE(P->anemoht.d);
    UseDS(P, CCMA, pk9real, baroht); Nbytes += BYTESIZE(P->baroht.d);
    UseDS(P, CCMA, pk1int, station_type); Nbytes += BYTESIZE(P->station_type.d);
    UseDS(P, CCMA, pk1int, sonde_type); Nbytes += BYTESIZE(P->sonde_type.d);
    UseDS(P, CCMA, pk1int, collection_identifier); Nbytes += BYTESIZE(P->collection_identifier.d);
    UseDS(P, CCMA, pk1int, country); Nbytes += BYTESIZE(P->country.d);
    UseDS(P, CCMA, pk1int, unique_identifier); Nbytes += BYTESIZE(P->unique_identifier.d);
    UseDS(P, CCMA, pk1int, timeseries_index); Nbytes += BYTESIZE(P->timeseries_index.d);
    UseDS(P, CCMA, pk9real, heading); Nbytes += BYTESIZE(P->heading.d);
    UseDS(P, CCMA, string, aircraft_type); Nbytes += BYTESIZE(P->aircraft_type.d);
    UseDS(P, CCMA, linkoffset_t, LINKOFFSET(conv_body)); Nbytes += BYTESIZE(P->LINKOFFSET(conv_body).d);
    UseDS(P, CCMA, linklen_t, LINKLEN(conv_body)); Nbytes += BYTESIZE(P->LINKLEN(conv_body).d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_conv(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_conv *P = T;
...
PreGetTable(CCMA, d, double, conv)
  Call_CopyGet_TABLE(CCMA, d, 1, conv, pk1int, D, flight_phase, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 2, conv, pk9real, D, flight_dp_o_dt, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, conv, pk9real, D, anemoht, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 4, conv, pk9real, D, baroht, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 5, conv, pk1int, D, station_type, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 6, conv, pk1int, D, sonde_type, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 7, conv, pk1int, D, collection_identifier, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 8, conv, pk1int, D, country, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 9, conv, pk1int, D, unique_identifier, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 10, conv, pk1int, D, timeseries_index, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 11, conv, pk9real, D, heading, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 12, conv, string, D, aircraft_type, Count, DATATYPE_STRING);
  Call_CopyGet_TABLE(CCMA, d, 13, conv, linkoffset_t, D, LINKOFFSET(conv_body), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(CCMA, d, 14, conv, linklen_t, D, LINKLEN(conv_body), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, conv)


PrePutTable(CCMA, d, double, conv)
  Call_CopyPut_TABLE(CCMA, d, 1, conv, pk1int, flight_phase, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 2, conv, pk9real, flight_dp_o_dt, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, conv, pk9real, anemoht, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 4, conv, pk9real, baroht, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 5, conv, pk1int, station_type, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 6, conv, pk1int, sonde_type, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 7, conv, pk1int, collection_identifier, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 8, conv, pk1int, country, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 9, conv, pk1int, unique_identifier, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 10, conv, pk1int, timeseries_index, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 11, conv, pk9real, heading, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 12, conv, string, aircraft_type, D, Count, DATATYPE_STRING);
  Call_CopyPut_TABLE(CCMA, d, 13, conv, linkoffset_t, LINKOFFSET(conv_body), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(CCMA, d, 14, conv, linklen_t, LINKLEN(conv_body), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, conv)

PreLoadTable(CCMA, conv);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, flight_phase);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, flight_dp_o_dt);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, anemoht);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, baroht);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, station_type);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, sonde_type);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, collection_identifier);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, country);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, unique_identifier);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, timeseries_index);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, heading);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, string, DATATYPE_STRING, aircraft_type);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(conv_body));
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(conv_body));
PostLoadTable(conv)

PreStoreTable(CCMA, conv);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, flight_phase);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, flight_dp_o_dt);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, anemoht);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, baroht);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, station_type);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, sonde_type);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, collection_identifier);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, country);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, unique_identifier);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, timeseries_index);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, heading);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, string, DATATYPE_STRING, aircraft_type);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(conv_body));
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(conv_body));
PostStoreTable(conv)

DefineLookupTable(conv)

PUBLIC void
CCMA_Dim_T_conv(void *T, int *Nrows, int *Ncols,
...
DefineRemoveTable(CCMA, conv)

PUBLIC int
CCMA_Sql_T_conv(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_conv(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_conv *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_conv);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 14);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 9;
    pfcom->wt = 3.001004;
    pfcom->tags = CCMA_Set_T_conv_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_conv_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_conv;
    pfcom->swapout = CCMA_Swapout_T_conv;
    pfcom->dim = CCMA_Dim_T_conv;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_conv;
    pfcom->remove = CCMA_Remove_T_conv;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_conv; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_conv; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_conv;
    pfcom->store = CCMA_Store_T_conv;
    pfcom->pack = CCMA_Pack_T_conv;
    pfcom->unpack = CCMA_Unpack_T_conv;
    pfcom->sql = CCMA_Sql_T_conv;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
