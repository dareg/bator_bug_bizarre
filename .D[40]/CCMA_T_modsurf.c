...
PUBLIC int
CCMA_Pack_T_modsurf(void *T)
{
  int Nbytes = 0;
  TABLE_modsurf *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk9real, lsm); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk9real, seaice); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk9real, orography); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk9real, snow_depth); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, pk9real, t2m); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, pk9real, albedo); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, pk9real, windspeed10m); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, CCMA, pk9real, u10m); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, CCMA, pk9real, v10m); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, CCMA, pk1int, surface_class); CHECK_PDS_ERROR(10);
    PDS = PackDS(P, CCMA, pk9real, tsfc); CHECK_PDS_ERROR(11);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_modsurf(void *T)
{
  int Nbytes = 0;
  TABLE_modsurf *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk9real, lsm); Nbytes += BYTESIZE(P->lsm.d);
    UseDS(P, CCMA, pk9real, seaice); Nbytes += BYTESIZE(P->seaice.d);
    UseDS(P, CCMA, pk9real, orography); Nbytes += BYTESIZE(P->orography.d);
    UseDS(P, CCMA, pk9real, snow_depth); Nbytes += BYTESIZE(P->snow_depth.d);
    UseDS(P, CCMA, pk9real, t2m); Nbytes += BYTESIZE(P->t2m.d);
    UseDS(P, CCMA, pk9real, albedo); Nbytes += BYTESIZE(P->albedo.d);
    UseDS(P, CCMA, pk9real, windspeed10m); Nbytes += BYTESIZE(P->windspeed10m.d);
    UseDS(P, CCMA, pk9real, u10m); Nbytes += BYTESIZE(P->u10m.d);
    UseDS(P, CCMA, pk9real, v10m); Nbytes += BYTESIZE(P->v10m.d);
    UseDS(P, CCMA, pk1int, surface_class); Nbytes += BYTESIZE(P->surface_class.d);
    UseDS(P, CCMA, pk9real, tsfc); Nbytes += BYTESIZE(P->tsfc.d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_modsurf(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_modsurf *P = T;
...
PreGetTable(CCMA, d, double, modsurf)
  Call_CopyGet_TABLE(CCMA, d, 1, modsurf, pk9real, D, lsm, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, modsurf, pk9real, D, seaice, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, modsurf, pk9real, D, orography, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 4, modsurf, pk9real, D, snow_depth, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 5, modsurf, pk9real, D, t2m, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 6, modsurf, pk9real, D, albedo, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 7, modsurf, pk9real, D, windspeed10m, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 8, modsurf, pk9real, D, u10m, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 9, modsurf, pk9real, D, v10m, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 10, modsurf, pk1int, D, surface_class, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 11, modsurf, pk9real, D, tsfc, Count, DATATYPE_REAL8);
PostGetTable(d, double, modsurf)


PrePutTable(CCMA, d, double, modsurf)
  Call_CopyPut_TABLE(CCMA, d, 1, modsurf, pk9real, lsm, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, modsurf, pk9real, seaice, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, modsurf, pk9real, orography, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 4, modsurf, pk9real, snow_depth, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 5, modsurf, pk9real, t2m, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 6, modsurf, pk9real, albedo, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 7, modsurf, pk9real, windspeed10m, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 8, modsurf, pk9real, u10m, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 9, modsurf, pk9real, v10m, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 10, modsurf, pk1int, surface_class, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 11, modsurf, pk9real, tsfc, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, modsurf)

PreLoadTable(CCMA, modsurf);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, lsm);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, seaice);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, orography);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, snow_depth);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, t2m);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, albedo);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, windspeed10m);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, u10m);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, v10m);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, surface_class);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, tsfc);
PostLoadTable(modsurf)

PreStoreTable(CCMA, modsurf);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, lsm);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, seaice);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, orography);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, snow_depth);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, t2m);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, albedo);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, windspeed10m);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, u10m);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, v10m);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, surface_class);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, tsfc);
PostStoreTable(modsurf)

DefineLookupTable(modsurf)

PUBLIC void
CCMA_Dim_T_modsurf(void *T, int *Nrows, int *Ncols,
...
DefineRemoveTable(CCMA, modsurf)

PUBLIC int
CCMA_Sql_T_modsurf(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_modsurf(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_modsurf *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_modsurf);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 11);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 11;
    pfcom->wt = 3.001004;
    pfcom->tags = CCMA_Set_T_modsurf_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_modsurf_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_modsurf;
    pfcom->swapout = CCMA_Swapout_T_modsurf;
    pfcom->dim = CCMA_Dim_T_modsurf;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_modsurf;
    pfcom->remove = CCMA_Remove_T_modsurf;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_modsurf; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_modsurf; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_modsurf;
    pfcom->store = CCMA_Store_T_modsurf;
    pfcom->pack = CCMA_Pack_T_modsurf;
    pfcom->unpack = CCMA_Unpack_T_modsurf;
    pfcom->sql = CCMA_Sql_T_modsurf;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
