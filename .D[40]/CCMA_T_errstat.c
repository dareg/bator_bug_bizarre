...
PUBLIC int
CCMA_Pack_T_errstat(void *T)
{
  int Nbytes = 0;
  TABLE_errstat *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk9real, final_obs_error); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk9real, obs_error); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk9real, repres_error); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk9real, pers_error); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, pk9real, fg_error); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, pk9real, eda_spread); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, pk9real, obs_ak_error); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, CCMA, pk9real, obs_corr_ev_1); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, CCMA, pk1int, obs_corr_mask); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, CCMA, pk9real, obs_corr_diag_1); CHECK_PDS_ERROR(10);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_errstat(void *T)
{
  int Nbytes = 0;
  TABLE_errstat *P = T;
  if (P->Is_loaded) {
    ...
    UseDS(P, CCMA, pk9real, repres_error); Nbytes += BYTESIZE(P->repres_error.d);
    UseDS(P, CCMA, pk9real, pers_error); Nbytes += BYTESIZE(P->pers_error.d);
    UseDS(P, CCMA, pk9real, fg_error); Nbytes += BYTESIZE(P->fg_error.d);
    UseDS(P, CCMA, pk9real, eda_spread); Nbytes += BYTESIZE(P->eda_spread.d);
    UseDS(P, CCMA, pk9real, obs_ak_error); Nbytes += BYTESIZE(P->obs_ak_error.d);
    UseDS(P, CCMA, pk9real, obs_corr_ev_1); Nbytes += BYTESIZE(P->obs_corr_ev_1.d);
    UseDS(P, CCMA, pk1int, obs_corr_mask); Nbytes += BYTESIZE(P->obs_corr_mask.d);
    UseDS(P, CCMA, pk9real, obs_corr_diag_1); Nbytes += BYTESIZE(P->obs_corr_diag_1.d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_errstat(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_errstat *P = T;
...
PreGetTable(CCMA, d, double, errstat)
  Call_CopyGet_TABLE(CCMA, d, 1, errstat, pk9real, D, final_obs_error, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, errstat, pk9real, D, obs_error, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, errstat, pk9real, D, repres_error, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 4, errstat, pk9real, D, pers_error, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 5, errstat, pk9real, D, fg_error, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 6, errstat, pk9real, D, eda_spread, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 7, errstat, pk9real, D, obs_ak_error, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 8, errstat, pk9real, D, obs_corr_ev_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 9, errstat, pk1int, D, obs_corr_mask, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 10, errstat, pk9real, D, obs_corr_diag_1, Count, DATATYPE_REAL8);
PostGetTable(d, double, errstat)


PrePutTable(CCMA, d, double, errstat)
  Call_CopyPut_TABLE(CCMA, d, 1, errstat, pk9real, final_obs_error, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, errstat, pk9real, obs_error, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, errstat, pk9real, repres_error, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 4, errstat, pk9real, pers_error, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 5, errstat, pk9real, fg_error, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 6, errstat, pk9real, eda_spread, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 7, errstat, pk9real, obs_ak_error, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 8, errstat, pk9real, obs_corr_ev_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 9, errstat, pk1int, obs_corr_mask, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 10, errstat, pk9real, obs_corr_diag_1, D, Count, DATATYPE_REAL8);
...
PreLoadTable(CCMA, errstat);
  ...
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, repres_error);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, pers_error);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, fg_error);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, eda_spread);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_ak_error);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_corr_ev_1);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, obs_corr_mask);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_corr_diag_1);
...
PreStoreTable(CCMA, errstat);
  ...
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, repres_error);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, pers_error);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, fg_error);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, eda_spread);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_ak_error);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_corr_ev_1);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, obs_corr_mask);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_corr_diag_1);
PostStoreTable(errstat)

DefineLookupTable(errstat)

PUBLIC void
CCMA_Dim_T_errstat(void *T, int *Nrows, int *Ncols,
...
CCMA_Swapout_T_errstat(void *T)
{
  ...
  FreeDS(P, pers_error, Nbytes, Count);
  FreeDS(P, fg_error, Nbytes, Count);
  FreeDS(P, eda_spread, Nbytes, Count);
  FreeDS(P, obs_ak_error, Nbytes, Count);
  FreeDS(P, obs_corr_ev_1, Nbytes, Count);
  FreeDS(P, obs_corr_mask, Nbytes, Count);
  FreeDS(P, obs_corr_diag_1, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(errstat,10);
}

DefineRemoveTable(CCMA, errstat)

PUBLIC int
CCMA_Sql_T_errstat(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_errstat(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_errstat *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_errstat);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 10);
  InitDS(pk9real, DATATYPE_REAL8, final_obs_error, errstat, 9);
  InitDS(pk9real, DATATYPE_REAL8, obs_error, errstat, 9);
  InitDS(pk9real, DATATYPE_REAL8, repres_error, errstat, 9);
  InitDS(pk9real, DATATYPE_REAL8, pers_error, errstat, 9);
  InitDS(pk9real, DATATYPE_REAL8, fg_error, errstat, 9);
  InitDS(pk9real, DATATYPE_REAL8, eda_spread, errstat, 9);
  InitDS(pk9real, DATATYPE_REAL8, obs_ak_error, errstat, 9);
  InitDS(pk9real, DATATYPE_REAL8, obs_corr_ev_1, errstat, 9);
  InitDS(pk1int, DATATYPE_INT4, obs_corr_mask, errstat, 1);
  InitDS(pk9real, DATATYPE_REAL8, obs_corr_diag_1, errstat, 9);
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 383;
    pfcom->wt = 358.000379;
    pfcom->tags = CCMA_Set_T_errstat_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_errstat_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_errstat;
    pfcom->swapout = CCMA_Swapout_T_errstat;
    pfcom->dim = CCMA_Dim_T_errstat;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_errstat;
    pfcom->remove = CCMA_Remove_T_errstat;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_errstat; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_errstat; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_errstat;
    pfcom->store = CCMA_Store_T_errstat;
    pfcom->pack = CCMA_Pack_T_errstat;
    pfcom->unpack = CCMA_Unpack_T_errstat;
    pfcom->sql = CCMA_Sql_T_errstat;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
