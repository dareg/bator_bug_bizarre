...
CCMA_Pack_T_enkf_117(void *T)
{
  ...
  if (P->Is_loaded) {
    ...
    PDS = PackDS(P, CCMA, pk9real, hprior); CHECK_PDS_ERROR(2);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_enkf_117(void *T)
{
  int Nbytes = 0;
  TABLE_enkf_117 *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk1int, member); Nbytes += BYTESIZE(P->member.d);
    UseDS(P, CCMA, pk9real, hprior); Nbytes += BYTESIZE(P->hprior.d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_enkf_117(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_enkf_117 *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(enkf_117);
  return P->Nrows;
}


PreGetTable(CCMA, d, double, enkf_117)
  Call_CopyGet_TABLE(CCMA, d, 1, enkf_117, pk1int, D, member, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 2, enkf_117, pk9real, D, hprior, Count, DATATYPE_REAL8);
PostGetTable(d, double, enkf_117)


PrePutTable(CCMA, d, double, enkf_117)
  Call_CopyPut_TABLE(CCMA, d, 1, enkf_117, pk1int, member, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 2, enkf_117, pk9real, hprior, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, enkf_117)

PreLoadTable(CCMA, enkf_117);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, member);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hprior);
PostLoadTable(enkf_117)

PreStoreTable(CCMA, enkf_117);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, member);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hprior);
PostStoreTable(enkf_117)

DefineLookupTable(enkf_117)

PUBLIC void
CCMA_Dim_T_enkf_117(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_enkf_117 *P = T;
  Call_LookupTable(enkf_117, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
CCMA_Swapout_T_enkf_117(void *T)
{
  TABLE_enkf_117 *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, hprior, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(enkf_117,2);
}

DefineRemoveTable(CCMA, enkf_117)

PUBLIC int
CCMA_Sql_T_enkf_117(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_enkf_117(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_enkf_117 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_enkf_117);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 2);
  InitDS(pk1int, DATATYPE_INT4, member, enkf_117, 1);
  InitDS(pk9real, DATATYPE_REAL8, hprior, enkf_117, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@enkf_117"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 146;
    pfcom->wt = 358.000122;
    pfcom->tags = CCMA_Set_T_enkf_117_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_enkf_117_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_enkf_117;
    pfcom->swapout = CCMA_Swapout_T_enkf_117;
    pfcom->dim = CCMA_Dim_T_enkf_117;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_enkf_117;
    pfcom->remove = CCMA_Remove_T_enkf_117;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_enkf_117; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_enkf_117; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_enkf_117;
    pfcom->store = CCMA_Store_T_enkf_117;
    pfcom->pack = CCMA_Pack_T_enkf_117;
    pfcom->unpack = CCMA_Unpack_T_enkf_117;
    pfcom->sql = CCMA_Sql_T_enkf_117;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
