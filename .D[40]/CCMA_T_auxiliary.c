...
PUBLIC int
CCMA_Pack_T_auxiliary(void *T)
{
  int Nbytes = 0;
  TABLE_auxiliary *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk9real, report_aux_1); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk9real, report_aux_2); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk9real, report_aux_3); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk9real, report_aux_4); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, pk9real, report_aux_5); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, pk9real, report_aux_6); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, pk9real, report_aux_7); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, CCMA, pk9real, report_aux_8); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, CCMA, pk9real, report_aux_9); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, CCMA, linkoffset_t, LINKOFFSET(auxiliary_body)); CHECK_PDS_ERROR(10);
    PDS = PackDS(P, CCMA, linklen_t, LINKLEN(auxiliary_body)); CHECK_PDS_ERROR(11);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_auxiliary(void *T)
{
  int Nbytes = 0;
  TABLE_auxiliary *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk9real, report_aux_1); Nbytes += BYTESIZE(P->report_aux_1.d);
    UseDS(P, CCMA, pk9real, report_aux_2); Nbytes += BYTESIZE(P->report_aux_2.d);
    UseDS(P, CCMA, pk9real, report_aux_3); Nbytes += BYTESIZE(P->report_aux_3.d);
    UseDS(P, CCMA, pk9real, report_aux_4); Nbytes += BYTESIZE(P->report_aux_4.d);
    UseDS(P, CCMA, pk9real, report_aux_5); Nbytes += BYTESIZE(P->report_aux_5.d);
    UseDS(P, CCMA, pk9real, report_aux_6); Nbytes += BYTESIZE(P->report_aux_6.d);
    UseDS(P, CCMA, pk9real, report_aux_7); Nbytes += BYTESIZE(P->report_aux_7.d);
    UseDS(P, CCMA, pk9real, report_aux_8); Nbytes += BYTESIZE(P->report_aux_8.d);
    UseDS(P, CCMA, pk9real, report_aux_9); Nbytes += BYTESIZE(P->report_aux_9.d);
    UseDS(P, CCMA, linkoffset_t, LINKOFFSET(auxiliary_body)); Nbytes += BYTESIZE(P->LINKOFFSET(auxiliary_body).d);
    UseDS(P, CCMA, linklen_t, LINKLEN(auxiliary_body)); Nbytes += BYTESIZE(P->LINKLEN(auxiliary_body).d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_auxiliary(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_auxiliary *P = T;
...
PreGetTable(CCMA, d, double, auxiliary)
  Call_CopyGet_TABLE(CCMA, d, 1, auxiliary, pk9real, D, report_aux_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, auxiliary, pk9real, D, report_aux_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, auxiliary, pk9real, D, report_aux_3, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 4, auxiliary, pk9real, D, report_aux_4, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 5, auxiliary, pk9real, D, report_aux_5, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 6, auxiliary, pk9real, D, report_aux_6, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 7, auxiliary, pk9real, D, report_aux_7, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 8, auxiliary, pk9real, D, report_aux_8, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 9, auxiliary, pk9real, D, report_aux_9, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 10, auxiliary, linkoffset_t, D, LINKOFFSET(auxiliary_body), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(CCMA, d, 11, auxiliary, linklen_t, D, LINKLEN(auxiliary_body), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, auxiliary)


PrePutTable(CCMA, d, double, auxiliary)
  Call_CopyPut_TABLE(CCMA, d, 1, auxiliary, pk9real, report_aux_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, auxiliary, pk9real, report_aux_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, auxiliary, pk9real, report_aux_3, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 4, auxiliary, pk9real, report_aux_4, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 5, auxiliary, pk9real, report_aux_5, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 6, auxiliary, pk9real, report_aux_6, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 7, auxiliary, pk9real, report_aux_7, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 8, auxiliary, pk9real, report_aux_8, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 9, auxiliary, pk9real, report_aux_9, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 10, auxiliary, linkoffset_t, LINKOFFSET(auxiliary_body), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(CCMA, d, 11, auxiliary, linklen_t, LINKLEN(auxiliary_body), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, auxiliary)

PreLoadTable(CCMA, auxiliary);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_1);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_2);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_3);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_4);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_5);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_6);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_7);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_8);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_9);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(auxiliary_body));
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(auxiliary_body));
PostLoadTable(auxiliary)

PreStoreTable(CCMA, auxiliary);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_1);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_2);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_3);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_4);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_5);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_6);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_7);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_8);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_9);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(auxiliary_body));
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(auxiliary_body));
PostStoreTable(auxiliary)

DefineLookupTable(auxiliary)

PUBLIC void
CCMA_Dim_T_auxiliary(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_auxiliary(void *T)
{
  TABLE_auxiliary *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, auxiliary)

PUBLIC int
CCMA_Sql_T_auxiliary(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_auxiliary(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_auxiliary *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_auxiliary);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 11);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 12;
    pfcom->wt = 3.001004;
    pfcom->tags = CCMA_Set_T_auxiliary_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_auxiliary_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_auxiliary;
    pfcom->swapout = CCMA_Swapout_T_auxiliary;
    pfcom->dim = CCMA_Dim_T_auxiliary;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_auxiliary;
    pfcom->remove = CCMA_Remove_T_auxiliary;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_auxiliary; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_auxiliary; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_auxiliary;
    pfcom->store = CCMA_Store_T_auxiliary;
    pfcom->pack = CCMA_Pack_T_auxiliary;
    pfcom->unpack = CCMA_Unpack_T_auxiliary;
    pfcom->sql = CCMA_Sql_T_auxiliary;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
