...
PUBLIC int
CCMA_Pack_T_fcdiagnostic_body_13(void *T)
{
  int Nbytes = 0;
...
PUBLIC int
CCMA_Unpack_T_fcdiagnostic_body_13(void *T)
{
  int Nbytes = 0;
  TABLE_fcdiagnostic_body_13 *P = T;
  if (P->Is_loaded) {
...
PUBLIC int
CCMA_Sel_T_fcdiagnostic_body_13(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_fcdiagnostic_body_13 *P = T;
...
PreGetTable(CCMA, d, double, fcdiagnostic_body_13)
  Call_CopyGet_TABLE(CCMA, d, 1, fcdiagnostic_body_13, pk9real, D, fc_depar, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, fcdiagnostic_body_13, yyyymmdd, D, fc_step, Count, DATATYPE_YYYYMMDD);
PostGetTable(d, double, fcdiagnostic_body_13)


PrePutTable(CCMA, d, double, fcdiagnostic_body_13)
  Call_CopyPut_TABLE(CCMA, d, 1, fcdiagnostic_body_13, pk9real, fc_depar, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, fcdiagnostic_body_13, yyyymmdd, fc_step, D, Count, DATATYPE_YYYYMMDD);
...
PUBLIC void
CCMA_Dim_T_fcdiagnostic_body_13(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_fcdiagnostic_body_13(void *T)
{
  TABLE_fcdiagnostic_body_13 *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, fcdiagnostic_body_13)

PUBLIC int
CCMA_Sql_T_fcdiagnostic_body_13(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_fcdiagnostic_body_13(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_fcdiagnostic_body_13 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_fcdiagnostic_body_13);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 2);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 362;
    pfcom->wt = 358.000339;
    pfcom->tags = CCMA_Set_T_fcdiagnostic_body_13_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_fcdiagnostic_body_13_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_fcdiagnostic_body_13;
    pfcom->swapout = CCMA_Swapout_T_fcdiagnostic_body_13;
    pfcom->dim = CCMA_Dim_T_fcdiagnostic_body_13;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_fcdiagnostic_body_13;
    pfcom->remove = CCMA_Remove_T_fcdiagnostic_body_13;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_fcdiagnostic_body_13; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_fcdiagnostic_body_13; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_fcdiagnostic_body_13;
    pfcom->store = CCMA_Store_T_fcdiagnostic_body_13;
    pfcom->pack = CCMA_Pack_T_fcdiagnostic_body_13;
    pfcom->unpack = CCMA_Unpack_T_fcdiagnostic_body_13;
    pfcom->sql = CCMA_Sql_T_fcdiagnostic_body_13;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
