...
PUBLIC int
CCMA_Pack_T_index(void *T)
{
  int Nbytes = 0;
  TABLE_index *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk1int, target); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk1int, procid); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk1int, timeslot); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk1int, kset); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, pk1int, abnob); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, pk1int, mapomm); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, pk1int, maptovscv); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, CCMA, linkoffset_t, LINKOFFSET(hdr)); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, CCMA, linklen_t, LINKLEN(hdr)); CHECK_PDS_ERROR(9);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_index(void *T)
{
  int Nbytes = 0;
  TABLE_index *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk1int, target); Nbytes += BYTESIZE(P->target.d);
    UseDS(P, CCMA, pk1int, procid); Nbytes += BYTESIZE(P->procid.d);
    UseDS(P, CCMA, pk1int, timeslot); Nbytes += BYTESIZE(P->timeslot.d);
    UseDS(P, CCMA, pk1int, kset); Nbytes += BYTESIZE(P->kset.d);
    UseDS(P, CCMA, pk1int, abnob); Nbytes += BYTESIZE(P->abnob.d);
    UseDS(P, CCMA, pk1int, mapomm); Nbytes += BYTESIZE(P->mapomm.d);
    UseDS(P, CCMA, pk1int, maptovscv); Nbytes += BYTESIZE(P->maptovscv.d);
    UseDS(P, CCMA, linkoffset_t, LINKOFFSET(hdr)); Nbytes += BYTESIZE(P->LINKOFFSET(hdr).d);
    UseDS(P, CCMA, linklen_t, LINKLEN(hdr)); Nbytes += BYTESIZE(P->LINKLEN(hdr).d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_index(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_index *P = T;
...
PreGetTable(CCMA, d, double, index)
  Call_CopyGet_TABLE(CCMA, d, 1, index, pk1int, D, target, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 2, index, pk1int, D, procid, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 3, index, pk1int, D, timeslot, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 4, index, pk1int, D, kset, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 5, index, pk1int, D, abnob, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 6, index, pk1int, D, mapomm, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 7, index, pk1int, D, maptovscv, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 8, index, linkoffset_t, D, LINKOFFSET(hdr), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(CCMA, d, 9, index, linklen_t, D, LINKLEN(hdr), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, index)


PrePutTable(CCMA, d, double, index)
  Call_CopyPut_TABLE(CCMA, d, 1, index, pk1int, target, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 2, index, pk1int, procid, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 3, index, pk1int, timeslot, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 4, index, pk1int, kset, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 5, index, pk1int, abnob, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 6, index, pk1int, mapomm, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 7, index, pk1int, maptovscv, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 8, index, linkoffset_t, LINKOFFSET(hdr), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(CCMA, d, 9, index, linklen_t, LINKLEN(hdr), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, index)

PreLoadTable(CCMA, index);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, target);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, procid);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, timeslot);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, kset);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, abnob);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, mapomm);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, maptovscv);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(hdr));
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(hdr));
PostLoadTable(index)

PreStoreTable(CCMA, index);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, target);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, procid);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, timeslot);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, kset);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, abnob);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, mapomm);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, maptovscv);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(hdr));
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(hdr));
PostStoreTable(index)

DefineLookupTable(index)

PUBLIC void
CCMA_Dim_T_index(void *T, int *Nrows, int *Ncols,
...
DefineRemoveTable(CCMA, index)

PUBLIC int
CCMA_Sql_T_index(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_index(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_index *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_index);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 9);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 3;
    pfcom->wt = 1.000003;
    pfcom->tags = CCMA_Set_T_index_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_index_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_index;
    pfcom->swapout = CCMA_Swapout_T_index;
    pfcom->dim = CCMA_Dim_T_index;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_index;
    pfcom->remove = CCMA_Remove_T_index;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_index; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_index; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_index;
    pfcom->store = CCMA_Store_T_index;
    pfcom->pack = CCMA_Pack_T_index;
    pfcom->unpack = CCMA_Unpack_T_index;
    pfcom->sql = CCMA_Sql_T_index;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
