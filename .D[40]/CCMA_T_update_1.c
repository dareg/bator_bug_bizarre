...
PUBLIC int
CCMA_Pack_T_update_1(void *T)
{
  int Nbytes = 0;
  TABLE_update_1 *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk9real, hires); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk9real, lores); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk1int, datum_tbflag_hires); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, Bitfield, datum_status_hires); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, pk9real, obs_diags_1); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, pk9real, obs_diags_2); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, pk9real, obs_diags_3); CHECK_PDS_ERROR(7);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_update_1(void *T)
{
  int Nbytes = 0;
  TABLE_update_1 *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk9real, hires); Nbytes += BYTESIZE(P->hires.d);
    UseDS(P, CCMA, pk9real, lores); Nbytes += BYTESIZE(P->lores.d);
    UseDS(P, CCMA, pk1int, datum_tbflag_hires); Nbytes += BYTESIZE(P->datum_tbflag_hires.d);
    UseDS(P, CCMA, Bitfield, datum_status_hires); Nbytes += BYTESIZE(P->datum_status_hires.d);
    UseDS(P, CCMA, pk9real, obs_diags_1); Nbytes += BYTESIZE(P->obs_diags_1.d);
    UseDS(P, CCMA, pk9real, obs_diags_2); Nbytes += BYTESIZE(P->obs_diags_2.d);
    UseDS(P, CCMA, pk9real, obs_diags_3); Nbytes += BYTESIZE(P->obs_diags_3.d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_update_1(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_update_1 *P = T;
...
PreGetTable(CCMA, d, double, update_1)
  Call_CopyGet_TABLE(CCMA, d, 1, update_1, pk9real, D, hires, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, update_1, pk9real, D, lores, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, update_1, pk1int, D, datum_tbflag_hires, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 4, update_1, Bitfield, D, datum_status_hires, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(CCMA, d, 5, update_1, pk9real, D, obs_diags_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 6, update_1, pk9real, D, obs_diags_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 7, update_1, pk9real, D, obs_diags_3, Count, DATATYPE_REAL8);
PostGetTable(d, double, update_1)


PrePutTable(CCMA, d, double, update_1)
  Call_CopyPut_TABLE(CCMA, d, 1, update_1, pk9real, hires, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, update_1, pk9real, lores, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, update_1, pk1int, datum_tbflag_hires, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 4, update_1, Bitfield, datum_status_hires, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(CCMA, d, 5, update_1, pk9real, obs_diags_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 6, update_1, pk9real, obs_diags_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 7, update_1, pk9real, obs_diags_3, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, update_1)

PreLoadTable(CCMA, update_1);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hires);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, lores);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, datum_tbflag_hires);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, datum_status_hires);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_diags_1);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_diags_2);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_diags_3);
PostLoadTable(update_1)

PreStoreTable(CCMA, update_1);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hires);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, lores);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, datum_tbflag_hires);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, datum_status_hires);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_diags_1);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_diags_2);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_diags_3);
PostStoreTable(update_1)

DefineLookupTable(update_1)

PUBLIC void
CCMA_Dim_T_update_1(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_update_1(void *T)
{
  TABLE_update_1 *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, update_1)

PUBLIC int
CCMA_Sql_T_update_1(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_update_1(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_update_1 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_update_1);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 7);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 384;
    pfcom->wt = 358.000380;
    pfcom->tags = CCMA_Set_T_update_1_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_update_1_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_update_1;
    pfcom->swapout = CCMA_Swapout_T_update_1;
    pfcom->dim = CCMA_Dim_T_update_1;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_update_1;
    pfcom->remove = CCMA_Remove_T_update_1;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_update_1; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_update_1; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_update_1;
    pfcom->store = CCMA_Store_T_update_1;
    pfcom->pack = CCMA_Pack_T_update_1;
    pfcom->unpack = CCMA_Unpack_T_update_1;
    pfcom->sql = CCMA_Sql_T_update_1;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
