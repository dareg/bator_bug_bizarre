...
PUBLIC int
CCMA_Pack_T_timeslot_index(void *T)
{
  int Nbytes = 0;
  ...
  if (P->Is_loaded) {
    ...
    PDS = PackDS(P, CCMA, pk1int, model_timestep); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, yyyymmdd, enddate); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, hhmmss, endtime); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, linkoffset_t, LINKOFFSET(index)); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, linklen_t, LINKLEN(index)); CHECK_PDS_ERROR(6);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_timeslot_index(void *T)
{
  int Nbytes = 0;
  TABLE_timeslot_index *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk1int, timeslot); Nbytes += BYTESIZE(P->timeslot.d);
    UseDS(P, CCMA, pk1int, model_timestep); Nbytes += BYTESIZE(P->model_timestep.d);
    UseDS(P, CCMA, yyyymmdd, enddate); Nbytes += BYTESIZE(P->enddate.d);
    UseDS(P, CCMA, hhmmss, endtime); Nbytes += BYTESIZE(P->endtime.d);
    UseDS(P, CCMA, linkoffset_t, LINKOFFSET(index)); Nbytes += BYTESIZE(P->LINKOFFSET(index).d);
    UseDS(P, CCMA, linklen_t, LINKLEN(index)); Nbytes += BYTESIZE(P->LINKLEN(index).d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_timeslot_index(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_timeslot_index *P = T;
...
PreGetTable(CCMA, d, double, timeslot_index)
  Call_CopyGet_TABLE(CCMA, d, 1, timeslot_index, pk1int, D, timeslot, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 2, timeslot_index, pk1int, D, model_timestep, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 3, timeslot_index, yyyymmdd, D, enddate, Count, DATATYPE_YYYYMMDD);
  Call_CopyGet_TABLE(CCMA, d, 4, timeslot_index, hhmmss, D, endtime, Count, DATATYPE_HHMMSS);
  Call_CopyGet_TABLE(CCMA, d, 5, timeslot_index, linkoffset_t, D, LINKOFFSET(index), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(CCMA, d, 6, timeslot_index, linklen_t, D, LINKLEN(index), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, timeslot_index)


PrePutTable(CCMA, d, double, timeslot_index)
  Call_CopyPut_TABLE(CCMA, d, 1, timeslot_index, pk1int, timeslot, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 2, timeslot_index, pk1int, model_timestep, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 3, timeslot_index, yyyymmdd, enddate, D, Count, DATATYPE_YYYYMMDD);
  Call_CopyPut_TABLE(CCMA, d, 4, timeslot_index, hhmmss, endtime, D, Count, DATATYPE_HHMMSS);
  Call_CopyPut_TABLE(CCMA, d, 5, timeslot_index, linkoffset_t, LINKOFFSET(index), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(CCMA, d, 6, timeslot_index, linklen_t, LINKLEN(index), D, Count, DATATYPE_LINKLEN);
...
PreLoadTable(CCMA, timeslot_index);
  ...
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, model_timestep);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, yyyymmdd, DATATYPE_YYYYMMDD, enddate);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, hhmmss, DATATYPE_HHMMSS, endtime);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(index));
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(index));
...
PreStoreTable(CCMA, timeslot_index);
  ...
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, model_timestep);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, yyyymmdd, DATATYPE_YYYYMMDD, enddate);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, hhmmss, DATATYPE_HHMMSS, endtime);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(index));
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(index));
PostStoreTable(timeslot_index)

DefineLookupTable(timeslot_index)

PUBLIC void
CCMA_Dim_T_timeslot_index(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_timeslot_index(void *T)
{
  TABLE_timeslot_index *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, timeslot_index)

PUBLIC int
CCMA_Sql_T_timeslot_index(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_timeslot_index(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_timeslot_index *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_timeslot_index);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 6);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 2;
    pfcom->wt = 1.000002;
    pfcom->tags = CCMA_Set_T_timeslot_index_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_timeslot_index_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_timeslot_index;
    pfcom->swapout = CCMA_Swapout_T_timeslot_index;
    pfcom->dim = CCMA_Dim_T_timeslot_index;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_timeslot_index;
    pfcom->remove = CCMA_Remove_T_timeslot_index;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_timeslot_index; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_timeslot_index; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_timeslot_index;
    pfcom->store = CCMA_Store_T_timeslot_index;
    pfcom->pack = CCMA_Pack_T_timeslot_index;
    pfcom->unpack = CCMA_Unpack_T_timeslot_index;
    pfcom->sql = CCMA_Sql_T_timeslot_index;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
