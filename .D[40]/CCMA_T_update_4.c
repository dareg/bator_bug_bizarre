...
PUBLIC int
CCMA_Sel_T_update_4(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_update_4 *P = T;
...
PreGetTable(CCMA, d, double, update_4)
  Call_CopyGet_TABLE(CCMA, d, 1, update_4, pk9real, D, hires, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, update_4, pk9real, D, lores, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, update_4, pk1int, D, datum_tbflag_hires, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 4, update_4, Bitfield, D, datum_status_hires, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(CCMA, d, 5, update_4, pk9real, D, obs_diags_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 6, update_4, pk9real, D, obs_diags_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 7, update_4, pk9real, D, obs_diags_3, Count, DATATYPE_REAL8);
PostGetTable(d, double, update_4)


PrePutTable(CCMA, d, double, update_4)
  Call_CopyPut_TABLE(CCMA, d, 1, update_4, pk9real, hires, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, update_4, pk9real, lores, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, update_4, pk1int, datum_tbflag_hires, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 4, update_4, Bitfield, datum_status_hires, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(CCMA, d, 5, update_4, pk9real, obs_diags_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 6, update_4, pk9real, obs_diags_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 7, update_4, pk9real, obs_diags_3, D, Count, DATATYPE_REAL8);
...
PUBLIC void
CCMA_Dim_T_update_4(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_update_4(void *T)
{
  TABLE_update_4 *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, update_4)

PUBLIC int
CCMA_Sql_T_update_4(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_update_4(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_update_4 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_update_4);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 7);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 387;
    pfcom->wt = 358.000383;
    pfcom->tags = CCMA_Set_T_update_4_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_update_4_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_update_4;
    pfcom->swapout = CCMA_Swapout_T_update_4;
    pfcom->dim = CCMA_Dim_T_update_4;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_update_4;
    pfcom->remove = CCMA_Remove_T_update_4;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_update_4; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_update_4; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_update_4;
    pfcom->store = CCMA_Store_T_update_4;
    pfcom->pack = CCMA_Pack_T_update_4;
    pfcom->unpack = CCMA_Unpack_T_update_4;
    pfcom->sql = CCMA_Sql_T_update_4;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
