...
PUBLIC int
CCMA_Pack_T_gnssro_body(void *T)
{
  int Nbytes = 0;
  TABLE_gnssro_body *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk9real, obs_dndz); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk9real, obs_refractivity); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk9real, bg_dndz); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk9real, bg_refractivity); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, pk9real, bg_layerno); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, pk9real, obs_tvalue); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, pk9real, obs_zvalue); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, CCMA, pk9real, bg_tvalue); CHECK_PDS_ERROR(8);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_gnssro_body(void *T)
{
  int Nbytes = 0;
  TABLE_gnssro_body *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk9real, obs_dndz); Nbytes += BYTESIZE(P->obs_dndz.d);
    UseDS(P, CCMA, pk9real, obs_refractivity); Nbytes += BYTESIZE(P->obs_refractivity.d);
    UseDS(P, CCMA, pk9real, bg_dndz); Nbytes += BYTESIZE(P->bg_dndz.d);
    UseDS(P, CCMA, pk9real, bg_refractivity); Nbytes += BYTESIZE(P->bg_refractivity.d);
    UseDS(P, CCMA, pk9real, bg_layerno); Nbytes += BYTESIZE(P->bg_layerno.d);
    UseDS(P, CCMA, pk9real, obs_tvalue); Nbytes += BYTESIZE(P->obs_tvalue.d);
    UseDS(P, CCMA, pk9real, obs_zvalue); Nbytes += BYTESIZE(P->obs_zvalue.d);
    UseDS(P, CCMA, pk9real, bg_tvalue); Nbytes += BYTESIZE(P->bg_tvalue.d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_gnssro_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_gnssro_body *P = T;
...
PreGetTable(CCMA, d, double, gnssro_body)
  Call_CopyGet_TABLE(CCMA, d, 1, gnssro_body, pk9real, D, obs_dndz, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, gnssro_body, pk9real, D, obs_refractivity, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, gnssro_body, pk9real, D, bg_dndz, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 4, gnssro_body, pk9real, D, bg_refractivity, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 5, gnssro_body, pk9real, D, bg_layerno, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 6, gnssro_body, pk9real, D, obs_tvalue, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 7, gnssro_body, pk9real, D, obs_zvalue, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 8, gnssro_body, pk9real, D, bg_tvalue, Count, DATATYPE_REAL8);
PostGetTable(d, double, gnssro_body)


PrePutTable(CCMA, d, double, gnssro_body)
  Call_CopyPut_TABLE(CCMA, d, 1, gnssro_body, pk9real, obs_dndz, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, gnssro_body, pk9real, obs_refractivity, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, gnssro_body, pk9real, bg_dndz, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 4, gnssro_body, pk9real, bg_refractivity, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 5, gnssro_body, pk9real, bg_layerno, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 6, gnssro_body, pk9real, obs_tvalue, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 7, gnssro_body, pk9real, obs_zvalue, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 8, gnssro_body, pk9real, bg_tvalue, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, gnssro_body)

PreLoadTable(CCMA, gnssro_body);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_dndz);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_refractivity);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_dndz);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_refractivity);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_layerno);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_tvalue);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_zvalue);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_tvalue);
PostLoadTable(gnssro_body)

PreStoreTable(CCMA, gnssro_body);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_dndz);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_refractivity);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_dndz);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_refractivity);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_layerno);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_tvalue);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_zvalue);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_tvalue);
PostStoreTable(gnssro_body)

DefineLookupTable(gnssro_body)

PUBLIC void
CCMA_Dim_T_gnssro_body(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_gnssro_body(void *T)
{
  TABLE_gnssro_body *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, gnssro_body)

PUBLIC int
CCMA_Sql_T_gnssro_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_gnssro_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_gnssro_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_gnssro_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 8);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 378;
    pfcom->wt = 358.000367;
    pfcom->tags = CCMA_Set_T_gnssro_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_gnssro_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_gnssro_body;
    pfcom->swapout = CCMA_Swapout_T_gnssro_body;
    pfcom->dim = CCMA_Dim_T_gnssro_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_gnssro_body;
    pfcom->remove = CCMA_Remove_T_gnssro_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_gnssro_body; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_gnssro_body; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_gnssro_body;
    pfcom->store = CCMA_Store_T_gnssro_body;
    pfcom->pack = CCMA_Pack_T_gnssro_body;
    pfcom->unpack = CCMA_Unpack_T_gnssro_body;
    pfcom->sql = CCMA_Sql_T_gnssro_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
