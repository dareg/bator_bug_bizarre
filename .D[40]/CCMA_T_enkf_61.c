...
PUBLIC int
CCMA_Sel_T_enkf_61(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_enkf_61 *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(enkf_61);
  return P->Nrows;
}


PreGetTable(CCMA, d, double, enkf_61)
  Call_CopyGet_TABLE(CCMA, d, 1, enkf_61, pk1int, D, member, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 2, enkf_61, pk9real, D, hprior, Count, DATATYPE_REAL8);
PostGetTable(d, double, enkf_61)


PrePutTable(CCMA, d, double, enkf_61)
  Call_CopyPut_TABLE(CCMA, d, 1, enkf_61, pk1int, member, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 2, enkf_61, pk9real, hprior, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, enkf_61)
...
PostStoreTable(enkf_61)

DefineLookupTable(enkf_61)

PUBLIC void
CCMA_Dim_T_enkf_61(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_enkf_61 *P = T;
  Call_LookupTable(enkf_61, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
CCMA_Swapout_T_enkf_61(void *T)
{
  TABLE_enkf_61 *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  ODBMAC_TRACE_SWAPOUT(enkf_61,2);
}

DefineRemoveTable(CCMA, enkf_61)

PUBLIC int
CCMA_Sql_T_enkf_61(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_enkf_61(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_enkf_61 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_enkf_61);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 2);
  InitDS(pk1int, DATATYPE_INT4, member, enkf_61, 1);
  InitDS(pk9real, DATATYPE_REAL8, hprior, enkf_61, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@enkf_61"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 90;
    pfcom->wt = 358.000066;
    pfcom->tags = CCMA_Set_T_enkf_61_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_enkf_61_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_enkf_61;
    pfcom->swapout = CCMA_Swapout_T_enkf_61;
    pfcom->dim = CCMA_Dim_T_enkf_61;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_enkf_61;
    pfcom->remove = CCMA_Remove_T_enkf_61;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_enkf_61; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_enkf_61; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_enkf_61;
    pfcom->store = CCMA_Store_T_enkf_61;
    pfcom->pack = CCMA_Pack_T_enkf_61;
    pfcom->unpack = CCMA_Unpack_T_enkf_61;
    pfcom->sql = CCMA_Sql_T_enkf_61;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
