...
PUBLIC int
CCMA_Pack_T_raingg_body(void *T)
{
  int Nbytes = 0;
  TABLE_raingg_body *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk9real, rrvalue); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk9real, rrvaluetl); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk9real, rrvaluead); CHECK_PDS_ERROR(3);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_raingg_body(void *T)
{
  int Nbytes = 0;
  TABLE_raingg_body *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk9real, rrvalue); Nbytes += BYTESIZE(P->rrvalue.d);
    UseDS(P, CCMA, pk9real, rrvaluetl); Nbytes += BYTESIZE(P->rrvaluetl.d);
    UseDS(P, CCMA, pk9real, rrvaluead); Nbytes += BYTESIZE(P->rrvaluead.d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_raingg_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_raingg_body *P = T;
...
PreGetTable(CCMA, d, double, raingg_body)
  Call_CopyGet_TABLE(CCMA, d, 1, raingg_body, pk9real, D, rrvalue, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, raingg_body, pk9real, D, rrvaluetl, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, raingg_body, pk9real, D, rrvaluead, Count, DATATYPE_REAL8);
PostGetTable(d, double, raingg_body)


PrePutTable(CCMA, d, double, raingg_body)
  Call_CopyPut_TABLE(CCMA, d, 1, raingg_body, pk9real, rrvalue, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, raingg_body, pk9real, rrvaluetl, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, raingg_body, pk9real, rrvaluead, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, raingg_body)

PreLoadTable(CCMA, raingg_body);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rrvalue);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rrvaluetl);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rrvaluead);
PostLoadTable(raingg_body)

PreStoreTable(CCMA, raingg_body);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rrvalue);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rrvaluetl);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rrvaluead);
PostStoreTable(raingg_body)

DefineLookupTable(raingg_body)

PUBLIC void
CCMA_Dim_T_raingg_body(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_raingg_body(void *T)
{
  TABLE_raingg_body *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, raingg_body)

PUBLIC int
CCMA_Sql_T_raingg_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_raingg_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_raingg_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_raingg_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 3);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 371;
    pfcom->wt = 358.000350;
    pfcom->tags = CCMA_Set_T_raingg_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_raingg_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_raingg_body;
    pfcom->swapout = CCMA_Swapout_T_raingg_body;
    pfcom->dim = CCMA_Dim_T_raingg_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_raingg_body;
    pfcom->remove = CCMA_Remove_T_raingg_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_raingg_body; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_raingg_body; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_raingg_body;
    pfcom->store = CCMA_Store_T_raingg_body;
    pfcom->pack = CCMA_Pack_T_raingg_body;
    pfcom->unpack = CCMA_Unpack_T_raingg_body;
    pfcom->sql = CCMA_Sql_T_raingg_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
