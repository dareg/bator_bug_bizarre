...
PUBLIC int
CCMA_Pack_T_conv_body(void *T)
{
  int Nbytes = 0;
  TABLE_conv_body *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk1int, ppcode); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, Bitfield, level); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk1int, datum_qcflag); CHECK_PDS_ERROR(3);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_conv_body(void *T)
{
  int Nbytes = 0;
  TABLE_conv_body *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk1int, ppcode); Nbytes += BYTESIZE(P->ppcode.d);
    UseDS(P, CCMA, Bitfield, level); Nbytes += BYTESIZE(P->level.d);
    UseDS(P, CCMA, pk1int, datum_qcflag); Nbytes += BYTESIZE(P->datum_qcflag.d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_conv_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_conv_body *P = T;
...
PreGetTable(CCMA, d, double, conv_body)
  Call_CopyGet_TABLE(CCMA, d, 1, conv_body, pk1int, D, ppcode, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 2, conv_body, Bitfield, D, level, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(CCMA, d, 3, conv_body, pk1int, D, datum_qcflag, Count, DATATYPE_INT4);
PostGetTable(d, double, conv_body)


PrePutTable(CCMA, d, double, conv_body)
  Call_CopyPut_TABLE(CCMA, d, 1, conv_body, pk1int, ppcode, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 2, conv_body, Bitfield, level, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(CCMA, d, 3, conv_body, pk1int, datum_qcflag, D, Count, DATATYPE_INT4);
PostPutTable(d, double, conv_body)

PreLoadTable(CCMA, conv_body);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, ppcode);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, level);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, datum_qcflag);
PostLoadTable(conv_body)

PreStoreTable(CCMA, conv_body);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, ppcode);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, level);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, datum_qcflag);
PostStoreTable(conv_body)

DefineLookupTable(conv_body)

PUBLIC void
CCMA_Dim_T_conv_body(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_conv_body(void *T)
{
  TABLE_conv_body *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, conv_body)

PUBLIC int
CCMA_Sql_T_conv_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_conv_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_conv_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_conv_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 3);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 372;
    pfcom->wt = 358.000352;
    pfcom->tags = CCMA_Set_T_conv_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_conv_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_conv_body;
    pfcom->swapout = CCMA_Swapout_T_conv_body;
    pfcom->dim = CCMA_Dim_T_conv_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_conv_body;
    pfcom->remove = CCMA_Remove_T_conv_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_conv_body; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_conv_body; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_conv_body;
    pfcom->store = CCMA_Store_T_conv_body;
    pfcom->pack = CCMA_Pack_T_conv_body;
    pfcom->unpack = CCMA_Unpack_T_conv_body;
    pfcom->sql = CCMA_Sql_T_conv_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
