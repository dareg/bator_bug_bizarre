...
PUBLIC int
CCMA_Pack_T_gnssro(void *T)
{
  int Nbytes = 0;
  TABLE_gnssro *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk9real, radcurv); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, pk9real, undulation); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, linkoffset_t, LINKOFFSET(gnssro_body)); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, linklen_t, LINKLEN(gnssro_body)); CHECK_PDS_ERROR(4);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_gnssro(void *T)
{
  int Nbytes = 0;
  TABLE_gnssro *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk9real, radcurv); Nbytes += BYTESIZE(P->radcurv.d);
    UseDS(P, CCMA, pk9real, undulation); Nbytes += BYTESIZE(P->undulation.d);
    UseDS(P, CCMA, linkoffset_t, LINKOFFSET(gnssro_body)); Nbytes += BYTESIZE(P->LINKOFFSET(gnssro_body).d);
    UseDS(P, CCMA, linklen_t, LINKLEN(gnssro_body)); Nbytes += BYTESIZE(P->LINKLEN(gnssro_body).d);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Sel_T_gnssro(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_gnssro *P = T;
...
PreGetTable(CCMA, d, double, gnssro)
  Call_CopyGet_TABLE(CCMA, d, 1, gnssro, pk9real, D, radcurv, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, gnssro, pk9real, D, undulation, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, gnssro, linkoffset_t, D, LINKOFFSET(gnssro_body), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(CCMA, d, 4, gnssro, linklen_t, D, LINKLEN(gnssro_body), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, gnssro)


PrePutTable(CCMA, d, double, gnssro)
  Call_CopyPut_TABLE(CCMA, d, 1, gnssro, pk9real, radcurv, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, gnssro, pk9real, undulation, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, gnssro, linkoffset_t, LINKOFFSET(gnssro_body), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(CCMA, d, 4, gnssro, linklen_t, LINKLEN(gnssro_body), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, gnssro)

PreLoadTable(CCMA, gnssro);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, radcurv);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, undulation);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(gnssro_body));
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(gnssro_body));
PostLoadTable(gnssro)

PreStoreTable(CCMA, gnssro);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, radcurv);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, undulation);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(gnssro_body));
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(gnssro_body));
PostStoreTable(gnssro)

DefineLookupTable(gnssro)

PUBLIC void
CCMA_Dim_T_gnssro(void *T, int *Nrows, int *Ncols,
...
DefineRemoveTable(CCMA, gnssro)

PUBLIC int
CCMA_Sql_T_gnssro(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_gnssro(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_gnssro *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_gnssro);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 4);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 15;
    pfcom->wt = 3.002004;
    pfcom->tags = CCMA_Set_T_gnssro_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_gnssro_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_gnssro;
    pfcom->swapout = CCMA_Swapout_T_gnssro;
    pfcom->dim = CCMA_Dim_T_gnssro;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_gnssro;
    pfcom->remove = CCMA_Remove_T_gnssro;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_gnssro; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_gnssro; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_gnssro;
    pfcom->store = CCMA_Store_T_gnssro;
    pfcom->pack = CCMA_Pack_T_gnssro;
    pfcom->unpack = CCMA_Unpack_T_gnssro;
    pfcom->sql = CCMA_Sql_T_gnssro;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
