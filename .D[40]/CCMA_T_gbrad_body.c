...
PUBLIC int
CCMA_Unpack_T_gbrad_body(void *T)
{
  int Nbytes = 0;
  TABLE_gbrad_body *P = T;
  if (P->Is_loaded) {
...
PUBLIC int
CCMA_Sel_T_gbrad_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_gbrad_body *P = T;
...
PreGetTable(CCMA, d, double, gbrad_body)
  Call_CopyGet_TABLE(CCMA, d, 1, gbrad_body, pk9real, D, rrvalue, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 2, gbrad_body, pk9real, D, rrvaluetl, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 3, gbrad_body, pk9real, D, rrvaluead, Count, DATATYPE_REAL8);
PostGetTable(d, double, gbrad_body)


PrePutTable(CCMA, d, double, gbrad_body)
  Call_CopyPut_TABLE(CCMA, d, 1, gbrad_body, pk9real, rrvalue, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 2, gbrad_body, pk9real, rrvaluetl, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 3, gbrad_body, pk9real, rrvaluead, D, Count, DATATYPE_REAL8);
...
PUBLIC void
CCMA_Dim_T_gbrad_body(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_gbrad_body(void *T)
{
  TABLE_gbrad_body *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, gbrad_body)

PUBLIC int
CCMA_Sql_T_gbrad_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_gbrad_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_gbrad_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_gbrad_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 3);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 370;
    pfcom->wt = 358.000348;
    pfcom->tags = CCMA_Set_T_gbrad_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_gbrad_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_gbrad_body;
    pfcom->swapout = CCMA_Swapout_T_gbrad_body;
    pfcom->dim = CCMA_Dim_T_gbrad_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_gbrad_body;
    pfcom->remove = CCMA_Remove_T_gbrad_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_gbrad_body; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_gbrad_body; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_gbrad_body;
    pfcom->store = CCMA_Store_T_gbrad_body;
    pfcom->pack = CCMA_Pack_T_gbrad_body;
    pfcom->unpack = CCMA_Unpack_T_gbrad_body;
    pfcom->sql = CCMA_Sql_T_gbrad_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
