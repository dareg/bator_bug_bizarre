...
PUBLIC int
CCMA_Pack_T_radar_station(void *T)
{
  int Nbytes = 0;
  TABLE_radar_station *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, CCMA, pk1int, ident); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, CCMA, string, type); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, CCMA, pk9real, lat); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, CCMA, pk9real, lon); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, CCMA, pk9real, stalt); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, CCMA, pk9real, antenht); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, CCMA, pk9real, beamwidth); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, CCMA, pk9real, frequency); CHECK_PDS_ERROR(8);
  }
  return Nbytes;
}

PUBLIC int
CCMA_Unpack_T_radar_station(void *T)
{
  int Nbytes = 0;
  TABLE_radar_station *P = T;
  if (P->Is_loaded) {
    UseDS(P, CCMA, pk1int, ident); Nbytes += BYTESIZE(P->ident.d);
    UseDS(P, CCMA, string, type); Nbytes += BYTESIZE(P->type.d);
    ...
    UseDS(P, CCMA, pk9real, antenht); Nbytes += BYTESIZE(P->antenht.d);
    UseDS(P, CCMA, pk9real, beamwidth); Nbytes += BYTESIZE(P->beamwidth.d);
...
PUBLIC int
CCMA_Sel_T_radar_station(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_radar_station *P = T;
...
PreGetTable(CCMA, d, double, radar_station)
  Call_CopyGet_TABLE(CCMA, d, 1, radar_station, pk1int, D, ident, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(CCMA, d, 2, radar_station, string, D, type, Count, DATATYPE_STRING);
  Call_CopyGet_TABLE(CCMA, d, 3, radar_station, pk9real, D, lat, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 4, radar_station, pk9real, D, lon, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 5, radar_station, pk9real, D, stalt, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 6, radar_station, pk9real, D, antenht, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 7, radar_station, pk9real, D, beamwidth, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(CCMA, d, 8, radar_station, pk9real, D, frequency, Count, DATATYPE_REAL8);
PostGetTable(d, double, radar_station)


PrePutTable(CCMA, d, double, radar_station)
  Call_CopyPut_TABLE(CCMA, d, 1, radar_station, pk1int, ident, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(CCMA, d, 2, radar_station, string, type, D, Count, DATATYPE_STRING);
  Call_CopyPut_TABLE(CCMA, d, 3, radar_station, pk9real, lat, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 4, radar_station, pk9real, lon, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 5, radar_station, pk9real, stalt, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 6, radar_station, pk9real, antenht, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 7, radar_station, pk9real, beamwidth, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(CCMA, d, 8, radar_station, pk9real, frequency, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, radar_station)

PreLoadTable(CCMA, radar_station);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, ident);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, string, DATATYPE_STRING, type);
  ...
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, antenht);
  Call_Read_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, beamwidth);
...
PreStoreTable(CCMA, radar_station);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, ident);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, string, DATATYPE_STRING, type);
  ...
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, antenht);
  Call_Write_DS(CCMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, beamwidth);
...
PUBLIC void
CCMA_Dim_T_radar_station(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
CCMA_Swapout_T_radar_station(void *T)
{
  TABLE_radar_station *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(CCMA, radar_station)

PUBLIC int
CCMA_Sql_T_radar_station(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
CCMA_Init_T_radar_station(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_radar_station *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(CCMA_Init_T_radar_station);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 8);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 20;
    pfcom->wt = 3.002004;
    pfcom->tags = CCMA_Set_T_radar_station_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = CCMA_Set_T_radar_station_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = CCMA_Init_T_radar_station;
    pfcom->swapout = CCMA_Swapout_T_radar_station;
    pfcom->dim = CCMA_Dim_T_radar_station;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = CCMA_Sel_T_radar_station;
    pfcom->remove = CCMA_Remove_T_radar_station;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = CCMA_dGet_T_radar_station; /* REAL(8) dbmgr */
    pfcom->dput = CCMA_dPut_T_radar_station; /* REAL(8) dbmgr */
    pfcom->load = CCMA_Load_T_radar_station;
    pfcom->store = CCMA_Store_T_radar_station;
    pfcom->pack = CCMA_Pack_T_radar_station;
    pfcom->unpack = CCMA_Unpack_T_radar_station;
    pfcom->sql = CCMA_Sql_T_radar_station;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
