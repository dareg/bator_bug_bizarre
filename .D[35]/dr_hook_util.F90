...
SUBROUTINE DR_HACK(CROUTINE,KSTART,KDRHACKUNIT)
...
INTEGER(KIND=JPIM),INTENT(IN) :: KDRHACKUNIT

INTEGER(KIND=JPIM) :: I
CHARACTER(LEN(CROUTINE)) :: CROUTINE_CLEAN
TYPE TDRHACK
  CHARACTER(LEN=120) :: CNAME
  TYPE(TDRHACK), POINTER :: PREV => NULL()
END TYPE TDRHACK
TYPE(TDRHACK), POINTER, SAVE :: STACK => NULL()
TYPE(TDRHACK), POINTER :: NODE => NULL()

! replace some special character
DO I = 1,LEN(CROUTINE)
 SELECT CASE (CROUTINE(I:I))
  CASE ("<")
   CROUTINE_CLEAN (I:I)="_"
  CASE (">")
   CROUTINE_CLEAN (I:I)="_"
  CASE (" ")
   CROUTINE_CLEAN (I:I)="_"
  CASE DEFAULT
   CROUTINE_CLEAN (I:I)=CROUTINE(I:I)
 END SELECT
END DO
...
IF (KSTART==0) THEN
 ALLOCATE(NODE)
 NODE%CNAME = TRIM(CROUTINE_CLEAN)
 NODE%PREV => STACK
 STACK => NODE
 WRITE(KDRHACKUNIT,*) '<',CROUTINE_CLEAN,'>'
ELSE
 IF (ASSOCIATED(STACK)) THEN
   WRITE(KDRHACKUNIT,*) '</',TRIM(ADJUSTL(STACK%CNAME)),'>'
   NODE => STACK
   STACK => NODE%PREV
   DEALLOCATE(NODE)
 ENDIF
 !CLOSE FILE IF LAST ROUTINE
 IF (CROUTINE_CLEAN .eq. 'MODEL_MOD_MODEL_DELETE') THEN
   CLOSE (KDRHACKUNIT)
 ENDIF
ENDIF
