...
INTEGER(KIND=JPIB)              :: msgoff    ! use JPIB to avoid integer overflow
INTEGER(KIND=JPIM)              :: nsent, nfileno
...
REAL(KIND=JPRD), allocatable    :: data_bytes(:) ! nactive_tables
REAL(KIND=JPRD)                 :: datavolume(0:ODBMP_nproc), waltim(2)
...
REAL(KIND=JPRD)                 :: timef,ttab,ttloop
REAL(KIND=JPRD)                 :: tprebar,tpostbar
REAL(KIND=JPRD), allocatable    :: ttwrite(:),ttsend(:),ttrecv(:),ttwait(:),ttclose(:)
REAL(KIND=JPRD), allocatable    :: tbar(:), ttable(:), ttcomm(:)
REAL(KIND=JPRD), allocatable    :: sendmb(:),recvmb(:)
...
do jt=1,ntables
  ...
  if (LL_include_tbl(jt).and.LL_write_tbl(jt).and.(maxval(istat(ISTAT_NBYTES,1:npools,jtact))>0).and. &
      maxval(istat(ISTAT_UPDATED,1:npools,jtact))>0)then  ! If table updated, has non-zero data and is write enabled 
    do j=1,npools
...
do jt=1,ntables
  ...
  if (LL_include_tbl(jt).and.LL_write_tbl(jt).and.(maxval(istat(ISTAT_NBYTES,1:npools,jtact))>0).and. &
    ...
    do j=1,npools
      destPE = fast_physproc(iope_map(j,jtact))
      if (destPE/=ODBMP_myproc)then  ! Only get data and send message if iope is not this pe
                                     ! (otherwise, get data in the write loop directly)
          if (LL_mypool(j)) then
            nbytes = istat(ISTAT_NBYTES,j,jtact)
            if (nbytes > 0) then
              nw = NHDR + (istat(ISTAT_NBYTES,j,jtact) + sizeof_int - 1)/sizeof_int
  ...
  if (LL_include_tbl(jt).and.LL_write_tbl(jt).and.(maxval(istat(ISTAT_NBYTES,1:npools,jtact))>0).and. &
    ...
    do j=1,npools
      if (fast_physproc(iope_map(j,jtact))==ODBMP_myproc) then
        ...
        if (((io /= -1).and.(mod(j-global_pool_offset(j), cur_io_grpsize) == 0)).or. &
           ((io /= -1).and.((fast_physproc(iope_map(j,jtact))==ODBMP_myproc).and.(fast_physproc(iope_map(j+1,jtact))/=ODBMP_myproc))))then
