...
INTEGER(KIND=JPIB)              :: msgoff  ! use JPIB to avoid integer overflow with large data volumes
INTEGER(KIND=JPIM)              :: nsent
          ...
          if (ioaid(j,jt,IOAID_LENGTH)>0) then
            ...
            !if (destPE /= ODBMP_myproc)then ! send message to owner pe, only if this pe is not the owner
              ...
              if (fast_physproc(iope_map(j,jtact))==ODBMP_myproc) then ! If this task is an iope for this pool/table
                ...
                msgoff = msgoff + nw ! Add up total volume of data that needs to be sent by this
                                     ! PE as required for jncore allocation
