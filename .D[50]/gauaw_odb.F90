...
REAL(KIND=JPRD), intent(out)    :: PA(K),PW(K)
INTEGER(KIND=JPIM), intent(out) :: KRET
! === END OF INTERFACE BLOCK ===

REAL(KIND=JPRD) :: ZEPS = 1E-14_JPRD
REAL(KIND=JPRD) :: ZPI, ZC, ZXZ, ZKM2, ZKM1, ZFN, ZPK, ZKMRK, ZSP, ZVSP
...
ZC=(1.0_JPRD-(2.0_JPRD/ZPI)**2)*0.25_JPRD
IFK=K
IKK=K/2
...
DO JS=1,IKK
  ZXZ=COS(PA(JS)/SQRT((IFK+0.5_JPRD)**2+ZC))
...
!*         2.2     COMPUTATION OF THE *LEGENDRE POLYNOMIAL.
  ...
  DO JN=2,K
    ZFN=JN
    ZPK=((2.0_JPRD*ZFN-1.0_JPRD)*ZXZ*ZKM1-(ZFN-1.0_JPRD)*ZKM2)/ZFN
    ZKM2=ZKM1
    ZKM1=ZPK
  ENDDO

  ZKM1=ZKM2
  ZKMRK=(IFK*(ZKM1-ZXZ*ZPK))/(1.0_JPRD-ZXZ**2)
  ZSP=ZPK/ZKMRK
  ZXZ=ZXZ-ZSP
  ZVSP=ABS(ZSP)
...
!*         2.3     ABSCISSAS AND WEIGHTS.
  ...
  PA(JS)=ZXZ
  PW(JS)=(2.0_JPRD*(1.0_JPRD-ZXZ**2))/(IFK*ZKM1)**2
