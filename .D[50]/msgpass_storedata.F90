...
INTEGER(KIND=JPIB)              :: MJNCORE = 20000000
INTEGER(KIND=JPIB)              :: njmax
INTEGER(KIND=JPIM)              :: msgoff, msgsnd, j1, j2, jreset
            ...
            if(j>j1) then
              ...
              if(msgsnd .gt. 0) then
                if(JWRITE>0) t0=timef()
                  CALL mpl_wait(KREQUEST=JSENDREQ(1:msgsnd),CDSTRING="MSGPASS_STOREDATA Wait")
...
!J          if (iwrite_empty /= 2 .and. imp_type == JP_NON_BLOCKING_STANDARD) CALL mpl_wait(KREQUEST=ISENDREQ)
