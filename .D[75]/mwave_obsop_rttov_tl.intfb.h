INTERFACE
    SUBROUTINE mwave_obsop_rttov_tl (kflevg, kproma, kchannels, & ! in: levels, profiles, max/ #channels        
      ...
      &  phys_rain_tl , &  ! in: rain mmr (large scale), TL 
      &  phys_snow_tl , &  ! in: snow mmr, TL
      &  phys_crain_tl , &  ! in: rain mmr (convective), TL 
      &  phys_csnow_tl , &  ! in: snow mmr, TL
      ...
      &  prad_ee_fw  , &  ! in:
      &  prad_tb_fw  , &  ! out: TB's
      &  prad_tb_tl  , &  ! out: TB's, TL, allsky
      &  prad_tb_tl_clr)  ! out: TB's, TL, clear-sky
    ...
    real    (kind=jprb), intent   (out), dimension (kproma,kchannels) :: prad_tb_fw, prad_tb_tl, prad_tb_tl_clr 
