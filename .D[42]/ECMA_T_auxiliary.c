...
PUBLIC int
ECMA_Pack_T_auxiliary(void *T)
{
  int Nbytes = 0;
  TABLE_auxiliary *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk9real, report_aux_1); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk9real, report_aux_2); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk9real, report_aux_3); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, pk9real, report_aux_4); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, pk9real, report_aux_5); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, pk9real, report_aux_6); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, ECMA, pk9real, report_aux_7); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, ECMA, pk9real, report_aux_8); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, ECMA, pk9real, report_aux_9); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, ECMA, linkoffset_t, LINKOFFSET(auxiliary_body)); CHECK_PDS_ERROR(10);
    PDS = PackDS(P, ECMA, linklen_t, LINKLEN(auxiliary_body)); CHECK_PDS_ERROR(11);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_auxiliary(void *T)
{
  int Nbytes = 0;
  TABLE_auxiliary *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk9real, report_aux_1); Nbytes += BYTESIZE(P->report_aux_1.d);
    UseDS(P, ECMA, pk9real, report_aux_2); Nbytes += BYTESIZE(P->report_aux_2.d);
    UseDS(P, ECMA, pk9real, report_aux_3); Nbytes += BYTESIZE(P->report_aux_3.d);
    UseDS(P, ECMA, pk9real, report_aux_4); Nbytes += BYTESIZE(P->report_aux_4.d);
    UseDS(P, ECMA, pk9real, report_aux_5); Nbytes += BYTESIZE(P->report_aux_5.d);
    UseDS(P, ECMA, pk9real, report_aux_6); Nbytes += BYTESIZE(P->report_aux_6.d);
    UseDS(P, ECMA, pk9real, report_aux_7); Nbytes += BYTESIZE(P->report_aux_7.d);
    UseDS(P, ECMA, pk9real, report_aux_8); Nbytes += BYTESIZE(P->report_aux_8.d);
    UseDS(P, ECMA, pk9real, report_aux_9); Nbytes += BYTESIZE(P->report_aux_9.d);
    UseDS(P, ECMA, linkoffset_t, LINKOFFSET(auxiliary_body)); Nbytes += BYTESIZE(P->LINKOFFSET(auxiliary_body).d);
    UseDS(P, ECMA, linklen_t, LINKLEN(auxiliary_body)); Nbytes += BYTESIZE(P->LINKLEN(auxiliary_body).d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_auxiliary(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_auxiliary *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(auxiliary);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, auxiliary)
  Call_CopyGet_TABLE(ECMA, d, 1, auxiliary, pk9real, D, report_aux_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, auxiliary, pk9real, D, report_aux_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 3, auxiliary, pk9real, D, report_aux_3, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 4, auxiliary, pk9real, D, report_aux_4, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 5, auxiliary, pk9real, D, report_aux_5, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 6, auxiliary, pk9real, D, report_aux_6, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 7, auxiliary, pk9real, D, report_aux_7, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 8, auxiliary, pk9real, D, report_aux_8, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 9, auxiliary, pk9real, D, report_aux_9, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 10, auxiliary, linkoffset_t, D, LINKOFFSET(auxiliary_body), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(ECMA, d, 11, auxiliary, linklen_t, D, LINKLEN(auxiliary_body), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, auxiliary)


PrePutTable(ECMA, d, double, auxiliary)
  Call_CopyPut_TABLE(ECMA, d, 1, auxiliary, pk9real, report_aux_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, auxiliary, pk9real, report_aux_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 3, auxiliary, pk9real, report_aux_3, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 4, auxiliary, pk9real, report_aux_4, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 5, auxiliary, pk9real, report_aux_5, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 6, auxiliary, pk9real, report_aux_6, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 7, auxiliary, pk9real, report_aux_7, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 8, auxiliary, pk9real, report_aux_8, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 9, auxiliary, pk9real, report_aux_9, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 10, auxiliary, linkoffset_t, LINKOFFSET(auxiliary_body), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(ECMA, d, 11, auxiliary, linklen_t, LINKLEN(auxiliary_body), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, auxiliary)

PreLoadTable(ECMA, auxiliary);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_1);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_2);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_3);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_4);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_5);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_6);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_7);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_8);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_9);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(auxiliary_body));
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(auxiliary_body));
PostLoadTable(auxiliary)

PreStoreTable(ECMA, auxiliary);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_1);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_2);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_3);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_4);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_5);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_6);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_7);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_8);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, report_aux_9);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(auxiliary_body));
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(auxiliary_body));
PostStoreTable(auxiliary)

DefineLookupTable(auxiliary)

PUBLIC void
ECMA_Dim_T_auxiliary(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_auxiliary *P = T;
  Call_LookupTable(auxiliary, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_auxiliary(void *T)
{
  TABLE_auxiliary *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, report_aux_1, Nbytes, Count);
  FreeDS(P, report_aux_2, Nbytes, Count);
  FreeDS(P, report_aux_3, Nbytes, Count);
  FreeDS(P, report_aux_4, Nbytes, Count);
  FreeDS(P, report_aux_5, Nbytes, Count);
  FreeDS(P, report_aux_6, Nbytes, Count);
  FreeDS(P, report_aux_7, Nbytes, Count);
  FreeDS(P, report_aux_8, Nbytes, Count);
  FreeDS(P, report_aux_9, Nbytes, Count);
  FreeDS(P, LINKOFFSET(auxiliary_body), Nbytes, Count);
  FreeDS(P, LINKLEN(auxiliary_body), Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(auxiliary,11);
}

DefineRemoveTable(ECMA, auxiliary)

PUBLIC int
ECMA_Sql_T_auxiliary(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_auxiliary(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_auxiliary *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_auxiliary);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 11);
  InitDS(pk9real, DATATYPE_REAL8, report_aux_1, auxiliary, 9);
  InitDS(pk9real, DATATYPE_REAL8, report_aux_2, auxiliary, 9);
  InitDS(pk9real, DATATYPE_REAL8, report_aux_3, auxiliary, 9);
  InitDS(pk9real, DATATYPE_REAL8, report_aux_4, auxiliary, 9);
  InitDS(pk9real, DATATYPE_REAL8, report_aux_5, auxiliary, 9);
  InitDS(pk9real, DATATYPE_REAL8, report_aux_6, auxiliary, 9);
  InitDS(pk9real, DATATYPE_REAL8, report_aux_7, auxiliary, 9);
  InitDS(pk9real, DATATYPE_REAL8, report_aux_8, auxiliary, 9);
  InitDS(pk9real, DATATYPE_REAL8, report_aux_9, auxiliary, 9);
  InitDS(linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(auxiliary_body), auxiliary, 1);
  InitDS(linklen_t, DATATYPE_LINKLEN, LINKLEN(auxiliary_body), auxiliary, 1);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@auxiliary"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 12;
    pfcom->wt = 3.001004;
    pfcom->tags = ECMA_Set_T_auxiliary_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_auxiliary_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_auxiliary;
    pfcom->swapout = ECMA_Swapout_T_auxiliary;
    pfcom->dim = ECMA_Dim_T_auxiliary;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_auxiliary;
    pfcom->remove = ECMA_Remove_T_auxiliary;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_auxiliary; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_auxiliary; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_auxiliary;
    pfcom->store = ECMA_Store_T_auxiliary;
    pfcom->pack = ECMA_Pack_T_auxiliary;
    pfcom->unpack = ECMA_Unpack_T_auxiliary;
    pfcom->sql = ECMA_Sql_T_auxiliary;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
