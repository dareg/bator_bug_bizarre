...
PUBLIC int
ECMA_Pack_T_poolmask(void *T)
{
  int Nbytes = 0;
  TABLE_poolmask *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk1int, timeslot); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk1int, obstype); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk1int, codetype); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, pk1int, sensor); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, pk1int, bufrtype); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, pk1int, subtype); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, ECMA, pk1int, poolno); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, ECMA, pk1int, hdr_count); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, ECMA, pk1int, body_count); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, ECMA, pk1int, max_bodylen); CHECK_PDS_ERROR(10);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_poolmask(void *T)
{
  int Nbytes = 0;
  TABLE_poolmask *P = T;
  if (P->Is_loaded) {
    ...
    UseDS(P, ECMA, pk1int, poolno); Nbytes += BYTESIZE(P->poolno.d);
    UseDS(P, ECMA, pk1int, hdr_count); Nbytes += BYTESIZE(P->hdr_count.d);
    UseDS(P, ECMA, pk1int, body_count); Nbytes += BYTESIZE(P->body_count.d);
    UseDS(P, ECMA, pk1int, max_bodylen); Nbytes += BYTESIZE(P->max_bodylen.d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_poolmask(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_poolmask *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(poolmask);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, poolmask)
  Call_CopyGet_TABLE(ECMA, d, 1, poolmask, pk1int, D, timeslot, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 2, poolmask, pk1int, D, obstype, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 3, poolmask, pk1int, D, codetype, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 4, poolmask, pk1int, D, sensor, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 5, poolmask, pk1int, D, bufrtype, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 6, poolmask, pk1int, D, subtype, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 7, poolmask, pk1int, D, poolno, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 8, poolmask, pk1int, D, hdr_count, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 9, poolmask, pk1int, D, body_count, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 10, poolmask, pk1int, D, max_bodylen, Count, DATATYPE_INT4);
PostGetTable(d, double, poolmask)


PrePutTable(ECMA, d, double, poolmask)
  Call_CopyPut_TABLE(ECMA, d, 1, poolmask, pk1int, timeslot, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 2, poolmask, pk1int, obstype, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 3, poolmask, pk1int, codetype, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 4, poolmask, pk1int, sensor, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 5, poolmask, pk1int, bufrtype, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 6, poolmask, pk1int, subtype, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 7, poolmask, pk1int, poolno, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 8, poolmask, pk1int, hdr_count, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 9, poolmask, pk1int, body_count, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 10, poolmask, pk1int, max_bodylen, D, Count, DATATYPE_INT4);
PostPutTable(d, double, poolmask)

PreLoadTable(ECMA, poolmask);
  ...
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, poolno);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, hdr_count);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, body_count);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, max_bodylen);
PostLoadTable(poolmask)

PreStoreTable(ECMA, poolmask);
  ...
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, poolno);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, hdr_count);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, body_count);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, max_bodylen);
PostStoreTable(poolmask)

DefineLookupTable(poolmask)

PUBLIC void
ECMA_Dim_T_poolmask(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_poolmask *P = T;
  Call_LookupTable(poolmask, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_poolmask(void *T)
{
  TABLE_poolmask *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, poolno, Nbytes, Count);
  FreeDS(P, hdr_count, Nbytes, Count);
  FreeDS(P, body_count, Nbytes, Count);
  FreeDS(P, max_bodylen, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(poolmask,10);
}

DefineRemoveTable(ECMA, poolmask)

PUBLIC int
ECMA_Sql_T_poolmask(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_poolmask(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_poolmask *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_poolmask);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 10);
  InitDS(pk1int, DATATYPE_INT4, timeslot, poolmask, 1);
  InitDS(pk1int, DATATYPE_INT4, obstype, poolmask, 1);
  InitDS(pk1int, DATATYPE_INT4, codetype, poolmask, 1);
  InitDS(pk1int, DATATYPE_INT4, sensor, poolmask, 1);
  InitDS(pk1int, DATATYPE_INT4, bufrtype, poolmask, 1);
  InitDS(pk1int, DATATYPE_INT4, subtype, poolmask, 1);
  InitDS(pk1int, DATATYPE_INT4, poolno, poolmask, 1);
  InitDS(pk1int, DATATYPE_INT4, hdr_count, poolmask, 1);
  InitDS(pk1int, DATATYPE_INT4, body_count, poolmask, 1);
  InitDS(pk1int, DATATYPE_INT4, max_bodylen, poolmask, 1);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@poolmask"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 1;
    pfcom->wt = 1.000001;
    pfcom->tags = ECMA_Set_T_poolmask_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_poolmask_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_poolmask;
    pfcom->swapout = ECMA_Swapout_T_poolmask;
    pfcom->dim = ECMA_Dim_T_poolmask;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_poolmask;
    pfcom->remove = ECMA_Remove_T_poolmask;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_poolmask; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_poolmask; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_poolmask;
    pfcom->store = ECMA_Store_T_poolmask;
    pfcom->pack = ECMA_Pack_T_poolmask;
    pfcom->unpack = ECMA_Unpack_T_poolmask;
    pfcom->sql = ECMA_Sql_T_poolmask;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
