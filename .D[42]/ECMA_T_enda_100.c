...
PUBLIC int
ECMA_Sel_T_enda_100(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_enda_100 *P = T;
...
PreGetTable(ECMA, d, double, enda_100)
  Call_CopyGet_TABLE(ECMA, d, 1, enda_100, pk1int, D, member, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 2, enda_100, Bitfield, D, report_status, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(ECMA, d, 3, enda_100, Bitfield, D, report_event1, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(ECMA, d, 4, enda_100, pk9real, D, obsvalue, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 5, enda_100, Bitfield, D, datum_anflag, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(ECMA, d, 6, enda_100, Bitfield, D, datum_status, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(ECMA, d, 7, enda_100, Bitfield, D, datum_event1, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(ECMA, d, 8, enda_100, pk9real, D, biascorr, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 9, enda_100, pk9real, D, biascorr_fg, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 10, enda_100, pk9real, D, an_depar, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 11, enda_100, pk9real, D, fg_depar, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 12, enda_100, pk9real, D, qc_pge, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 13, enda_100, pk9real, D, final_obs_error, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 14, enda_100, pk9real, D, obs_error, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 15, enda_100, pk9real, D, fg_error, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 16, enda_100, pk9real, D, skintemp_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 17, enda_100, pk9real, D, skintemp_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 18, enda_100, pk9real, D, skintemp_3, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 19, enda_100, pk9real, D, skintemp_4, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 20, enda_100, pk9real, D, skintemp_5, Count, DATATYPE_REAL8);
PostGetTable(d, double, enda_100)


PrePutTable(ECMA, d, double, enda_100)
  Call_CopyPut_TABLE(ECMA, d, 1, enda_100, pk1int, member, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 2, enda_100, Bitfield, report_status, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(ECMA, d, 3, enda_100, Bitfield, report_event1, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(ECMA, d, 4, enda_100, pk9real, obsvalue, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 5, enda_100, Bitfield, datum_anflag, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(ECMA, d, 6, enda_100, Bitfield, datum_status, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(ECMA, d, 7, enda_100, Bitfield, datum_event1, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(ECMA, d, 8, enda_100, pk9real, biascorr, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 9, enda_100, pk9real, biascorr_fg, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 10, enda_100, pk9real, an_depar, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 11, enda_100, pk9real, fg_depar, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 12, enda_100, pk9real, qc_pge, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 13, enda_100, pk9real, final_obs_error, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 14, enda_100, pk9real, obs_error, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 15, enda_100, pk9real, fg_error, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 16, enda_100, pk9real, skintemp_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 17, enda_100, pk9real, skintemp_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 18, enda_100, pk9real, skintemp_3, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 19, enda_100, pk9real, skintemp_4, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 20, enda_100, pk9real, skintemp_5, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, enda_100)

PreLoadTable(ECMA, enda_100);
...
PUBLIC void
ECMA_Dim_T_enda_100(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
ECMA_Swapout_T_enda_100(void *T)
{
  TABLE_enda_100 *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(ECMA, enda_100)

PUBLIC int
ECMA_Sql_T_enda_100(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_enda_100(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_enda_100 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_enda_100);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 20);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 249;
    pfcom->wt = 358.000225;
    pfcom->tags = ECMA_Set_T_enda_100_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_enda_100_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_enda_100;
    pfcom->swapout = ECMA_Swapout_T_enda_100;
    pfcom->dim = ECMA_Dim_T_enda_100;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_enda_100;
    pfcom->remove = ECMA_Remove_T_enda_100;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_enda_100; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_enda_100; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_enda_100;
    pfcom->store = ECMA_Store_T_enda_100;
    pfcom->pack = ECMA_Pack_T_enda_100;
    pfcom->unpack = ECMA_Unpack_T_enda_100;
    pfcom->sql = ECMA_Sql_T_enda_100;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
