#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_aeolus_hdr 1
#include "ECMA.h"
...
PUBLIC int
ECMA_Pack_T_aeolus_hdr(void *T)
{
  int Nbytes = 0;
  TABLE_aeolus_hdr *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, Bitfield, aeolus_hdrflag); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, linkoffset_t, LINKOFFSET(aeolus_auxmet)); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, linklen_t, LINKLEN(aeolus_auxmet)); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, linkoffset_t, LINKOFFSET(aeolus_l2c)); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, linklen_t, LINKLEN(aeolus_l2c)); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, linkoffset_t, LINKOFFSET(aeolus_l2b)); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, ECMA, linklen_t, LINKLEN(aeolus_l2b)); CHECK_PDS_ERROR(7);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_aeolus_hdr(void *T)
{
  int Nbytes = 0;
  TABLE_aeolus_hdr *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, Bitfield, aeolus_hdrflag); Nbytes += BYTESIZE(P->aeolus_hdrflag.d);
    UseDS(P, ECMA, linkoffset_t, LINKOFFSET(aeolus_auxmet)); Nbytes += BYTESIZE(P->LINKOFFSET(aeolus_auxmet).d);
    UseDS(P, ECMA, linklen_t, LINKLEN(aeolus_auxmet)); Nbytes += BYTESIZE(P->LINKLEN(aeolus_auxmet).d);
    UseDS(P, ECMA, linkoffset_t, LINKOFFSET(aeolus_l2c)); Nbytes += BYTESIZE(P->LINKOFFSET(aeolus_l2c).d);
    UseDS(P, ECMA, linklen_t, LINKLEN(aeolus_l2c)); Nbytes += BYTESIZE(P->LINKLEN(aeolus_l2c).d);
    UseDS(P, ECMA, linkoffset_t, LINKOFFSET(aeolus_l2b)); Nbytes += BYTESIZE(P->LINKOFFSET(aeolus_l2b).d);
    UseDS(P, ECMA, linklen_t, LINKLEN(aeolus_l2b)); Nbytes += BYTESIZE(P->LINKLEN(aeolus_l2b).d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_aeolus_hdr(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_aeolus_hdr *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(aeolus_hdr);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, aeolus_hdr)
  Call_CopyGet_TABLE(ECMA, d, 1, aeolus_hdr, Bitfield, D, aeolus_hdrflag, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(ECMA, d, 2, aeolus_hdr, linkoffset_t, D, LINKOFFSET(aeolus_auxmet), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(ECMA, d, 3, aeolus_hdr, linklen_t, D, LINKLEN(aeolus_auxmet), Count, DATATYPE_LINKLEN);
  Call_CopyGet_TABLE(ECMA, d, 4, aeolus_hdr, linkoffset_t, D, LINKOFFSET(aeolus_l2c), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(ECMA, d, 5, aeolus_hdr, linklen_t, D, LINKLEN(aeolus_l2c), Count, DATATYPE_LINKLEN);
  Call_CopyGet_TABLE(ECMA, d, 6, aeolus_hdr, linkoffset_t, D, LINKOFFSET(aeolus_l2b), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(ECMA, d, 7, aeolus_hdr, linklen_t, D, LINKLEN(aeolus_l2b), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, aeolus_hdr)


PrePutTable(ECMA, d, double, aeolus_hdr)
  Call_CopyPut_TABLE(ECMA, d, 1, aeolus_hdr, Bitfield, aeolus_hdrflag, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(ECMA, d, 2, aeolus_hdr, linkoffset_t, LINKOFFSET(aeolus_auxmet), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(ECMA, d, 3, aeolus_hdr, linklen_t, LINKLEN(aeolus_auxmet), D, Count, DATATYPE_LINKLEN);
  Call_CopyPut_TABLE(ECMA, d, 4, aeolus_hdr, linkoffset_t, LINKOFFSET(aeolus_l2c), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(ECMA, d, 5, aeolus_hdr, linklen_t, LINKLEN(aeolus_l2c), D, Count, DATATYPE_LINKLEN);
  Call_CopyPut_TABLE(ECMA, d, 6, aeolus_hdr, linkoffset_t, LINKOFFSET(aeolus_l2b), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(ECMA, d, 7, aeolus_hdr, linklen_t, LINKLEN(aeolus_l2b), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, aeolus_hdr)

PreLoadTable(ECMA, aeolus_hdr);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, aeolus_hdrflag);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_auxmet));
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_auxmet));
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_l2c));
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_l2c));
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_l2b));
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_l2b));
PostLoadTable(aeolus_hdr)

PreStoreTable(ECMA, aeolus_hdr);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, aeolus_hdrflag);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_auxmet));
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_auxmet));
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_l2c));
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_l2c));
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_l2b));
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_l2b));
PostStoreTable(aeolus_hdr)

DefineLookupTable(aeolus_hdr)

PUBLIC void
ECMA_Dim_T_aeolus_hdr(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_aeolus_hdr *P = T;
  Call_LookupTable(aeolus_hdr, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_aeolus_hdr(void *T)
{
  TABLE_aeolus_hdr *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, aeolus_hdrflag, Nbytes, Count);
  FreeDS(P, LINKOFFSET(aeolus_auxmet), Nbytes, Count);
  FreeDS(P, LINKLEN(aeolus_auxmet), Nbytes, Count);
  FreeDS(P, LINKOFFSET(aeolus_l2c), Nbytes, Count);
  FreeDS(P, LINKLEN(aeolus_l2c), Nbytes, Count);
  FreeDS(P, LINKOFFSET(aeolus_l2b), Nbytes, Count);
  FreeDS(P, LINKLEN(aeolus_l2b), Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(aeolus_hdr,7);
}

DefineRemoveTable(ECMA, aeolus_hdr)

PUBLIC int
ECMA_Sql_T_aeolus_hdr(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_aeolus_hdr(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_aeolus_hdr *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_aeolus_hdr);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 7);
  InitDS(Bitfield, DATATYPE_BITFIELD, aeolus_hdrflag, aeolus_hdr, 1);
  InitDS(linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_auxmet), aeolus_hdr, 1);
  InitDS(linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_auxmet), aeolus_hdr, 1);
  InitDS(linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_l2c), aeolus_hdr, 1);
  InitDS(linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_l2c), aeolus_hdr, 1);
  InitDS(linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(aeolus_l2b), aeolus_hdr, 1);
  InitDS(linklen_t, DATATYPE_LINKLEN, LINKLEN(aeolus_l2b), aeolus_hdr, 1);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@aeolus_hdr"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 23;
    pfcom->wt = 3.002004;
    pfcom->tags = ECMA_Set_T_aeolus_hdr_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_aeolus_hdr_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_aeolus_hdr;
    pfcom->swapout = ECMA_Swapout_T_aeolus_hdr;
    pfcom->dim = ECMA_Dim_T_aeolus_hdr;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_aeolus_hdr;
    pfcom->remove = ECMA_Remove_T_aeolus_hdr;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_aeolus_hdr; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_aeolus_hdr; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_aeolus_hdr;
    pfcom->store = ECMA_Store_T_aeolus_hdr;
    pfcom->pack = ECMA_Pack_T_aeolus_hdr;
    pfcom->unpack = ECMA_Unpack_T_aeolus_hdr;
    pfcom->sql = ECMA_Sql_T_aeolus_hdr;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
