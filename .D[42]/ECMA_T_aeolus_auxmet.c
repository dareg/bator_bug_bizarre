#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_aeolus_auxmet 1
#include "ECMA.h"
...
PUBLIC int
ECMA_Pack_T_aeolus_auxmet(void *T)
{
  int Nbytes = 0;
  TABLE_aeolus_auxmet *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk9real, lev); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk9real, ptop); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk9real, pnom); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, pk9real, ztop); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, pk9real, znom); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, pk9real, u); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, ECMA, pk9real, v); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, ECMA, pk9real, t); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, ECMA, pk9real, rh); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, ECMA, pk9real, q); CHECK_PDS_ERROR(10);
    PDS = PackDS(P, ECMA, pk9real, cc); CHECK_PDS_ERROR(11);
    PDS = PackDS(P, ECMA, pk9real, clwc); CHECK_PDS_ERROR(12);
    PDS = PackDS(P, ECMA, pk9real, ciwc); CHECK_PDS_ERROR(13);
    PDS = PackDS(P, ECMA, pk9real, error_t); CHECK_PDS_ERROR(14);
    PDS = PackDS(P, ECMA, pk9real, error_rh); CHECK_PDS_ERROR(15);
    PDS = PackDS(P, ECMA, pk9real, error_p); CHECK_PDS_ERROR(16);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_aeolus_auxmet(void *T)
{
  int Nbytes = 0;
  TABLE_aeolus_auxmet *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk9real, lev); Nbytes += BYTESIZE(P->lev.d);
    UseDS(P, ECMA, pk9real, ptop); Nbytes += BYTESIZE(P->ptop.d);
    UseDS(P, ECMA, pk9real, pnom); Nbytes += BYTESIZE(P->pnom.d);
    UseDS(P, ECMA, pk9real, ztop); Nbytes += BYTESIZE(P->ztop.d);
    UseDS(P, ECMA, pk9real, znom); Nbytes += BYTESIZE(P->znom.d);
    UseDS(P, ECMA, pk9real, u); Nbytes += BYTESIZE(P->u.d);
    UseDS(P, ECMA, pk9real, v); Nbytes += BYTESIZE(P->v.d);
    UseDS(P, ECMA, pk9real, t); Nbytes += BYTESIZE(P->t.d);
    UseDS(P, ECMA, pk9real, rh); Nbytes += BYTESIZE(P->rh.d);
    UseDS(P, ECMA, pk9real, q); Nbytes += BYTESIZE(P->q.d);
    UseDS(P, ECMA, pk9real, cc); Nbytes += BYTESIZE(P->cc.d);
    UseDS(P, ECMA, pk9real, clwc); Nbytes += BYTESIZE(P->clwc.d);
    UseDS(P, ECMA, pk9real, ciwc); Nbytes += BYTESIZE(P->ciwc.d);
    UseDS(P, ECMA, pk9real, error_t); Nbytes += BYTESIZE(P->error_t.d);
    UseDS(P, ECMA, pk9real, error_rh); Nbytes += BYTESIZE(P->error_rh.d);
    UseDS(P, ECMA, pk9real, error_p); Nbytes += BYTESIZE(P->error_p.d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_aeolus_auxmet(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_aeolus_auxmet *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(aeolus_auxmet);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, aeolus_auxmet)
  Call_CopyGet_TABLE(ECMA, d, 1, aeolus_auxmet, pk9real, D, lev, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, aeolus_auxmet, pk9real, D, ptop, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 3, aeolus_auxmet, pk9real, D, pnom, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 4, aeolus_auxmet, pk9real, D, ztop, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 5, aeolus_auxmet, pk9real, D, znom, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 6, aeolus_auxmet, pk9real, D, u, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 7, aeolus_auxmet, pk9real, D, v, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 8, aeolus_auxmet, pk9real, D, t, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 9, aeolus_auxmet, pk9real, D, rh, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 10, aeolus_auxmet, pk9real, D, q, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 11, aeolus_auxmet, pk9real, D, cc, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 12, aeolus_auxmet, pk9real, D, clwc, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 13, aeolus_auxmet, pk9real, D, ciwc, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 14, aeolus_auxmet, pk9real, D, error_t, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 15, aeolus_auxmet, pk9real, D, error_rh, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 16, aeolus_auxmet, pk9real, D, error_p, Count, DATATYPE_REAL8);
PostGetTable(d, double, aeolus_auxmet)


PrePutTable(ECMA, d, double, aeolus_auxmet)
  Call_CopyPut_TABLE(ECMA, d, 1, aeolus_auxmet, pk9real, lev, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, aeolus_auxmet, pk9real, ptop, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 3, aeolus_auxmet, pk9real, pnom, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 4, aeolus_auxmet, pk9real, ztop, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 5, aeolus_auxmet, pk9real, znom, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 6, aeolus_auxmet, pk9real, u, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 7, aeolus_auxmet, pk9real, v, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 8, aeolus_auxmet, pk9real, t, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 9, aeolus_auxmet, pk9real, rh, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 10, aeolus_auxmet, pk9real, q, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 11, aeolus_auxmet, pk9real, cc, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 12, aeolus_auxmet, pk9real, clwc, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 13, aeolus_auxmet, pk9real, ciwc, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 14, aeolus_auxmet, pk9real, error_t, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 15, aeolus_auxmet, pk9real, error_rh, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 16, aeolus_auxmet, pk9real, error_p, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, aeolus_auxmet)

PreLoadTable(ECMA, aeolus_auxmet);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, lev);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ptop);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, pnom);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ztop);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, znom);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, u);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, v);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, t);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rh);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, q);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, cc);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, clwc);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ciwc);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, error_t);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, error_rh);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, error_p);
PostLoadTable(aeolus_auxmet)

PreStoreTable(ECMA, aeolus_auxmet);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, lev);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ptop);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, pnom);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ztop);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, znom);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, u);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, v);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, t);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rh);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, q);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, cc);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, clwc);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, ciwc);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, error_t);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, error_rh);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, error_p);
PostStoreTable(aeolus_auxmet)

DefineLookupTable(aeolus_auxmet)

PUBLIC void
ECMA_Dim_T_aeolus_auxmet(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_aeolus_auxmet *P = T;
  Call_LookupTable(aeolus_auxmet, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_aeolus_auxmet(void *T)
{
  TABLE_aeolus_auxmet *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, lev, Nbytes, Count);
  FreeDS(P, ptop, Nbytes, Count);
  FreeDS(P, pnom, Nbytes, Count);
  FreeDS(P, ztop, Nbytes, Count);
  FreeDS(P, znom, Nbytes, Count);
  FreeDS(P, u, Nbytes, Count);
  FreeDS(P, v, Nbytes, Count);
  FreeDS(P, t, Nbytes, Count);
  FreeDS(P, rh, Nbytes, Count);
  FreeDS(P, q, Nbytes, Count);
  FreeDS(P, cc, Nbytes, Count);
  FreeDS(P, clwc, Nbytes, Count);
  FreeDS(P, ciwc, Nbytes, Count);
  FreeDS(P, error_t, Nbytes, Count);
  FreeDS(P, error_rh, Nbytes, Count);
  FreeDS(P, error_p, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(aeolus_auxmet,16);
}

DefineRemoveTable(ECMA, aeolus_auxmet)

PUBLIC int
ECMA_Sql_T_aeolus_auxmet(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_aeolus_auxmet(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_aeolus_auxmet *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_aeolus_auxmet);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 16);
  InitDS(pk9real, DATATYPE_REAL8, lev, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, ptop, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, pnom, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, ztop, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, znom, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, u, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, v, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, t, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, rh, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, q, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, cc, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, clwc, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, ciwc, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, error_t, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, error_rh, aeolus_auxmet, 9);
  InitDS(pk9real, DATATYPE_REAL8, error_p, aeolus_auxmet, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@aeolus_auxmet"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 27;
    pfcom->wt = 3.003004;
    pfcom->tags = ECMA_Set_T_aeolus_auxmet_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_aeolus_auxmet_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_aeolus_auxmet;
    pfcom->swapout = ECMA_Swapout_T_aeolus_auxmet;
    pfcom->dim = ECMA_Dim_T_aeolus_auxmet;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_aeolus_auxmet;
    pfcom->remove = ECMA_Remove_T_aeolus_auxmet;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_aeolus_auxmet; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_aeolus_auxmet; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_aeolus_auxmet;
    pfcom->store = ECMA_Store_T_aeolus_auxmet;
    pfcom->pack = ECMA_Pack_T_aeolus_auxmet;
    pfcom->unpack = ECMA_Unpack_T_aeolus_auxmet;
    pfcom->sql = ECMA_Sql_T_aeolus_auxmet;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
