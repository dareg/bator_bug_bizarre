#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_gbrad_body 1
#include "ECMA.h"
...
PUBLIC int
ECMA_Pack_T_gbrad_body(void *T)
{
  int Nbytes = 0;
  TABLE_gbrad_body *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
...
PUBLIC int
ECMA_Unpack_T_gbrad_body(void *T)
{
  int Nbytes = 0;
  TABLE_gbrad_body *P = T;
  if (P->Is_loaded) {
...
PUBLIC int
ECMA_Sel_T_gbrad_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_gbrad_body *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(gbrad_body);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, gbrad_body)
  Call_CopyGet_TABLE(ECMA, d, 1, gbrad_body, pk9real, D, rrvalue, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, gbrad_body, pk9real, D, rrvaluetl, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 3, gbrad_body, pk9real, D, rrvaluead, Count, DATATYPE_REAL8);
PostGetTable(d, double, gbrad_body)


PrePutTable(ECMA, d, double, gbrad_body)
  Call_CopyPut_TABLE(ECMA, d, 1, gbrad_body, pk9real, rrvalue, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, gbrad_body, pk9real, rrvaluetl, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 3, gbrad_body, pk9real, rrvaluead, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, gbrad_body)

PreLoadTable(ECMA, gbrad_body);
...
PostLoadTable(gbrad_body)

PreStoreTable(ECMA, gbrad_body);
...
PostStoreTable(gbrad_body)

DefineLookupTable(gbrad_body)

PUBLIC void
ECMA_Dim_T_gbrad_body(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_gbrad_body *P = T;
  Call_LookupTable(gbrad_body, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_gbrad_body(void *T)
{
  TABLE_gbrad_body *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  ODBMAC_TRACE_SWAPOUT(gbrad_body,3);
}

DefineRemoveTable(ECMA, gbrad_body)

PUBLIC int
ECMA_Sql_T_gbrad_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_gbrad_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_gbrad_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_gbrad_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 3);
  InitDS(pk9real, DATATYPE_REAL8, rrvalue, gbrad_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, rrvaluetl, gbrad_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, rrvaluead, gbrad_body, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@gbrad_body"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 370;
    pfcom->wt = 358.000348;
    pfcom->tags = ECMA_Set_T_gbrad_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_gbrad_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_gbrad_body;
    pfcom->swapout = ECMA_Swapout_T_gbrad_body;
    pfcom->dim = ECMA_Dim_T_gbrad_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_gbrad_body;
    pfcom->remove = ECMA_Remove_T_gbrad_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_gbrad_body; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_gbrad_body; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_gbrad_body;
    pfcom->store = ECMA_Store_T_gbrad_body;
    pfcom->pack = ECMA_Pack_T_gbrad_body;
    pfcom->unpack = ECMA_Unpack_T_gbrad_body;
    pfcom->sql = ECMA_Sql_T_gbrad_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
