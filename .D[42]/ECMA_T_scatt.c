...
PUBLIC int
ECMA_Pack_T_scatt(void *T)
{
  int Nbytes = 0;
  TABLE_scatt *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk1int, cellno); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk1int, nretr_amb); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk1int, prodflag); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, pk1int, wvc_qf); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, linkoffset_t, LINKOFFSET(scatt_body)); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, linklen_t, LINKLEN(scatt_body)); CHECK_PDS_ERROR(6);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_scatt(void *T)
{
  int Nbytes = 0;
  TABLE_scatt *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk1int, cellno); Nbytes += BYTESIZE(P->cellno.d);
    UseDS(P, ECMA, pk1int, nretr_amb); Nbytes += BYTESIZE(P->nretr_amb.d);
    UseDS(P, ECMA, pk1int, prodflag); Nbytes += BYTESIZE(P->prodflag.d);
    UseDS(P, ECMA, pk1int, wvc_qf); Nbytes += BYTESIZE(P->wvc_qf.d);
    UseDS(P, ECMA, linkoffset_t, LINKOFFSET(scatt_body)); Nbytes += BYTESIZE(P->LINKOFFSET(scatt_body).d);
    UseDS(P, ECMA, linklen_t, LINKLEN(scatt_body)); Nbytes += BYTESIZE(P->LINKLEN(scatt_body).d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_scatt(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_scatt *P = T;
...
PreGetTable(ECMA, d, double, scatt)
  Call_CopyGet_TABLE(ECMA, d, 1, scatt, pk1int, D, cellno, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 2, scatt, pk1int, D, nretr_amb, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 3, scatt, pk1int, D, prodflag, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 4, scatt, pk1int, D, wvc_qf, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 5, scatt, linkoffset_t, D, LINKOFFSET(scatt_body), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(ECMA, d, 6, scatt, linklen_t, D, LINKLEN(scatt_body), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, scatt)


PrePutTable(ECMA, d, double, scatt)
  Call_CopyPut_TABLE(ECMA, d, 1, scatt, pk1int, cellno, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 2, scatt, pk1int, nretr_amb, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 3, scatt, pk1int, prodflag, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 4, scatt, pk1int, wvc_qf, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 5, scatt, linkoffset_t, LINKOFFSET(scatt_body), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(ECMA, d, 6, scatt, linklen_t, LINKLEN(scatt_body), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, scatt)

PreLoadTable(ECMA, scatt);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, cellno);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, nretr_amb);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, prodflag);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, wvc_qf);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(scatt_body));
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(scatt_body));
PostLoadTable(scatt)

PreStoreTable(ECMA, scatt);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, cellno);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, nretr_amb);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, prodflag);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, wvc_qf);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(scatt_body));
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(scatt_body));
PostStoreTable(scatt)

DefineLookupTable(scatt)

PUBLIC void
ECMA_Dim_T_scatt(void *T, int *Nrows, int *Ncols,
...
ECMA_Swapout_T_scatt(void *T)
{
  ...
  FreeDS(P, nretr_amb, Nbytes, Count);
  FreeDS(P, prodflag, Nbytes, Count);
  FreeDS(P, wvc_qf, Nbytes, Count);
  FreeDS(P, LINKOFFSET(scatt_body), Nbytes, Count);
  FreeDS(P, LINKLEN(scatt_body), Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(scatt,6);
}

DefineRemoveTable(ECMA, scatt)

PUBLIC int
ECMA_Sql_T_scatt(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_scatt(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_scatt *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_scatt);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 6);
  InitDS(pk1int, DATATYPE_INT4, cellno, scatt, 1);
  InitDS(pk1int, DATATYPE_INT4, nretr_amb, scatt, 1);
  InitDS(pk1int, DATATYPE_INT4, prodflag, scatt, 1);
  InitDS(pk1int, DATATYPE_INT4, wvc_qf, scatt, 1);
  InitDS(linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(scatt_body), scatt, 1);
  InitDS(linklen_t, DATATYPE_LINKLEN, LINKLEN(scatt_body), scatt, 1);
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 17;
    pfcom->wt = 3.002004;
    pfcom->tags = ECMA_Set_T_scatt_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_scatt_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_scatt;
    pfcom->swapout = ECMA_Swapout_T_scatt;
    pfcom->dim = ECMA_Dim_T_scatt;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_scatt;
    pfcom->remove = ECMA_Remove_T_scatt;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_scatt; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_scatt; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_scatt;
    pfcom->store = ECMA_Store_T_scatt;
    pfcom->pack = ECMA_Pack_T_scatt;
    pfcom->unpack = ECMA_Unpack_T_scatt;
    pfcom->sql = ECMA_Sql_T_scatt;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
