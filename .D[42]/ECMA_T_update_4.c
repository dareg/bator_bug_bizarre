...
PUBLIC int
ECMA_Sel_T_update_4(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_update_4 *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(update_4);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, update_4)
  Call_CopyGet_TABLE(ECMA, d, 1, update_4, pk9real, D, hires, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, update_4, pk9real, D, lores, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 3, update_4, pk1int, D, datum_tbflag_hires, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 4, update_4, Bitfield, D, datum_status_hires, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(ECMA, d, 5, update_4, pk9real, D, obs_diags_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 6, update_4, pk9real, D, obs_diags_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 7, update_4, pk9real, D, obs_diags_3, Count, DATATYPE_REAL8);
PostGetTable(d, double, update_4)


PrePutTable(ECMA, d, double, update_4)
  Call_CopyPut_TABLE(ECMA, d, 1, update_4, pk9real, hires, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, update_4, pk9real, lores, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 3, update_4, pk1int, datum_tbflag_hires, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 4, update_4, Bitfield, datum_status_hires, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(ECMA, d, 5, update_4, pk9real, obs_diags_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 6, update_4, pk9real, obs_diags_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 7, update_4, pk9real, obs_diags_3, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, update_4)

PreLoadTable(ECMA, update_4);
...
PostLoadTable(update_4)

PreStoreTable(ECMA, update_4);
...
PostStoreTable(update_4)

DefineLookupTable(update_4)

PUBLIC void
ECMA_Dim_T_update_4(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_update_4 *P = T;
  Call_LookupTable(update_4, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_update_4(void *T)
{
  TABLE_update_4 *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  ODBMAC_TRACE_SWAPOUT(update_4,7);
}

DefineRemoveTable(ECMA, update_4)

PUBLIC int
ECMA_Sql_T_update_4(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_update_4(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_update_4 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_update_4);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 7);
  InitDS(pk9real, DATATYPE_REAL8, hires, update_4, 9);
  InitDS(pk9real, DATATYPE_REAL8, lores, update_4, 9);
  InitDS(pk1int, DATATYPE_INT4, datum_tbflag_hires, update_4, 1);
  InitDS(Bitfield, DATATYPE_BITFIELD, datum_status_hires, update_4, 1);
  InitDS(pk9real, DATATYPE_REAL8, obs_diags_1, update_4, 9);
  InitDS(pk9real, DATATYPE_REAL8, obs_diags_2, update_4, 9);
  InitDS(pk9real, DATATYPE_REAL8, obs_diags_3, update_4, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@update_4"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 387;
    pfcom->wt = 358.000383;
    pfcom->tags = ECMA_Set_T_update_4_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_update_4_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_update_4;
    pfcom->swapout = ECMA_Swapout_T_update_4;
    pfcom->dim = ECMA_Dim_T_update_4;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_update_4;
    pfcom->remove = ECMA_Remove_T_update_4;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_update_4; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_update_4; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_update_4;
    pfcom->store = ECMA_Store_T_update_4;
    pfcom->pack = ECMA_Pack_T_update_4;
    pfcom->unpack = ECMA_Unpack_T_update_4;
    pfcom->sql = ECMA_Sql_T_update_4;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
