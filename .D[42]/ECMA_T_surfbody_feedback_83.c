...
PUBLIC int
ECMA_Pack_T_surfbody_feedback_83(void *T)
{
  int Nbytes = 0;
...
PUBLIC int
ECMA_Unpack_T_surfbody_feedback_83(void *T)
{
  int Nbytes = 0;
  TABLE_surfbody_feedback_83 *P = T;
  if (P->Is_loaded) {
...
PUBLIC int
ECMA_Sel_T_surfbody_feedback_83(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_surfbody_feedback_83 *P = T;
...
PreGetTable(ECMA, d, double, surfbody_feedback_83)
  Call_CopyGet_TABLE(ECMA, d, 1, surfbody_feedback_83, Bitfield, D, datum_status, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(ECMA, d, 2, surfbody_feedback_83, Bitfield, D, datum_sfc_event, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(ECMA, d, 3, surfbody_feedback_83, pk9real, D, an_depar, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 4, surfbody_feedback_83, pk9real, D, fg_depar, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 5, surfbody_feedback_83, pk9real, D, snow_depth, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 6, surfbody_feedback_83, pk9real, D, snow_density, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 7, surfbody_feedback_83, pk9real, D, lsm, Count, DATATYPE_REAL8);
PostGetTable(d, double, surfbody_feedback_83)


PrePutTable(ECMA, d, double, surfbody_feedback_83)
  Call_CopyPut_TABLE(ECMA, d, 1, surfbody_feedback_83, Bitfield, datum_status, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(ECMA, d, 2, surfbody_feedback_83, Bitfield, datum_sfc_event, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(ECMA, d, 3, surfbody_feedback_83, pk9real, an_depar, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 4, surfbody_feedback_83, pk9real, fg_depar, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 5, surfbody_feedback_83, pk9real, snow_depth, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 6, surfbody_feedback_83, pk9real, snow_density, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 7, surfbody_feedback_83, pk9real, lsm, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, surfbody_feedback_83)

PreLoadTable(ECMA, surfbody_feedback_83);
...
PreStoreTable(ECMA, surfbody_feedback_83);
...
PUBLIC void
ECMA_Dim_T_surfbody_feedback_83(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
ECMA_Swapout_T_surfbody_feedback_83(void *T)
{
  TABLE_surfbody_feedback_83 *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(ECMA, surfbody_feedback_83)

PUBLIC int
ECMA_Sql_T_surfbody_feedback_83(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_surfbody_feedback_83(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_surfbody_feedback_83 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_surfbody_feedback_83);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 7);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 332;
    pfcom->wt = 358.000308;
    pfcom->tags = ECMA_Set_T_surfbody_feedback_83_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_surfbody_feedback_83_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_surfbody_feedback_83;
    pfcom->swapout = ECMA_Swapout_T_surfbody_feedback_83;
    pfcom->dim = ECMA_Dim_T_surfbody_feedback_83;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_surfbody_feedback_83;
    pfcom->remove = ECMA_Remove_T_surfbody_feedback_83;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_surfbody_feedback_83; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_surfbody_feedback_83; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_surfbody_feedback_83;
    pfcom->store = ECMA_Store_T_surfbody_feedback_83;
    pfcom->pack = ECMA_Pack_T_surfbody_feedback_83;
    pfcom->unpack = ECMA_Unpack_T_surfbody_feedback_83;
    pfcom->sql = ECMA_Sql_T_surfbody_feedback_83;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
