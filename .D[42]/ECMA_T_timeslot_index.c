#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_timeslot_index 1
#include "ECMA.h"
...
PUBLIC int
ECMA_Pack_T_timeslot_index(void *T)
{
  int Nbytes = 0;
  TABLE_timeslot_index *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk1int, timeslot); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk1int, model_timestep); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, yyyymmdd, enddate); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, hhmmss, endtime); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, linkoffset_t, LINKOFFSET(index)); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, linklen_t, LINKLEN(index)); CHECK_PDS_ERROR(6);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_timeslot_index(void *T)
{
  int Nbytes = 0;
  TABLE_timeslot_index *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk1int, timeslot); Nbytes += BYTESIZE(P->timeslot.d);
    UseDS(P, ECMA, pk1int, model_timestep); Nbytes += BYTESIZE(P->model_timestep.d);
    UseDS(P, ECMA, yyyymmdd, enddate); Nbytes += BYTESIZE(P->enddate.d);
    UseDS(P, ECMA, hhmmss, endtime); Nbytes += BYTESIZE(P->endtime.d);
    UseDS(P, ECMA, linkoffset_t, LINKOFFSET(index)); Nbytes += BYTESIZE(P->LINKOFFSET(index).d);
    UseDS(P, ECMA, linklen_t, LINKLEN(index)); Nbytes += BYTESIZE(P->LINKLEN(index).d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_timeslot_index(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_timeslot_index *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(timeslot_index);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, timeslot_index)
  Call_CopyGet_TABLE(ECMA, d, 1, timeslot_index, pk1int, D, timeslot, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 2, timeslot_index, pk1int, D, model_timestep, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 3, timeslot_index, yyyymmdd, D, enddate, Count, DATATYPE_YYYYMMDD);
  Call_CopyGet_TABLE(ECMA, d, 4, timeslot_index, hhmmss, D, endtime, Count, DATATYPE_HHMMSS);
  Call_CopyGet_TABLE(ECMA, d, 5, timeslot_index, linkoffset_t, D, LINKOFFSET(index), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(ECMA, d, 6, timeslot_index, linklen_t, D, LINKLEN(index), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, timeslot_index)


PrePutTable(ECMA, d, double, timeslot_index)
  Call_CopyPut_TABLE(ECMA, d, 1, timeslot_index, pk1int, timeslot, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 2, timeslot_index, pk1int, model_timestep, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 3, timeslot_index, yyyymmdd, enddate, D, Count, DATATYPE_YYYYMMDD);
  Call_CopyPut_TABLE(ECMA, d, 4, timeslot_index, hhmmss, endtime, D, Count, DATATYPE_HHMMSS);
  Call_CopyPut_TABLE(ECMA, d, 5, timeslot_index, linkoffset_t, LINKOFFSET(index), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(ECMA, d, 6, timeslot_index, linklen_t, LINKLEN(index), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, timeslot_index)

PreLoadTable(ECMA, timeslot_index);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, timeslot);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, model_timestep);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, yyyymmdd, DATATYPE_YYYYMMDD, enddate);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, hhmmss, DATATYPE_HHMMSS, endtime);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(index));
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(index));
PostLoadTable(timeslot_index)

PreStoreTable(ECMA, timeslot_index);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, timeslot);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, model_timestep);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, yyyymmdd, DATATYPE_YYYYMMDD, enddate);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, hhmmss, DATATYPE_HHMMSS, endtime);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(index));
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(index));
PostStoreTable(timeslot_index)

DefineLookupTable(timeslot_index)

PUBLIC void
ECMA_Dim_T_timeslot_index(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_timeslot_index *P = T;
  Call_LookupTable(timeslot_index, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_timeslot_index(void *T)
{
  TABLE_timeslot_index *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, model_timestep, Nbytes, Count);
  FreeDS(P, enddate, Nbytes, Count);
  FreeDS(P, endtime, Nbytes, Count);
  FreeDS(P, LINKOFFSET(index), Nbytes, Count);
  FreeDS(P, LINKLEN(index), Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(timeslot_index,6);
}

DefineRemoveTable(ECMA, timeslot_index)

PUBLIC int
ECMA_Sql_T_timeslot_index(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_timeslot_index(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_timeslot_index *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_timeslot_index);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 6);
  InitDS(pk1int, DATATYPE_INT4, timeslot, timeslot_index, 1);
  InitDS(pk1int, DATATYPE_INT4, model_timestep, timeslot_index, 1);
  InitDS(yyyymmdd, DATATYPE_YYYYMMDD, enddate, timeslot_index, 1);
  InitDS(hhmmss, DATATYPE_HHMMSS, endtime, timeslot_index, 1);
  InitDS(linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(index), timeslot_index, 1);
  InitDS(linklen_t, DATATYPE_LINKLEN, LINKLEN(index), timeslot_index, 1);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@timeslot_index"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 2;
    pfcom->wt = 1.000002;
    pfcom->tags = ECMA_Set_T_timeslot_index_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_timeslot_index_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_timeslot_index;
    pfcom->swapout = ECMA_Swapout_T_timeslot_index;
    pfcom->dim = ECMA_Dim_T_timeslot_index;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_timeslot_index;
    pfcom->remove = ECMA_Remove_T_timeslot_index;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_timeslot_index; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_timeslot_index; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_timeslot_index;
    pfcom->store = ECMA_Store_T_timeslot_index;
    pfcom->pack = ECMA_Pack_T_timeslot_index;
    pfcom->unpack = ECMA_Unpack_T_timeslot_index;
    pfcom->sql = ECMA_Sql_T_timeslot_index;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
