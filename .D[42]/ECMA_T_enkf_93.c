...
PUBLIC int
ECMA_Sel_T_enkf_93(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_enkf_93 *P = T;
...
PreGetTable(ECMA, d, double, enkf_93)
  Call_CopyGet_TABLE(ECMA, d, 1, enkf_93, pk1int, D, member, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 2, enkf_93, pk9real, D, hprior, Count, DATATYPE_REAL8);
PostGetTable(d, double, enkf_93)


PrePutTable(ECMA, d, double, enkf_93)
  Call_CopyPut_TABLE(ECMA, d, 1, enkf_93, pk1int, member, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 2, enkf_93, pk9real, hprior, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, enkf_93)

PreLoadTable(ECMA, enkf_93);
...
PUBLIC void
ECMA_Dim_T_enkf_93(void *T, int *Nrows, int *Ncols,
...
DefineRemoveTable(ECMA, enkf_93)

PUBLIC int
ECMA_Sql_T_enkf_93(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_enkf_93(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_enkf_93 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_enkf_93);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 2);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 122;
    pfcom->wt = 358.000098;
    pfcom->tags = ECMA_Set_T_enkf_93_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_enkf_93_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_enkf_93;
    pfcom->swapout = ECMA_Swapout_T_enkf_93;
    pfcom->dim = ECMA_Dim_T_enkf_93;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_enkf_93;
    pfcom->remove = ECMA_Remove_T_enkf_93;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_enkf_93; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_enkf_93; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_enkf_93;
    pfcom->store = ECMA_Store_T_enkf_93;
    pfcom->pack = ECMA_Pack_T_enkf_93;
    pfcom->unpack = ECMA_Unpack_T_enkf_93;
    pfcom->sql = ECMA_Sql_T_enkf_93;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
