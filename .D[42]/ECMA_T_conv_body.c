...
PUBLIC int
ECMA_Pack_T_conv_body(void *T)
{
  int Nbytes = 0;
  TABLE_conv_body *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk1int, ppcode); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, Bitfield, level); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk1int, datum_qcflag); CHECK_PDS_ERROR(3);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_conv_body(void *T)
{
  int Nbytes = 0;
  TABLE_conv_body *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk1int, ppcode); Nbytes += BYTESIZE(P->ppcode.d);
    UseDS(P, ECMA, Bitfield, level); Nbytes += BYTESIZE(P->level.d);
    UseDS(P, ECMA, pk1int, datum_qcflag); Nbytes += BYTESIZE(P->datum_qcflag.d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_conv_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_conv_body *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(conv_body);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, conv_body)
  Call_CopyGet_TABLE(ECMA, d, 1, conv_body, pk1int, D, ppcode, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 2, conv_body, Bitfield, D, level, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(ECMA, d, 3, conv_body, pk1int, D, datum_qcflag, Count, DATATYPE_INT4);
PostGetTable(d, double, conv_body)


PrePutTable(ECMA, d, double, conv_body)
  Call_CopyPut_TABLE(ECMA, d, 1, conv_body, pk1int, ppcode, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 2, conv_body, Bitfield, level, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(ECMA, d, 3, conv_body, pk1int, datum_qcflag, D, Count, DATATYPE_INT4);
PostPutTable(d, double, conv_body)

PreLoadTable(ECMA, conv_body);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, ppcode);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, level);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, datum_qcflag);
PostLoadTable(conv_body)

PreStoreTable(ECMA, conv_body);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, ppcode);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, level);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, datum_qcflag);
PostStoreTable(conv_body)

DefineLookupTable(conv_body)

PUBLIC void
ECMA_Dim_T_conv_body(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_conv_body *P = T;
  Call_LookupTable(conv_body, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_conv_body(void *T)
{
  TABLE_conv_body *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, ppcode, Nbytes, Count);
  FreeDS(P, level, Nbytes, Count);
  FreeDS(P, datum_qcflag, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(conv_body,3);
}

DefineRemoveTable(ECMA, conv_body)

PUBLIC int
ECMA_Sql_T_conv_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_conv_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_conv_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_conv_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 3);
  InitDS(pk1int, DATATYPE_INT4, ppcode, conv_body, 1);
  InitDS(Bitfield, DATATYPE_BITFIELD, level, conv_body, 1);
  InitDS(pk1int, DATATYPE_INT4, datum_qcflag, conv_body, 1);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@conv_body"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 372;
    pfcom->wt = 358.000352;
    pfcom->tags = ECMA_Set_T_conv_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_conv_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_conv_body;
    pfcom->swapout = ECMA_Swapout_T_conv_body;
    pfcom->dim = ECMA_Dim_T_conv_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_conv_body;
    pfcom->remove = ECMA_Remove_T_conv_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_conv_body; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_conv_body; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_conv_body;
    pfcom->store = ECMA_Store_T_conv_body;
    pfcom->pack = ECMA_Pack_T_conv_body;
    pfcom->unpack = ECMA_Unpack_T_conv_body;
    pfcom->sql = ECMA_Sql_T_conv_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
