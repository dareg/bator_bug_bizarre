#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_raingg_body 1
#include "ECMA.h"
...
PUBLIC int
ECMA_Pack_T_raingg_body(void *T)
{
  int Nbytes = 0;
  TABLE_raingg_body *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk9real, rrvalue); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk9real, rrvaluetl); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk9real, rrvaluead); CHECK_PDS_ERROR(3);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_raingg_body(void *T)
{
  int Nbytes = 0;
  TABLE_raingg_body *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk9real, rrvalue); Nbytes += BYTESIZE(P->rrvalue.d);
    UseDS(P, ECMA, pk9real, rrvaluetl); Nbytes += BYTESIZE(P->rrvaluetl.d);
    UseDS(P, ECMA, pk9real, rrvaluead); Nbytes += BYTESIZE(P->rrvaluead.d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_raingg_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_raingg_body *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(raingg_body);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, raingg_body)
  Call_CopyGet_TABLE(ECMA, d, 1, raingg_body, pk9real, D, rrvalue, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, raingg_body, pk9real, D, rrvaluetl, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 3, raingg_body, pk9real, D, rrvaluead, Count, DATATYPE_REAL8);
PostGetTable(d, double, raingg_body)


PrePutTable(ECMA, d, double, raingg_body)
  Call_CopyPut_TABLE(ECMA, d, 1, raingg_body, pk9real, rrvalue, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, raingg_body, pk9real, rrvaluetl, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 3, raingg_body, pk9real, rrvaluead, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, raingg_body)

PreLoadTable(ECMA, raingg_body);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rrvalue);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rrvaluetl);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rrvaluead);
PostLoadTable(raingg_body)

PreStoreTable(ECMA, raingg_body);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rrvalue);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rrvaluetl);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, rrvaluead);
PostStoreTable(raingg_body)

DefineLookupTable(raingg_body)

PUBLIC void
ECMA_Dim_T_raingg_body(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_raingg_body *P = T;
  Call_LookupTable(raingg_body, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_raingg_body(void *T)
{
  TABLE_raingg_body *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, rrvalue, Nbytes, Count);
  FreeDS(P, rrvaluetl, Nbytes, Count);
  FreeDS(P, rrvaluead, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(raingg_body,3);
}

DefineRemoveTable(ECMA, raingg_body)

PUBLIC int
ECMA_Sql_T_raingg_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_raingg_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_raingg_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_raingg_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 3);
  InitDS(pk9real, DATATYPE_REAL8, rrvalue, raingg_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, rrvaluetl, raingg_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, rrvaluead, raingg_body, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@raingg_body"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 371;
    pfcom->wt = 358.000350;
    pfcom->tags = ECMA_Set_T_raingg_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_raingg_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_raingg_body;
    pfcom->swapout = ECMA_Swapout_T_raingg_body;
    pfcom->dim = ECMA_Dim_T_raingg_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_raingg_body;
    pfcom->remove = ECMA_Remove_T_raingg_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_raingg_body; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_raingg_body; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_raingg_body;
    pfcom->store = ECMA_Store_T_raingg_body;
    pfcom->pack = ECMA_Pack_T_raingg_body;
    pfcom->unpack = ECMA_Unpack_T_raingg_body;
    pfcom->sql = ECMA_Sql_T_raingg_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
