...
PUBLIC int
ECMA_Pack_T_index(void *T)
{
  int Nbytes = 0;
  TABLE_index *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk1int, target); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk1int, procid); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk1int, timeslot); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, pk1int, kset); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, pk1int, abnob); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, pk1int, mapomm); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, ECMA, pk1int, maptovscv); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, ECMA, linkoffset_t, LINKOFFSET(hdr)); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, ECMA, linklen_t, LINKLEN(hdr)); CHECK_PDS_ERROR(9);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_index(void *T)
{
  int Nbytes = 0;
  TABLE_index *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk1int, target); Nbytes += BYTESIZE(P->target.d);
    UseDS(P, ECMA, pk1int, procid); Nbytes += BYTESIZE(P->procid.d);
    UseDS(P, ECMA, pk1int, timeslot); Nbytes += BYTESIZE(P->timeslot.d);
    UseDS(P, ECMA, pk1int, kset); Nbytes += BYTESIZE(P->kset.d);
    UseDS(P, ECMA, pk1int, abnob); Nbytes += BYTESIZE(P->abnob.d);
    UseDS(P, ECMA, pk1int, mapomm); Nbytes += BYTESIZE(P->mapomm.d);
    UseDS(P, ECMA, pk1int, maptovscv); Nbytes += BYTESIZE(P->maptovscv.d);
    UseDS(P, ECMA, linkoffset_t, LINKOFFSET(hdr)); Nbytes += BYTESIZE(P->LINKOFFSET(hdr).d);
    UseDS(P, ECMA, linklen_t, LINKLEN(hdr)); Nbytes += BYTESIZE(P->LINKLEN(hdr).d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_index(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_index *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(index);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, index)
  Call_CopyGet_TABLE(ECMA, d, 1, index, pk1int, D, target, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 2, index, pk1int, D, procid, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 3, index, pk1int, D, timeslot, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 4, index, pk1int, D, kset, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 5, index, pk1int, D, abnob, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 6, index, pk1int, D, mapomm, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 7, index, pk1int, D, maptovscv, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 8, index, linkoffset_t, D, LINKOFFSET(hdr), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(ECMA, d, 9, index, linklen_t, D, LINKLEN(hdr), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, index)


PrePutTable(ECMA, d, double, index)
  Call_CopyPut_TABLE(ECMA, d, 1, index, pk1int, target, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 2, index, pk1int, procid, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 3, index, pk1int, timeslot, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 4, index, pk1int, kset, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 5, index, pk1int, abnob, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 6, index, pk1int, mapomm, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 7, index, pk1int, maptovscv, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 8, index, linkoffset_t, LINKOFFSET(hdr), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(ECMA, d, 9, index, linklen_t, LINKLEN(hdr), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, index)

PreLoadTable(ECMA, index);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, target);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, procid);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, timeslot);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, kset);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, abnob);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, mapomm);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, maptovscv);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(hdr));
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(hdr));
PostLoadTable(index)

PreStoreTable(ECMA, index);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, target);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, procid);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, timeslot);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, kset);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, abnob);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, mapomm);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, maptovscv);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(hdr));
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(hdr));
PostStoreTable(index)

DefineLookupTable(index)

PUBLIC void
ECMA_Dim_T_index(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_index *P = T;
  Call_LookupTable(index, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_index(void *T)
{
  TABLE_index *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, target, Nbytes, Count);
  FreeDS(P, procid, Nbytes, Count);
  FreeDS(P, timeslot, Nbytes, Count);
  FreeDS(P, kset, Nbytes, Count);
  FreeDS(P, abnob, Nbytes, Count);
  FreeDS(P, mapomm, Nbytes, Count);
  FreeDS(P, maptovscv, Nbytes, Count);
  ...
  ODBMAC_TRACE_SWAPOUT(index,9);
}

DefineRemoveTable(ECMA, index)

PUBLIC int
ECMA_Sql_T_index(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_index(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_index *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_index);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 9);
  InitDS(pk1int, DATATYPE_INT4, target, index, 1);
  InitDS(pk1int, DATATYPE_INT4, procid, index, 1);
  InitDS(pk1int, DATATYPE_INT4, timeslot, index, 1);
  InitDS(pk1int, DATATYPE_INT4, kset, index, 1);
  InitDS(pk1int, DATATYPE_INT4, abnob, index, 1);
  InitDS(pk1int, DATATYPE_INT4, mapomm, index, 1);
  InitDS(pk1int, DATATYPE_INT4, maptovscv, index, 1);
  InitDS(linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(hdr), index, 1);
  InitDS(linklen_t, DATATYPE_LINKLEN, LINKLEN(hdr), index, 1);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@index"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 3;
    pfcom->wt = 1.000003;
    pfcom->tags = ECMA_Set_T_index_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_index_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_index;
    pfcom->swapout = ECMA_Swapout_T_index;
    pfcom->dim = ECMA_Dim_T_index;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_index;
    pfcom->remove = ECMA_Remove_T_index;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_index; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_index; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_index;
    pfcom->store = ECMA_Store_T_index;
    pfcom->pack = ECMA_Pack_T_index;
    pfcom->unpack = ECMA_Unpack_T_index;
    pfcom->sql = ECMA_Sql_T_index;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
