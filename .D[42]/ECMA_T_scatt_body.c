#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_scatt_body 1
#include "ECMA.h"
...
PUBLIC int
ECMA_Pack_T_scatt_body(void *T)
{
  int Nbytes = 0;
  TABLE_scatt_body *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk9real, azimuth); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk9real, incidence); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk9real, kp); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, pk9real, invresid); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, pk9real, dirskill); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, pk1int, mpc); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, ECMA, pk1int, kp_qf); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, ECMA, pk1int, ambig_select); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, ECMA, pk1int, sigma0_qf); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, ECMA, pk9real, sigma0_sm); CHECK_PDS_ERROR(10);
    PDS = PackDS(P, ECMA, pk9real, soilmoist_sd); CHECK_PDS_ERROR(11);
    PDS = PackDS(P, ECMA, pk1int, soilmoist_cf); CHECK_PDS_ERROR(12);
    PDS = PackDS(P, ECMA, pk1int, soilmoist_pf); CHECK_PDS_ERROR(13);
    PDS = PackDS(P, ECMA, pk9real, land_fraction); CHECK_PDS_ERROR(14);
    PDS = PackDS(P, ECMA, pk9real, wetland_fraction); CHECK_PDS_ERROR(15);
    PDS = PackDS(P, ECMA, pk9real, topo_complex); CHECK_PDS_ERROR(16);
    PDS = PackDS(P, ECMA, pk9real, likelihood); CHECK_PDS_ERROR(17);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_scatt_body(void *T)
{
  int Nbytes = 0;
  TABLE_scatt_body *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk9real, azimuth); Nbytes += BYTESIZE(P->azimuth.d);
    UseDS(P, ECMA, pk9real, incidence); Nbytes += BYTESIZE(P->incidence.d);
    UseDS(P, ECMA, pk9real, kp); Nbytes += BYTESIZE(P->kp.d);
    UseDS(P, ECMA, pk9real, invresid); Nbytes += BYTESIZE(P->invresid.d);
    UseDS(P, ECMA, pk9real, dirskill); Nbytes += BYTESIZE(P->dirskill.d);
    UseDS(P, ECMA, pk1int, mpc); Nbytes += BYTESIZE(P->mpc.d);
    UseDS(P, ECMA, pk1int, kp_qf); Nbytes += BYTESIZE(P->kp_qf.d);
    UseDS(P, ECMA, pk1int, ambig_select); Nbytes += BYTESIZE(P->ambig_select.d);
    UseDS(P, ECMA, pk1int, sigma0_qf); Nbytes += BYTESIZE(P->sigma0_qf.d);
    UseDS(P, ECMA, pk9real, sigma0_sm); Nbytes += BYTESIZE(P->sigma0_sm.d);
    UseDS(P, ECMA, pk9real, soilmoist_sd); Nbytes += BYTESIZE(P->soilmoist_sd.d);
    UseDS(P, ECMA, pk1int, soilmoist_cf); Nbytes += BYTESIZE(P->soilmoist_cf.d);
    UseDS(P, ECMA, pk1int, soilmoist_pf); Nbytes += BYTESIZE(P->soilmoist_pf.d);
    UseDS(P, ECMA, pk9real, land_fraction); Nbytes += BYTESIZE(P->land_fraction.d);
    UseDS(P, ECMA, pk9real, wetland_fraction); Nbytes += BYTESIZE(P->wetland_fraction.d);
    UseDS(P, ECMA, pk9real, topo_complex); Nbytes += BYTESIZE(P->topo_complex.d);
    UseDS(P, ECMA, pk9real, likelihood); Nbytes += BYTESIZE(P->likelihood.d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_scatt_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_scatt_body *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(scatt_body);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, scatt_body)
  Call_CopyGet_TABLE(ECMA, d, 1, scatt_body, pk9real, D, azimuth, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, scatt_body, pk9real, D, incidence, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 3, scatt_body, pk9real, D, kp, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 4, scatt_body, pk9real, D, invresid, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 5, scatt_body, pk9real, D, dirskill, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 6, scatt_body, pk1int, D, mpc, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 7, scatt_body, pk1int, D, kp_qf, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 8, scatt_body, pk1int, D, ambig_select, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 9, scatt_body, pk1int, D, sigma0_qf, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 10, scatt_body, pk9real, D, sigma0_sm, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 11, scatt_body, pk9real, D, soilmoist_sd, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 12, scatt_body, pk1int, D, soilmoist_cf, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 13, scatt_body, pk1int, D, soilmoist_pf, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 14, scatt_body, pk9real, D, land_fraction, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 15, scatt_body, pk9real, D, wetland_fraction, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 16, scatt_body, pk9real, D, topo_complex, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 17, scatt_body, pk9real, D, likelihood, Count, DATATYPE_REAL8);
PostGetTable(d, double, scatt_body)


PrePutTable(ECMA, d, double, scatt_body)
  Call_CopyPut_TABLE(ECMA, d, 1, scatt_body, pk9real, azimuth, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, scatt_body, pk9real, incidence, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 3, scatt_body, pk9real, kp, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 4, scatt_body, pk9real, invresid, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 5, scatt_body, pk9real, dirskill, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 6, scatt_body, pk1int, mpc, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 7, scatt_body, pk1int, kp_qf, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 8, scatt_body, pk1int, ambig_select, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 9, scatt_body, pk1int, sigma0_qf, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 10, scatt_body, pk9real, sigma0_sm, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 11, scatt_body, pk9real, soilmoist_sd, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 12, scatt_body, pk1int, soilmoist_cf, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 13, scatt_body, pk1int, soilmoist_pf, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 14, scatt_body, pk9real, land_fraction, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 15, scatt_body, pk9real, wetland_fraction, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 16, scatt_body, pk9real, topo_complex, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 17, scatt_body, pk9real, likelihood, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, scatt_body)

PreLoadTable(ECMA, scatt_body);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, azimuth);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, incidence);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, kp);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, invresid);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, dirskill);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, mpc);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, kp_qf);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, ambig_select);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, sigma0_qf);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, sigma0_sm);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, soilmoist_sd);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, soilmoist_cf);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, soilmoist_pf);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, land_fraction);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, wetland_fraction);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, topo_complex);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, likelihood);
PostLoadTable(scatt_body)

PreStoreTable(ECMA, scatt_body);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, azimuth);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, incidence);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, kp);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, invresid);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, dirskill);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, mpc);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, kp_qf);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, ambig_select);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, sigma0_qf);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, sigma0_sm);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, soilmoist_sd);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, soilmoist_cf);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, soilmoist_pf);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, land_fraction);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, wetland_fraction);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, topo_complex);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, likelihood);
PostStoreTable(scatt_body)

DefineLookupTable(scatt_body)

PUBLIC void
ECMA_Dim_T_scatt_body(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_scatt_body *P = T;
  Call_LookupTable(scatt_body, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_scatt_body(void *T)
{
  TABLE_scatt_body *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, incidence, Nbytes, Count);
  FreeDS(P, kp, Nbytes, Count);
  FreeDS(P, invresid, Nbytes, Count);
  FreeDS(P, dirskill, Nbytes, Count);
  FreeDS(P, mpc, Nbytes, Count);
  FreeDS(P, kp_qf, Nbytes, Count);
  FreeDS(P, ambig_select, Nbytes, Count);
  FreeDS(P, sigma0_qf, Nbytes, Count);
  FreeDS(P, sigma0_sm, Nbytes, Count);
  FreeDS(P, soilmoist_sd, Nbytes, Count);
  FreeDS(P, soilmoist_cf, Nbytes, Count);
  FreeDS(P, soilmoist_pf, Nbytes, Count);
  FreeDS(P, land_fraction, Nbytes, Count);
  FreeDS(P, wetland_fraction, Nbytes, Count);
  FreeDS(P, topo_complex, Nbytes, Count);
  FreeDS(P, likelihood, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(scatt_body,17);
}

DefineRemoveTable(ECMA, scatt_body)

PUBLIC int
ECMA_Sql_T_scatt_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_scatt_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_scatt_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_scatt_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 17);
  InitDS(pk9real, DATATYPE_REAL8, azimuth, scatt_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, incidence, scatt_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, kp, scatt_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, invresid, scatt_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, dirskill, scatt_body, 9);
  InitDS(pk1int, DATATYPE_INT4, mpc, scatt_body, 1);
  InitDS(pk1int, DATATYPE_INT4, kp_qf, scatt_body, 1);
  InitDS(pk1int, DATATYPE_INT4, ambig_select, scatt_body, 1);
  InitDS(pk1int, DATATYPE_INT4, sigma0_qf, scatt_body, 1);
  InitDS(pk9real, DATATYPE_REAL8, sigma0_sm, scatt_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, soilmoist_sd, scatt_body, 9);
  InitDS(pk1int, DATATYPE_INT4, soilmoist_cf, scatt_body, 1);
  InitDS(pk1int, DATATYPE_INT4, soilmoist_pf, scatt_body, 1);
  InitDS(pk9real, DATATYPE_REAL8, land_fraction, scatt_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, wetland_fraction, scatt_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, topo_complex, scatt_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, likelihood, scatt_body, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@scatt_body"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 379;
    pfcom->wt = 358.000370;
    pfcom->tags = ECMA_Set_T_scatt_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_scatt_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_scatt_body;
    pfcom->swapout = ECMA_Swapout_T_scatt_body;
    pfcom->dim = ECMA_Dim_T_scatt_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_scatt_body;
    pfcom->remove = ECMA_Remove_T_scatt_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_scatt_body; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_scatt_body; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_scatt_body;
    pfcom->store = ECMA_Store_T_scatt_body;
    pfcom->pack = ECMA_Pack_T_scatt_body;
    pfcom->unpack = ECMA_Unpack_T_scatt_body;
    pfcom->sql = ECMA_Sql_T_scatt_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
