...
PUBLIC int
ECMA_Pack_T_surfbody_feedback_100(void *T)
{
  int Nbytes = 0;
  TABLE_surfbody_feedback_100 *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, Bitfield, datum_status); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, Bitfield, datum_sfc_event); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk9real, an_depar); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, pk9real, fg_depar); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, pk9real, snow_depth); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, pk9real, snow_density); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, ECMA, pk9real, lsm); CHECK_PDS_ERROR(7);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_surfbody_feedback_100(void *T)
{
  int Nbytes = 0;
  TABLE_surfbody_feedback_100 *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, Bitfield, datum_status); Nbytes += BYTESIZE(P->datum_status.d);
    UseDS(P, ECMA, Bitfield, datum_sfc_event); Nbytes += BYTESIZE(P->datum_sfc_event.d);
    ...
    UseDS(P, ECMA, pk9real, snow_density); Nbytes += BYTESIZE(P->snow_density.d);
...
PUBLIC int
ECMA_Sel_T_surfbody_feedback_100(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_surfbody_feedback_100 *P = T;
...
PreGetTable(ECMA, d, double, surfbody_feedback_100)
  Call_CopyGet_TABLE(ECMA, d, 1, surfbody_feedback_100, Bitfield, D, datum_status, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(ECMA, d, 2, surfbody_feedback_100, Bitfield, D, datum_sfc_event, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(ECMA, d, 3, surfbody_feedback_100, pk9real, D, an_depar, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 4, surfbody_feedback_100, pk9real, D, fg_depar, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 5, surfbody_feedback_100, pk9real, D, snow_depth, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 6, surfbody_feedback_100, pk9real, D, snow_density, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 7, surfbody_feedback_100, pk9real, D, lsm, Count, DATATYPE_REAL8);
PostGetTable(d, double, surfbody_feedback_100)


PrePutTable(ECMA, d, double, surfbody_feedback_100)
  Call_CopyPut_TABLE(ECMA, d, 1, surfbody_feedback_100, Bitfield, datum_status, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(ECMA, d, 2, surfbody_feedback_100, Bitfield, datum_sfc_event, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(ECMA, d, 3, surfbody_feedback_100, pk9real, an_depar, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 4, surfbody_feedback_100, pk9real, fg_depar, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 5, surfbody_feedback_100, pk9real, snow_depth, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 6, surfbody_feedback_100, pk9real, snow_density, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 7, surfbody_feedback_100, pk9real, lsm, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, surfbody_feedback_100)

PreLoadTable(ECMA, surfbody_feedback_100);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, datum_status);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, datum_sfc_event);
  ...
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, snow_density);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, lsm);
PostLoadTable(surfbody_feedback_100)

PreStoreTable(ECMA, surfbody_feedback_100);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, datum_status);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, datum_sfc_event);
  ...
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, snow_density);
...
PUBLIC void
ECMA_Dim_T_surfbody_feedback_100(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
ECMA_Swapout_T_surfbody_feedback_100(void *T)
{
  TABLE_surfbody_feedback_100 *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(ECMA, surfbody_feedback_100)

PUBLIC int
ECMA_Sql_T_surfbody_feedback_100(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_surfbody_feedback_100(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_surfbody_feedback_100 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_surfbody_feedback_100);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 7);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 349;
    pfcom->wt = 358.000325;
    pfcom->tags = ECMA_Set_T_surfbody_feedback_100_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_surfbody_feedback_100_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_surfbody_feedback_100;
    pfcom->swapout = ECMA_Swapout_T_surfbody_feedback_100;
    pfcom->dim = ECMA_Dim_T_surfbody_feedback_100;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_surfbody_feedback_100;
    pfcom->remove = ECMA_Remove_T_surfbody_feedback_100;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_surfbody_feedback_100; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_surfbody_feedback_100; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_surfbody_feedback_100;
    pfcom->store = ECMA_Store_T_surfbody_feedback_100;
    pfcom->pack = ECMA_Pack_T_surfbody_feedback_100;
    pfcom->unpack = ECMA_Unpack_T_surfbody_feedback_100;
    pfcom->sql = ECMA_Sql_T_surfbody_feedback_100;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
