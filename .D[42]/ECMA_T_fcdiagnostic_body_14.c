...
PUBLIC int
ECMA_Pack_T_fcdiagnostic_body_14(void *T)
{
  int Nbytes = 0;
...
PUBLIC int
ECMA_Unpack_T_fcdiagnostic_body_14(void *T)
{
  int Nbytes = 0;
  TABLE_fcdiagnostic_body_14 *P = T;
  if (P->Is_loaded) {
...
PUBLIC int
ECMA_Sel_T_fcdiagnostic_body_14(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_fcdiagnostic_body_14 *P = T;
...
PreGetTable(ECMA, d, double, fcdiagnostic_body_14)
  Call_CopyGet_TABLE(ECMA, d, 1, fcdiagnostic_body_14, pk9real, D, fc_depar, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, fcdiagnostic_body_14, yyyymmdd, D, fc_step, Count, DATATYPE_YYYYMMDD);
PostGetTable(d, double, fcdiagnostic_body_14)


PrePutTable(ECMA, d, double, fcdiagnostic_body_14)
  Call_CopyPut_TABLE(ECMA, d, 1, fcdiagnostic_body_14, pk9real, fc_depar, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, fcdiagnostic_body_14, yyyymmdd, fc_step, D, Count, DATATYPE_YYYYMMDD);
PostPutTable(d, double, fcdiagnostic_body_14)

PreLoadTable(ECMA, fcdiagnostic_body_14);
...
PreStoreTable(ECMA, fcdiagnostic_body_14);
...
PUBLIC void
ECMA_Dim_T_fcdiagnostic_body_14(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
ECMA_Swapout_T_fcdiagnostic_body_14(void *T)
{
  TABLE_fcdiagnostic_body_14 *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(ECMA, fcdiagnostic_body_14)

PUBLIC int
ECMA_Sql_T_fcdiagnostic_body_14(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_fcdiagnostic_body_14(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_fcdiagnostic_body_14 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_fcdiagnostic_body_14);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 2);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 363;
    pfcom->wt = 358.000340;
    pfcom->tags = ECMA_Set_T_fcdiagnostic_body_14_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_fcdiagnostic_body_14_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_fcdiagnostic_body_14;
    pfcom->swapout = ECMA_Swapout_T_fcdiagnostic_body_14;
    pfcom->dim = ECMA_Dim_T_fcdiagnostic_body_14;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_fcdiagnostic_body_14;
    pfcom->remove = ECMA_Remove_T_fcdiagnostic_body_14;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_fcdiagnostic_body_14; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_fcdiagnostic_body_14; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_fcdiagnostic_body_14;
    pfcom->store = ECMA_Store_T_fcdiagnostic_body_14;
    pfcom->pack = ECMA_Pack_T_fcdiagnostic_body_14;
    pfcom->unpack = ECMA_Unpack_T_fcdiagnostic_body_14;
    pfcom->sql = ECMA_Sql_T_fcdiagnostic_body_14;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
