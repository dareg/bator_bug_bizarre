#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_radar_station 1
#include "ECMA.h"
...
PUBLIC int
ECMA_Pack_T_radar_station(void *T)
{
  int Nbytes = 0;
  TABLE_radar_station *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk1int, ident); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, string, type); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk9real, lat); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, pk9real, lon); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, pk9real, stalt); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, pk9real, antenht); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, ECMA, pk9real, beamwidth); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, ECMA, pk9real, frequency); CHECK_PDS_ERROR(8);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_radar_station(void *T)
{
  int Nbytes = 0;
  TABLE_radar_station *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk1int, ident); Nbytes += BYTESIZE(P->ident.d);
    UseDS(P, ECMA, string, type); Nbytes += BYTESIZE(P->type.d);
    ...
    UseDS(P, ECMA, pk9real, antenht); Nbytes += BYTESIZE(P->antenht.d);
    UseDS(P, ECMA, pk9real, beamwidth); Nbytes += BYTESIZE(P->beamwidth.d);
...
PUBLIC int
ECMA_Sel_T_radar_station(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_radar_station *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(radar_station);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, radar_station)
  Call_CopyGet_TABLE(ECMA, d, 1, radar_station, pk1int, D, ident, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 2, radar_station, string, D, type, Count, DATATYPE_STRING);
  Call_CopyGet_TABLE(ECMA, d, 3, radar_station, pk9real, D, lat, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 4, radar_station, pk9real, D, lon, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 5, radar_station, pk9real, D, stalt, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 6, radar_station, pk9real, D, antenht, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 7, radar_station, pk9real, D, beamwidth, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 8, radar_station, pk9real, D, frequency, Count, DATATYPE_REAL8);
PostGetTable(d, double, radar_station)


PrePutTable(ECMA, d, double, radar_station)
  Call_CopyPut_TABLE(ECMA, d, 1, radar_station, pk1int, ident, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 2, radar_station, string, type, D, Count, DATATYPE_STRING);
  Call_CopyPut_TABLE(ECMA, d, 3, radar_station, pk9real, lat, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 4, radar_station, pk9real, lon, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 5, radar_station, pk9real, stalt, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 6, radar_station, pk9real, antenht, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 7, radar_station, pk9real, beamwidth, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 8, radar_station, pk9real, frequency, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, radar_station)

PreLoadTable(ECMA, radar_station);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, ident);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, string, DATATYPE_STRING, type);
  ...
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, antenht);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, beamwidth);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, frequency);
PostLoadTable(radar_station)

PreStoreTable(ECMA, radar_station);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, ident);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, string, DATATYPE_STRING, type);
  ...
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, antenht);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, beamwidth);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, frequency);
PostStoreTable(radar_station)

DefineLookupTable(radar_station)

PUBLIC void
ECMA_Dim_T_radar_station(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_radar_station *P = T;
  Call_LookupTable(radar_station, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_radar_station(void *T)
{
  TABLE_radar_station *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, antenht, Nbytes, Count);
  FreeDS(P, beamwidth, Nbytes, Count);
  FreeDS(P, frequency, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(radar_station,8);
}

DefineRemoveTable(ECMA, radar_station)

PUBLIC int
ECMA_Sql_T_radar_station(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_radar_station(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_radar_station *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_radar_station);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 8);
  InitDS(pk1int, DATATYPE_INT4, ident, radar_station, 1);
  InitDS(string, DATATYPE_STRING, type, radar_station, 3);
  InitDS(pk9real, DATATYPE_REAL8, lat, radar_station, 9);
  InitDS(pk9real, DATATYPE_REAL8, lon, radar_station, 9);
  InitDS(pk9real, DATATYPE_REAL8, stalt, radar_station, 9);
  InitDS(pk9real, DATATYPE_REAL8, antenht, radar_station, 9);
  InitDS(pk9real, DATATYPE_REAL8, beamwidth, radar_station, 9);
  InitDS(pk9real, DATATYPE_REAL8, frequency, radar_station, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@radar_station"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 20;
    pfcom->wt = 3.002004;
    pfcom->tags = ECMA_Set_T_radar_station_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_radar_station_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_radar_station;
    pfcom->swapout = ECMA_Swapout_T_radar_station;
    pfcom->dim = ECMA_Dim_T_radar_station;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_radar_station;
    pfcom->remove = ECMA_Remove_T_radar_station;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_radar_station; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_radar_station; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_radar_station;
    pfcom->store = ECMA_Store_T_radar_station;
    pfcom->pack = ECMA_Pack_T_radar_station;
    pfcom->unpack = ECMA_Unpack_T_radar_station;
    pfcom->sql = ECMA_Sql_T_radar_station;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
