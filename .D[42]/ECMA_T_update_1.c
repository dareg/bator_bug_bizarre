...
PUBLIC int
ECMA_Pack_T_update_1(void *T)
{
  int Nbytes = 0;
  TABLE_update_1 *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk9real, hires); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk9real, lores); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk1int, datum_tbflag_hires); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, Bitfield, datum_status_hires); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, pk9real, obs_diags_1); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, pk9real, obs_diags_2); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, ECMA, pk9real, obs_diags_3); CHECK_PDS_ERROR(7);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_update_1(void *T)
{
  int Nbytes = 0;
  TABLE_update_1 *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk9real, hires); Nbytes += BYTESIZE(P->hires.d);
    UseDS(P, ECMA, pk9real, lores); Nbytes += BYTESIZE(P->lores.d);
    UseDS(P, ECMA, pk1int, datum_tbflag_hires); Nbytes += BYTESIZE(P->datum_tbflag_hires.d);
    UseDS(P, ECMA, Bitfield, datum_status_hires); Nbytes += BYTESIZE(P->datum_status_hires.d);
    UseDS(P, ECMA, pk9real, obs_diags_1); Nbytes += BYTESIZE(P->obs_diags_1.d);
    UseDS(P, ECMA, pk9real, obs_diags_2); Nbytes += BYTESIZE(P->obs_diags_2.d);
    UseDS(P, ECMA, pk9real, obs_diags_3); Nbytes += BYTESIZE(P->obs_diags_3.d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_update_1(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_update_1 *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(update_1);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, update_1)
  Call_CopyGet_TABLE(ECMA, d, 1, update_1, pk9real, D, hires, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, update_1, pk9real, D, lores, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 3, update_1, pk1int, D, datum_tbflag_hires, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 4, update_1, Bitfield, D, datum_status_hires, Count, DATATYPE_BITFIELD);
  Call_CopyGet_TABLE(ECMA, d, 5, update_1, pk9real, D, obs_diags_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 6, update_1, pk9real, D, obs_diags_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 7, update_1, pk9real, D, obs_diags_3, Count, DATATYPE_REAL8);
PostGetTable(d, double, update_1)


PrePutTable(ECMA, d, double, update_1)
  Call_CopyPut_TABLE(ECMA, d, 1, update_1, pk9real, hires, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, update_1, pk9real, lores, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 3, update_1, pk1int, datum_tbflag_hires, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 4, update_1, Bitfield, datum_status_hires, D, Count, DATATYPE_BITFIELD);
  Call_CopyPut_TABLE(ECMA, d, 5, update_1, pk9real, obs_diags_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 6, update_1, pk9real, obs_diags_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 7, update_1, pk9real, obs_diags_3, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, update_1)

PreLoadTable(ECMA, update_1);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hires);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, lores);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, datum_tbflag_hires);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, datum_status_hires);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_diags_1);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_diags_2);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_diags_3);
PostLoadTable(update_1)

PreStoreTable(ECMA, update_1);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hires);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, lores);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, datum_tbflag_hires);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, Bitfield, DATATYPE_BITFIELD, datum_status_hires);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_diags_1);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_diags_2);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_diags_3);
PostStoreTable(update_1)

DefineLookupTable(update_1)

PUBLIC void
ECMA_Dim_T_update_1(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_update_1 *P = T;
  Call_LookupTable(update_1, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_update_1(void *T)
{
  TABLE_update_1 *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, datum_tbflag_hires, Nbytes, Count);
  FreeDS(P, datum_status_hires, Nbytes, Count);
  FreeDS(P, obs_diags_1, Nbytes, Count);
  FreeDS(P, obs_diags_2, Nbytes, Count);
  FreeDS(P, obs_diags_3, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(update_1,7);
}

DefineRemoveTable(ECMA, update_1)

PUBLIC int
ECMA_Sql_T_update_1(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_update_1(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_update_1 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_update_1);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 7);
  InitDS(pk9real, DATATYPE_REAL8, hires, update_1, 9);
  InitDS(pk9real, DATATYPE_REAL8, lores, update_1, 9);
  InitDS(pk1int, DATATYPE_INT4, datum_tbflag_hires, update_1, 1);
  InitDS(Bitfield, DATATYPE_BITFIELD, datum_status_hires, update_1, 1);
  InitDS(pk9real, DATATYPE_REAL8, obs_diags_1, update_1, 9);
  InitDS(pk9real, DATATYPE_REAL8, obs_diags_2, update_1, 9);
  InitDS(pk9real, DATATYPE_REAL8, obs_diags_3, update_1, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@update_1"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 384;
    pfcom->wt = 358.000380;
    pfcom->tags = ECMA_Set_T_update_1_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_update_1_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_update_1;
    pfcom->swapout = ECMA_Swapout_T_update_1;
    pfcom->dim = ECMA_Dim_T_update_1;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_update_1;
    pfcom->remove = ECMA_Remove_T_update_1;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_update_1; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_update_1; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_update_1;
    pfcom->store = ECMA_Store_T_update_1;
    pfcom->pack = ECMA_Pack_T_update_1;
    pfcom->unpack = ECMA_Unpack_T_update_1;
    pfcom->sql = ECMA_Sql_T_update_1;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
