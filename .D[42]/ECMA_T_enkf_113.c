...
ECMA_Pack_T_enkf_113(void *T)
{
  ...
  if (P->Is_loaded) {
    ...
    PDS = PackDS(P, ECMA, pk9real, hprior); CHECK_PDS_ERROR(2);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_enkf_113(void *T)
{
  int Nbytes = 0;
  TABLE_enkf_113 *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk1int, member); Nbytes += BYTESIZE(P->member.d);
    UseDS(P, ECMA, pk9real, hprior); Nbytes += BYTESIZE(P->hprior.d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_enkf_113(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_enkf_113 *P = T;
...
PreGetTable(ECMA, d, double, enkf_113)
  Call_CopyGet_TABLE(ECMA, d, 1, enkf_113, pk1int, D, member, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 2, enkf_113, pk9real, D, hprior, Count, DATATYPE_REAL8);
PostGetTable(d, double, enkf_113)


PrePutTable(ECMA, d, double, enkf_113)
  Call_CopyPut_TABLE(ECMA, d, 1, enkf_113, pk1int, member, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 2, enkf_113, pk9real, hprior, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, enkf_113)

PreLoadTable(ECMA, enkf_113);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, member);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hprior);
PostLoadTable(enkf_113)

PreStoreTable(ECMA, enkf_113);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, member);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, hprior);
PostStoreTable(enkf_113)

DefineLookupTable(enkf_113)

PUBLIC void
ECMA_Dim_T_enkf_113(void *T, int *Nrows, int *Ncols,
...
PUBLIC void
ECMA_Swapout_T_enkf_113(void *T)
{
  TABLE_enkf_113 *P = T;
  int Nbytes = 0;
  int Count = 0;
...
DefineRemoveTable(ECMA, enkf_113)

PUBLIC int
ECMA_Sql_T_enkf_113(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_enkf_113(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_enkf_113 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_enkf_113);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 2);
  ...
  if (!pfcom) { /* Initialize once only */
    ...
    pfcom->rank = 142;
    pfcom->wt = 358.000118;
    pfcom->tags = ECMA_Set_T_enkf_113_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_enkf_113_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_enkf_113;
    pfcom->swapout = ECMA_Swapout_T_enkf_113;
    pfcom->dim = ECMA_Dim_T_enkf_113;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_enkf_113;
    pfcom->remove = ECMA_Remove_T_enkf_113;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_enkf_113; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_enkf_113; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_enkf_113;
    pfcom->store = ECMA_Store_T_enkf_113;
    pfcom->pack = ECMA_Pack_T_enkf_113;
    pfcom->unpack = ECMA_Unpack_T_enkf_113;
    pfcom->sql = ECMA_Sql_T_enkf_113;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
