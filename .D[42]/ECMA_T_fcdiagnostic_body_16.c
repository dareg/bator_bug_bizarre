#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_fcdiagnostic_body_16 1
#include "ECMA.h"
...
PUBLIC int
ECMA_Pack_T_fcdiagnostic_body_16(void *T)
{
  int Nbytes = 0;
  TABLE_fcdiagnostic_body_16 *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk9real, fc_depar); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, yyyymmdd, fc_step); CHECK_PDS_ERROR(2);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_fcdiagnostic_body_16(void *T)
{
  int Nbytes = 0;
  TABLE_fcdiagnostic_body_16 *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk9real, fc_depar); Nbytes += BYTESIZE(P->fc_depar.d);
    UseDS(P, ECMA, yyyymmdd, fc_step); Nbytes += BYTESIZE(P->fc_step.d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_fcdiagnostic_body_16(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_fcdiagnostic_body_16 *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(fcdiagnostic_body_16);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, fcdiagnostic_body_16)
  Call_CopyGet_TABLE(ECMA, d, 1, fcdiagnostic_body_16, pk9real, D, fc_depar, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, fcdiagnostic_body_16, yyyymmdd, D, fc_step, Count, DATATYPE_YYYYMMDD);
PostGetTable(d, double, fcdiagnostic_body_16)


PrePutTable(ECMA, d, double, fcdiagnostic_body_16)
  Call_CopyPut_TABLE(ECMA, d, 1, fcdiagnostic_body_16, pk9real, fc_depar, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, fcdiagnostic_body_16, yyyymmdd, fc_step, D, Count, DATATYPE_YYYYMMDD);
PostPutTable(d, double, fcdiagnostic_body_16)

PreLoadTable(ECMA, fcdiagnostic_body_16);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, fc_depar);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, yyyymmdd, DATATYPE_YYYYMMDD, fc_step);
PostLoadTable(fcdiagnostic_body_16)

PreStoreTable(ECMA, fcdiagnostic_body_16);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, fc_depar);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, yyyymmdd, DATATYPE_YYYYMMDD, fc_step);
PostStoreTable(fcdiagnostic_body_16)

DefineLookupTable(fcdiagnostic_body_16)

PUBLIC void
ECMA_Dim_T_fcdiagnostic_body_16(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_fcdiagnostic_body_16 *P = T;
  Call_LookupTable(fcdiagnostic_body_16, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_fcdiagnostic_body_16(void *T)
{
  TABLE_fcdiagnostic_body_16 *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  ODBMAC_TRACE_SWAPOUT(fcdiagnostic_body_16,2);
}

DefineRemoveTable(ECMA, fcdiagnostic_body_16)

PUBLIC int
ECMA_Sql_T_fcdiagnostic_body_16(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_fcdiagnostic_body_16(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_fcdiagnostic_body_16 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_fcdiagnostic_body_16);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 2);
  InitDS(pk9real, DATATYPE_REAL8, fc_depar, fcdiagnostic_body_16, 9);
  InitDS(yyyymmdd, DATATYPE_YYYYMMDD, fc_step, fcdiagnostic_body_16, 1);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@fcdiagnostic_body_16"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 365;
    pfcom->wt = 358.000342;
    pfcom->tags = ECMA_Set_T_fcdiagnostic_body_16_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_fcdiagnostic_body_16_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_fcdiagnostic_body_16;
    pfcom->swapout = ECMA_Swapout_T_fcdiagnostic_body_16;
    pfcom->dim = ECMA_Dim_T_fcdiagnostic_body_16;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_fcdiagnostic_body_16;
    pfcom->remove = ECMA_Remove_T_fcdiagnostic_body_16;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_fcdiagnostic_body_16; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_fcdiagnostic_body_16; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_fcdiagnostic_body_16;
    pfcom->store = ECMA_Store_T_fcdiagnostic_body_16;
    pfcom->pack = ECMA_Pack_T_fcdiagnostic_body_16;
    pfcom->unpack = ECMA_Unpack_T_fcdiagnostic_body_16;
    pfcom->sql = ECMA_Sql_T_fcdiagnostic_body_16;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
...
/* *************** End of TABLE "fcdiagnostic_body_16" *************** */
