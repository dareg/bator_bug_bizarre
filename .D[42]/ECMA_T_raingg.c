...
PUBLIC int
ECMA_Pack_T_raingg(void *T)
{
  int Nbytes = 0;
  TABLE_raingg *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk1int, report_rrflag); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, linkoffset_t, LINKOFFSET(raingg_body)); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, linklen_t, LINKLEN(raingg_body)); CHECK_PDS_ERROR(3);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_raingg(void *T)
{
  int Nbytes = 0;
  TABLE_raingg *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk1int, report_rrflag); Nbytes += BYTESIZE(P->report_rrflag.d);
    UseDS(P, ECMA, linkoffset_t, LINKOFFSET(raingg_body)); Nbytes += BYTESIZE(P->LINKOFFSET(raingg_body).d);
    UseDS(P, ECMA, linklen_t, LINKLEN(raingg_body)); Nbytes += BYTESIZE(P->LINKLEN(raingg_body).d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_raingg(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_raingg *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(raingg);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, raingg)
  Call_CopyGet_TABLE(ECMA, d, 1, raingg, pk1int, D, report_rrflag, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 2, raingg, linkoffset_t, D, LINKOFFSET(raingg_body), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(ECMA, d, 3, raingg, linklen_t, D, LINKLEN(raingg_body), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, raingg)


PrePutTable(ECMA, d, double, raingg)
  Call_CopyPut_TABLE(ECMA, d, 1, raingg, pk1int, report_rrflag, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 2, raingg, linkoffset_t, LINKOFFSET(raingg_body), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(ECMA, d, 3, raingg, linklen_t, LINKLEN(raingg_body), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, raingg)

PreLoadTable(ECMA, raingg);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, report_rrflag);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(raingg_body));
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(raingg_body));
PostLoadTable(raingg)

PreStoreTable(ECMA, raingg);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, report_rrflag);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(raingg_body));
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(raingg_body));
PostStoreTable(raingg)

DefineLookupTable(raingg)

PUBLIC void
ECMA_Dim_T_raingg(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_raingg *P = T;
  Call_LookupTable(raingg, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_raingg(void *T)
{
  TABLE_raingg *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, report_rrflag, Nbytes, Count);
  FreeDS(P, LINKOFFSET(raingg_body), Nbytes, Count);
  FreeDS(P, LINKLEN(raingg_body), Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(raingg,3);
}

DefineRemoveTable(ECMA, raingg)

PUBLIC int
ECMA_Sql_T_raingg(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_raingg(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_raingg *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_raingg);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 3);
  InitDS(pk1int, DATATYPE_INT4, report_rrflag, raingg, 1);
  InitDS(linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(raingg_body), raingg, 1);
  InitDS(linklen_t, DATATYPE_LINKLEN, LINKLEN(raingg_body), raingg, 1);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@raingg"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 8;
    pfcom->wt = 3.001004;
    pfcom->tags = ECMA_Set_T_raingg_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_raingg_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_raingg;
    pfcom->swapout = ECMA_Swapout_T_raingg;
    pfcom->dim = ECMA_Dim_T_raingg;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_raingg;
    pfcom->remove = ECMA_Remove_T_raingg;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_raingg; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_raingg; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_raingg;
    pfcom->store = ECMA_Store_T_raingg;
    pfcom->pack = ECMA_Pack_T_raingg;
    pfcom->unpack = ECMA_Unpack_T_raingg;
    pfcom->sql = ECMA_Sql_T_raingg;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
