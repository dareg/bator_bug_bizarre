#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_auxiliary_body 1
#include "ECMA.h"
...
PUBLIC int
ECMA_Pack_T_auxiliary_body(void *T)
{
  int Nbytes = 0;
  TABLE_auxiliary_body *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk9real, datum_aux_1); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk9real, datum_aux_2); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk9real, datum_aux_3); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, pk9real, datum_aux_4); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, pk9real, datum_aux_5); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, pk9real, datum_aux_6); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, ECMA, pk9real, datum_aux_7); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, ECMA, pk9real, datum_aux_8); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, ECMA, pk9real, datum_aux_9); CHECK_PDS_ERROR(9);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_auxiliary_body(void *T)
{
  int Nbytes = 0;
  TABLE_auxiliary_body *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk9real, datum_aux_1); Nbytes += BYTESIZE(P->datum_aux_1.d);
    UseDS(P, ECMA, pk9real, datum_aux_2); Nbytes += BYTESIZE(P->datum_aux_2.d);
    UseDS(P, ECMA, pk9real, datum_aux_3); Nbytes += BYTESIZE(P->datum_aux_3.d);
    UseDS(P, ECMA, pk9real, datum_aux_4); Nbytes += BYTESIZE(P->datum_aux_4.d);
    UseDS(P, ECMA, pk9real, datum_aux_5); Nbytes += BYTESIZE(P->datum_aux_5.d);
    UseDS(P, ECMA, pk9real, datum_aux_6); Nbytes += BYTESIZE(P->datum_aux_6.d);
    UseDS(P, ECMA, pk9real, datum_aux_7); Nbytes += BYTESIZE(P->datum_aux_7.d);
    UseDS(P, ECMA, pk9real, datum_aux_8); Nbytes += BYTESIZE(P->datum_aux_8.d);
    UseDS(P, ECMA, pk9real, datum_aux_9); Nbytes += BYTESIZE(P->datum_aux_9.d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_auxiliary_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_auxiliary_body *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(auxiliary_body);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, auxiliary_body)
  Call_CopyGet_TABLE(ECMA, d, 1, auxiliary_body, pk9real, D, datum_aux_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, auxiliary_body, pk9real, D, datum_aux_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 3, auxiliary_body, pk9real, D, datum_aux_3, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 4, auxiliary_body, pk9real, D, datum_aux_4, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 5, auxiliary_body, pk9real, D, datum_aux_5, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 6, auxiliary_body, pk9real, D, datum_aux_6, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 7, auxiliary_body, pk9real, D, datum_aux_7, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 8, auxiliary_body, pk9real, D, datum_aux_8, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 9, auxiliary_body, pk9real, D, datum_aux_9, Count, DATATYPE_REAL8);
PostGetTable(d, double, auxiliary_body)


PrePutTable(ECMA, d, double, auxiliary_body)
  Call_CopyPut_TABLE(ECMA, d, 1, auxiliary_body, pk9real, datum_aux_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, auxiliary_body, pk9real, datum_aux_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 3, auxiliary_body, pk9real, datum_aux_3, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 4, auxiliary_body, pk9real, datum_aux_4, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 5, auxiliary_body, pk9real, datum_aux_5, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 6, auxiliary_body, pk9real, datum_aux_6, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 7, auxiliary_body, pk9real, datum_aux_7, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 8, auxiliary_body, pk9real, datum_aux_8, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 9, auxiliary_body, pk9real, datum_aux_9, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, auxiliary_body)

PreLoadTable(ECMA, auxiliary_body);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_1);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_2);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_3);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_4);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_5);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_6);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_7);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_8);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_9);
PostLoadTable(auxiliary_body)

PreStoreTable(ECMA, auxiliary_body);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_1);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_2);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_3);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_4);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_5);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_6);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_7);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_8);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, datum_aux_9);
PostStoreTable(auxiliary_body)

DefineLookupTable(auxiliary_body)

PUBLIC void
ECMA_Dim_T_auxiliary_body(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_auxiliary_body *P = T;
  Call_LookupTable(auxiliary_body, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_auxiliary_body(void *T)
{
  TABLE_auxiliary_body *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, datum_aux_1, Nbytes, Count);
  FreeDS(P, datum_aux_2, Nbytes, Count);
  FreeDS(P, datum_aux_3, Nbytes, Count);
  FreeDS(P, datum_aux_4, Nbytes, Count);
  FreeDS(P, datum_aux_5, Nbytes, Count);
  FreeDS(P, datum_aux_6, Nbytes, Count);
  FreeDS(P, datum_aux_7, Nbytes, Count);
  FreeDS(P, datum_aux_8, Nbytes, Count);
  FreeDS(P, datum_aux_9, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(auxiliary_body,9);
}

DefineRemoveTable(ECMA, auxiliary_body)

PUBLIC int
ECMA_Sql_T_auxiliary_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_auxiliary_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_auxiliary_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_auxiliary_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 9);
  InitDS(pk9real, DATATYPE_REAL8, datum_aux_1, auxiliary_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, datum_aux_2, auxiliary_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, datum_aux_3, auxiliary_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, datum_aux_4, auxiliary_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, datum_aux_5, auxiliary_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, datum_aux_6, auxiliary_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, datum_aux_7, auxiliary_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, datum_aux_8, auxiliary_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, datum_aux_9, auxiliary_body, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@auxiliary_body"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 376;
    pfcom->wt = 358.000363;
    pfcom->tags = ECMA_Set_T_auxiliary_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_auxiliary_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_auxiliary_body;
    pfcom->swapout = ECMA_Swapout_T_auxiliary_body;
    pfcom->dim = ECMA_Dim_T_auxiliary_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_auxiliary_body;
    pfcom->remove = ECMA_Remove_T_auxiliary_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_auxiliary_body; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_auxiliary_body; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_auxiliary_body;
    pfcom->store = ECMA_Store_T_auxiliary_body;
    pfcom->pack = ECMA_Pack_T_auxiliary_body;
    pfcom->unpack = ECMA_Unpack_T_auxiliary_body;
    pfcom->sql = ECMA_Sql_T_auxiliary_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
