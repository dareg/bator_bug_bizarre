#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_radar_body 1
#include "ECMA.h"
...
PUBLIC int
ECMA_Pack_T_radar_body(void *T)
{
  int Nbytes = 0;
  TABLE_radar_body *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk1int, flgdyn); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, hhmmss, time); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk9real, distance); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, pk9real, elevation); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, pk9real, polarisation); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, pk1int, anaprop); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, ECMA, pk9real, reflcost); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, ECMA, pk9real, azimuth); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, ECMA, pk9real, press); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, ECMA, pk9real, temp_1); CHECK_PDS_ERROR(10);
    PDS = PackDS(P, ECMA, pk9real, temp_2); CHECK_PDS_ERROR(11);
    PDS = PackDS(P, ECMA, pk9real, q_1); CHECK_PDS_ERROR(12);
    PDS = PackDS(P, ECMA, pk9real, q_2); CHECK_PDS_ERROR(13);
    PDS = PackDS(P, ECMA, pk9real, temp_1dv); CHECK_PDS_ERROR(14);
    PDS = PackDS(P, ECMA, pk9real, q_1dv); CHECK_PDS_ERROR(15);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_radar_body(void *T)
{
  int Nbytes = 0;
  TABLE_radar_body *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk1int, flgdyn); Nbytes += BYTESIZE(P->flgdyn.d);
    UseDS(P, ECMA, hhmmss, time); Nbytes += BYTESIZE(P->time.d);
    UseDS(P, ECMA, pk9real, distance); Nbytes += BYTESIZE(P->distance.d);
    UseDS(P, ECMA, pk9real, elevation); Nbytes += BYTESIZE(P->elevation.d);
    UseDS(P, ECMA, pk9real, polarisation); Nbytes += BYTESIZE(P->polarisation.d);
    UseDS(P, ECMA, pk1int, anaprop); Nbytes += BYTESIZE(P->anaprop.d);
    UseDS(P, ECMA, pk9real, reflcost); Nbytes += BYTESIZE(P->reflcost.d);
    ...
    UseDS(P, ECMA, pk9real, temp_1dv); Nbytes += BYTESIZE(P->temp_1dv.d);
    UseDS(P, ECMA, pk9real, q_1dv); Nbytes += BYTESIZE(P->q_1dv.d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_radar_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_radar_body *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(radar_body);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, radar_body)
  Call_CopyGet_TABLE(ECMA, d, 1, radar_body, pk1int, D, flgdyn, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 2, radar_body, hhmmss, D, time, Count, DATATYPE_HHMMSS);
  Call_CopyGet_TABLE(ECMA, d, 3, radar_body, pk9real, D, distance, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 4, radar_body, pk9real, D, elevation, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 5, radar_body, pk9real, D, polarisation, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 6, radar_body, pk1int, D, anaprop, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 7, radar_body, pk9real, D, reflcost, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 8, radar_body, pk9real, D, azimuth, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 9, radar_body, pk9real, D, press, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 10, radar_body, pk9real, D, temp_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 11, radar_body, pk9real, D, temp_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 12, radar_body, pk9real, D, q_1, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 13, radar_body, pk9real, D, q_2, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 14, radar_body, pk9real, D, temp_1dv, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 15, radar_body, pk9real, D, q_1dv, Count, DATATYPE_REAL8);
PostGetTable(d, double, radar_body)


PrePutTable(ECMA, d, double, radar_body)
  Call_CopyPut_TABLE(ECMA, d, 1, radar_body, pk1int, flgdyn, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 2, radar_body, hhmmss, time, D, Count, DATATYPE_HHMMSS);
  Call_CopyPut_TABLE(ECMA, d, 3, radar_body, pk9real, distance, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 4, radar_body, pk9real, elevation, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 5, radar_body, pk9real, polarisation, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 6, radar_body, pk1int, anaprop, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 7, radar_body, pk9real, reflcost, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 8, radar_body, pk9real, azimuth, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 9, radar_body, pk9real, press, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 10, radar_body, pk9real, temp_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 11, radar_body, pk9real, temp_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 12, radar_body, pk9real, q_1, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 13, radar_body, pk9real, q_2, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 14, radar_body, pk9real, temp_1dv, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 15, radar_body, pk9real, q_1dv, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, radar_body)

PreLoadTable(ECMA, radar_body);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, flgdyn);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, hhmmss, DATATYPE_HHMMSS, time);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, distance);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, elevation);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, polarisation);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, anaprop);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, reflcost);
  ...
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, temp_1dv);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, q_1dv);
PostLoadTable(radar_body)

PreStoreTable(ECMA, radar_body);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, flgdyn);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, hhmmss, DATATYPE_HHMMSS, time);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, distance);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, elevation);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, polarisation);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, anaprop);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, reflcost);
  ...
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, temp_1dv);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, q_1dv);
PostStoreTable(radar_body)

DefineLookupTable(radar_body)

PUBLIC void
ECMA_Dim_T_radar_body(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_radar_body *P = T;
  Call_LookupTable(radar_body, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_radar_body(void *T)
{
  TABLE_radar_body *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, flgdyn, Nbytes, Count);
  FreeDS(P, time, Nbytes, Count);
  FreeDS(P, distance, Nbytes, Count);
  FreeDS(P, elevation, Nbytes, Count);
  FreeDS(P, polarisation, Nbytes, Count);
  FreeDS(P, anaprop, Nbytes, Count);
  FreeDS(P, reflcost, Nbytes, Count);
  ...
  FreeDS(P, temp_1dv, Nbytes, Count);
  FreeDS(P, q_1dv, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(radar_body,15);
}

DefineRemoveTable(ECMA, radar_body)

PUBLIC int
ECMA_Sql_T_radar_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_radar_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_radar_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_radar_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 15);
  InitDS(pk1int, DATATYPE_INT4, flgdyn, radar_body, 1);
  InitDS(hhmmss, DATATYPE_HHMMSS, time, radar_body, 1);
  InitDS(pk9real, DATATYPE_REAL8, distance, radar_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, elevation, radar_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, polarisation, radar_body, 9);
  InitDS(pk1int, DATATYPE_INT4, anaprop, radar_body, 1);
  InitDS(pk9real, DATATYPE_REAL8, reflcost, radar_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, azimuth, radar_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, press, radar_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, temp_1, radar_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, temp_2, radar_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, q_1, radar_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, q_2, radar_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, temp_1dv, radar_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, q_1dv, radar_body, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@radar_body"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 381;
    pfcom->wt = 358.000376;
    pfcom->tags = ECMA_Set_T_radar_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_radar_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_radar_body;
    pfcom->swapout = ECMA_Swapout_T_radar_body;
    pfcom->dim = ECMA_Dim_T_radar_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_radar_body;
    pfcom->remove = ECMA_Remove_T_radar_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_radar_body; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_radar_body; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_radar_body;
    pfcom->store = ECMA_Store_T_radar_body;
    pfcom->pack = ECMA_Pack_T_radar_body;
    pfcom->unpack = ECMA_Unpack_T_radar_body;
    pfcom->sql = ECMA_Sql_T_radar_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
