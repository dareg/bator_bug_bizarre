#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_aeolus_l2b 1
#include "ECMA.h"
...
PUBLIC int
ECMA_Pack_T_aeolus_l2b(void *T)
{
  int Nbytes = 0;
  TABLE_aeolus_l2b *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk9real, t_ref); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk9real, p_ref); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk9real, beta); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, pk9real, dhlos_dt); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, pk9real, dhlos_dp); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, pk9real, dhlos_dbeta); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, ECMA, pk9real, horiz_length); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, ECMA, pk9real, vert_length); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, ECMA, pk1int, conf_flag); CHECK_PDS_ERROR(9);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_aeolus_l2b(void *T)
{
  int Nbytes = 0;
  TABLE_aeolus_l2b *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk9real, t_ref); Nbytes += BYTESIZE(P->t_ref.d);
    UseDS(P, ECMA, pk9real, p_ref); Nbytes += BYTESIZE(P->p_ref.d);
    UseDS(P, ECMA, pk9real, beta); Nbytes += BYTESIZE(P->beta.d);
    UseDS(P, ECMA, pk9real, dhlos_dt); Nbytes += BYTESIZE(P->dhlos_dt.d);
    UseDS(P, ECMA, pk9real, dhlos_dp); Nbytes += BYTESIZE(P->dhlos_dp.d);
    UseDS(P, ECMA, pk9real, dhlos_dbeta); Nbytes += BYTESIZE(P->dhlos_dbeta.d);
    UseDS(P, ECMA, pk9real, horiz_length); Nbytes += BYTESIZE(P->horiz_length.d);
    UseDS(P, ECMA, pk9real, vert_length); Nbytes += BYTESIZE(P->vert_length.d);
    UseDS(P, ECMA, pk1int, conf_flag); Nbytes += BYTESIZE(P->conf_flag.d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_aeolus_l2b(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_aeolus_l2b *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(aeolus_l2b);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, aeolus_l2b)
  Call_CopyGet_TABLE(ECMA, d, 1, aeolus_l2b, pk9real, D, t_ref, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, aeolus_l2b, pk9real, D, p_ref, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 3, aeolus_l2b, pk9real, D, beta, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 4, aeolus_l2b, pk9real, D, dhlos_dt, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 5, aeolus_l2b, pk9real, D, dhlos_dp, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 6, aeolus_l2b, pk9real, D, dhlos_dbeta, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 7, aeolus_l2b, pk9real, D, horiz_length, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 8, aeolus_l2b, pk9real, D, vert_length, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 9, aeolus_l2b, pk1int, D, conf_flag, Count, DATATYPE_INT4);
PostGetTable(d, double, aeolus_l2b)


PrePutTable(ECMA, d, double, aeolus_l2b)
  Call_CopyPut_TABLE(ECMA, d, 1, aeolus_l2b, pk9real, t_ref, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, aeolus_l2b, pk9real, p_ref, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 3, aeolus_l2b, pk9real, beta, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 4, aeolus_l2b, pk9real, dhlos_dt, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 5, aeolus_l2b, pk9real, dhlos_dp, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 6, aeolus_l2b, pk9real, dhlos_dbeta, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 7, aeolus_l2b, pk9real, horiz_length, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 8, aeolus_l2b, pk9real, vert_length, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 9, aeolus_l2b, pk1int, conf_flag, D, Count, DATATYPE_INT4);
PostPutTable(d, double, aeolus_l2b)

PreLoadTable(ECMA, aeolus_l2b);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, t_ref);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, p_ref);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, beta);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, dhlos_dt);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, dhlos_dp);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, dhlos_dbeta);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, horiz_length);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, vert_length);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, conf_flag);
PostLoadTable(aeolus_l2b)

PreStoreTable(ECMA, aeolus_l2b);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, t_ref);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, p_ref);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, beta);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, dhlos_dt);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, dhlos_dp);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, dhlos_dbeta);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, horiz_length);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, vert_length);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, conf_flag);
PostStoreTable(aeolus_l2b)

DefineLookupTable(aeolus_l2b)

PUBLIC void
ECMA_Dim_T_aeolus_l2b(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_aeolus_l2b *P = T;
  Call_LookupTable(aeolus_l2b, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_aeolus_l2b(void *T)
{
  TABLE_aeolus_l2b *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, t_ref, Nbytes, Count);
  FreeDS(P, p_ref, Nbytes, Count);
  FreeDS(P, beta, Nbytes, Count);
  FreeDS(P, dhlos_dt, Nbytes, Count);
  FreeDS(P, dhlos_dp, Nbytes, Count);
  FreeDS(P, dhlos_dbeta, Nbytes, Count);
  FreeDS(P, horiz_length, Nbytes, Count);
  FreeDS(P, vert_length, Nbytes, Count);
  FreeDS(P, conf_flag, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(aeolus_l2b,9);
}

DefineRemoveTable(ECMA, aeolus_l2b)

PUBLIC int
ECMA_Sql_T_aeolus_l2b(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_aeolus_l2b(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_aeolus_l2b *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_aeolus_l2b);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 9);
  InitDS(pk9real, DATATYPE_REAL8, t_ref, aeolus_l2b, 9);
  InitDS(pk9real, DATATYPE_REAL8, p_ref, aeolus_l2b, 9);
  InitDS(pk9real, DATATYPE_REAL8, beta, aeolus_l2b, 9);
  InitDS(pk9real, DATATYPE_REAL8, dhlos_dt, aeolus_l2b, 9);
  InitDS(pk9real, DATATYPE_REAL8, dhlos_dp, aeolus_l2b, 9);
  InitDS(pk9real, DATATYPE_REAL8, dhlos_dbeta, aeolus_l2b, 9);
  InitDS(pk9real, DATATYPE_REAL8, horiz_length, aeolus_l2b, 9);
  InitDS(pk9real, DATATYPE_REAL8, vert_length, aeolus_l2b, 9);
  InitDS(pk1int, DATATYPE_INT4, conf_flag, aeolus_l2b, 1);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@aeolus_l2b"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 29;
    pfcom->wt = 3.003004;
    pfcom->tags = ECMA_Set_T_aeolus_l2b_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_aeolus_l2b_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_aeolus_l2b;
    pfcom->swapout = ECMA_Swapout_T_aeolus_l2b;
    pfcom->dim = ECMA_Dim_T_aeolus_l2b;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_aeolus_l2b;
    pfcom->remove = ECMA_Remove_T_aeolus_l2b;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_aeolus_l2b; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_aeolus_l2b; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_aeolus_l2b;
    pfcom->store = ECMA_Store_T_aeolus_l2b;
    pfcom->pack = ECMA_Pack_T_aeolus_l2b;
    pfcom->unpack = ECMA_Unpack_T_aeolus_l2b;
    pfcom->sql = ECMA_Sql_T_aeolus_l2b;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
