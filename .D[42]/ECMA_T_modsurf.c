...
PUBLIC int
ECMA_Pack_T_modsurf(void *T)
{
  int Nbytes = 0;
  TABLE_modsurf *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk9real, lsm); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk9real, seaice); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk9real, orography); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, pk9real, snow_depth); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, pk9real, t2m); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, pk9real, albedo); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, ECMA, pk9real, windspeed10m); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, ECMA, pk9real, u10m); CHECK_PDS_ERROR(8);
    PDS = PackDS(P, ECMA, pk9real, v10m); CHECK_PDS_ERROR(9);
    PDS = PackDS(P, ECMA, pk1int, surface_class); CHECK_PDS_ERROR(10);
    PDS = PackDS(P, ECMA, pk9real, tsfc); CHECK_PDS_ERROR(11);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_modsurf(void *T)
{
  int Nbytes = 0;
  TABLE_modsurf *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk9real, lsm); Nbytes += BYTESIZE(P->lsm.d);
    UseDS(P, ECMA, pk9real, seaice); Nbytes += BYTESIZE(P->seaice.d);
    UseDS(P, ECMA, pk9real, orography); Nbytes += BYTESIZE(P->orography.d);
    UseDS(P, ECMA, pk9real, snow_depth); Nbytes += BYTESIZE(P->snow_depth.d);
    UseDS(P, ECMA, pk9real, t2m); Nbytes += BYTESIZE(P->t2m.d);
    UseDS(P, ECMA, pk9real, albedo); Nbytes += BYTESIZE(P->albedo.d);
    UseDS(P, ECMA, pk9real, windspeed10m); Nbytes += BYTESIZE(P->windspeed10m.d);
    UseDS(P, ECMA, pk9real, u10m); Nbytes += BYTESIZE(P->u10m.d);
    UseDS(P, ECMA, pk9real, v10m); Nbytes += BYTESIZE(P->v10m.d);
    UseDS(P, ECMA, pk1int, surface_class); Nbytes += BYTESIZE(P->surface_class.d);
    UseDS(P, ECMA, pk9real, tsfc); Nbytes += BYTESIZE(P->tsfc.d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_modsurf(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_modsurf *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(modsurf);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, modsurf)
  Call_CopyGet_TABLE(ECMA, d, 1, modsurf, pk9real, D, lsm, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, modsurf, pk9real, D, seaice, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 3, modsurf, pk9real, D, orography, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 4, modsurf, pk9real, D, snow_depth, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 5, modsurf, pk9real, D, t2m, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 6, modsurf, pk9real, D, albedo, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 7, modsurf, pk9real, D, windspeed10m, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 8, modsurf, pk9real, D, u10m, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 9, modsurf, pk9real, D, v10m, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 10, modsurf, pk1int, D, surface_class, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 11, modsurf, pk9real, D, tsfc, Count, DATATYPE_REAL8);
PostGetTable(d, double, modsurf)


PrePutTable(ECMA, d, double, modsurf)
  Call_CopyPut_TABLE(ECMA, d, 1, modsurf, pk9real, lsm, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, modsurf, pk9real, seaice, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 3, modsurf, pk9real, orography, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 4, modsurf, pk9real, snow_depth, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 5, modsurf, pk9real, t2m, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 6, modsurf, pk9real, albedo, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 7, modsurf, pk9real, windspeed10m, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 8, modsurf, pk9real, u10m, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 9, modsurf, pk9real, v10m, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 10, modsurf, pk1int, surface_class, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 11, modsurf, pk9real, tsfc, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, modsurf)

PreLoadTable(ECMA, modsurf);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, lsm);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, seaice);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, orography);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, snow_depth);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, t2m);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, albedo);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, windspeed10m);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, u10m);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, v10m);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, surface_class);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, tsfc);
PostLoadTable(modsurf)

PreStoreTable(ECMA, modsurf);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, lsm);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, seaice);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, orography);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, snow_depth);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, t2m);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, albedo);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, windspeed10m);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, u10m);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, v10m);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, surface_class);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, tsfc);
PostStoreTable(modsurf)

DefineLookupTable(modsurf)

PUBLIC void
ECMA_Dim_T_modsurf(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_modsurf *P = T;
  Call_LookupTable(modsurf, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_modsurf(void *T)
{
  TABLE_modsurf *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, lsm, Nbytes, Count);
  FreeDS(P, seaice, Nbytes, Count);
  FreeDS(P, orography, Nbytes, Count);
  FreeDS(P, snow_depth, Nbytes, Count);
  FreeDS(P, t2m, Nbytes, Count);
  FreeDS(P, albedo, Nbytes, Count);
  FreeDS(P, windspeed10m, Nbytes, Count);
  FreeDS(P, u10m, Nbytes, Count);
  FreeDS(P, v10m, Nbytes, Count);
  FreeDS(P, surface_class, Nbytes, Count);
  FreeDS(P, tsfc, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(modsurf,11);
}

DefineRemoveTable(ECMA, modsurf)

PUBLIC int
ECMA_Sql_T_modsurf(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_modsurf(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_modsurf *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_modsurf);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 11);
  InitDS(pk9real, DATATYPE_REAL8, lsm, modsurf, 9);
  InitDS(pk9real, DATATYPE_REAL8, seaice, modsurf, 9);
  InitDS(pk9real, DATATYPE_REAL8, orography, modsurf, 9);
  InitDS(pk9real, DATATYPE_REAL8, snow_depth, modsurf, 9);
  InitDS(pk9real, DATATYPE_REAL8, t2m, modsurf, 9);
  InitDS(pk9real, DATATYPE_REAL8, albedo, modsurf, 9);
  InitDS(pk9real, DATATYPE_REAL8, windspeed10m, modsurf, 9);
  InitDS(pk9real, DATATYPE_REAL8, u10m, modsurf, 9);
  InitDS(pk9real, DATATYPE_REAL8, v10m, modsurf, 9);
  InitDS(pk1int, DATATYPE_INT4, surface_class, modsurf, 1);
  InitDS(pk9real, DATATYPE_REAL8, tsfc, modsurf, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@modsurf"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 11;
    pfcom->wt = 3.001004;
    pfcom->tags = ECMA_Set_T_modsurf_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_modsurf_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_modsurf;
    pfcom->swapout = ECMA_Swapout_T_modsurf;
    pfcom->dim = ECMA_Dim_T_modsurf;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_modsurf;
    pfcom->remove = ECMA_Remove_T_modsurf;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_modsurf; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_modsurf; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_modsurf;
    pfcom->store = ECMA_Store_T_modsurf;
    pfcom->pack = ECMA_Pack_T_modsurf;
    pfcom->unpack = ECMA_Unpack_T_modsurf;
    pfcom->sql = ECMA_Sql_T_modsurf;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
