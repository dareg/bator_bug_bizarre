...
PUBLIC int
ECMA_Pack_T_gnssro(void *T)
{
  int Nbytes = 0;
  TABLE_gnssro *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk9real, radcurv); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk9real, undulation); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, linkoffset_t, LINKOFFSET(gnssro_body)); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, linklen_t, LINKLEN(gnssro_body)); CHECK_PDS_ERROR(4);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_gnssro(void *T)
{
  int Nbytes = 0;
  TABLE_gnssro *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk9real, radcurv); Nbytes += BYTESIZE(P->radcurv.d);
    UseDS(P, ECMA, pk9real, undulation); Nbytes += BYTESIZE(P->undulation.d);
    UseDS(P, ECMA, linkoffset_t, LINKOFFSET(gnssro_body)); Nbytes += BYTESIZE(P->LINKOFFSET(gnssro_body).d);
    UseDS(P, ECMA, linklen_t, LINKLEN(gnssro_body)); Nbytes += BYTESIZE(P->LINKLEN(gnssro_body).d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_gnssro(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_gnssro *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(gnssro);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, gnssro)
  Call_CopyGet_TABLE(ECMA, d, 1, gnssro, pk9real, D, radcurv, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, gnssro, pk9real, D, undulation, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 3, gnssro, linkoffset_t, D, LINKOFFSET(gnssro_body), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(ECMA, d, 4, gnssro, linklen_t, D, LINKLEN(gnssro_body), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, gnssro)


PrePutTable(ECMA, d, double, gnssro)
  Call_CopyPut_TABLE(ECMA, d, 1, gnssro, pk9real, radcurv, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, gnssro, pk9real, undulation, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 3, gnssro, linkoffset_t, LINKOFFSET(gnssro_body), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(ECMA, d, 4, gnssro, linklen_t, LINKLEN(gnssro_body), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, gnssro)

PreLoadTable(ECMA, gnssro);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, radcurv);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, undulation);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(gnssro_body));
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(gnssro_body));
PostLoadTable(gnssro)

PreStoreTable(ECMA, gnssro);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, radcurv);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, undulation);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(gnssro_body));
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(gnssro_body));
PostStoreTable(gnssro)

DefineLookupTable(gnssro)

PUBLIC void
ECMA_Dim_T_gnssro(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_gnssro *P = T;
  Call_LookupTable(gnssro, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_gnssro(void *T)
{
  TABLE_gnssro *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, radcurv, Nbytes, Count);
  FreeDS(P, undulation, Nbytes, Count);
  FreeDS(P, LINKOFFSET(gnssro_body), Nbytes, Count);
  FreeDS(P, LINKLEN(gnssro_body), Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(gnssro,4);
}

DefineRemoveTable(ECMA, gnssro)

PUBLIC int
ECMA_Sql_T_gnssro(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_gnssro(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_gnssro *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_gnssro);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 4);
  InitDS(pk9real, DATATYPE_REAL8, radcurv, gnssro, 9);
  InitDS(pk9real, DATATYPE_REAL8, undulation, gnssro, 9);
  InitDS(linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(gnssro_body), gnssro, 1);
  InitDS(linklen_t, DATATYPE_LINKLEN, LINKLEN(gnssro_body), gnssro, 1);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@gnssro"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 15;
    pfcom->wt = 3.002004;
    pfcom->tags = ECMA_Set_T_gnssro_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_gnssro_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_gnssro;
    pfcom->swapout = ECMA_Swapout_T_gnssro;
    pfcom->dim = ECMA_Dim_T_gnssro;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_gnssro;
    pfcom->remove = ECMA_Remove_T_gnssro;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_gnssro; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_gnssro; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_gnssro;
    pfcom->store = ECMA_Store_T_gnssro;
    pfcom->pack = ECMA_Pack_T_gnssro;
    pfcom->unpack = ECMA_Unpack_T_gnssro;
    pfcom->sql = ECMA_Sql_T_gnssro;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
