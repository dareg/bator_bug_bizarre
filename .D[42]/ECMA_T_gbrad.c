...
ECMA_Pack_T_gbrad(void *T)
{
  ...
  if (P->Is_loaded) {
    ...
    PDS = PackDS(P, ECMA, linkoffset_t, LINKOFFSET(gbrad_body)); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, linklen_t, LINKLEN(gbrad_body)); CHECK_PDS_ERROR(3);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_gbrad(void *T)
{
  int Nbytes = 0;
  TABLE_gbrad *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk1int, report_rrflag); Nbytes += BYTESIZE(P->report_rrflag.d);
    UseDS(P, ECMA, linkoffset_t, LINKOFFSET(gbrad_body)); Nbytes += BYTESIZE(P->LINKOFFSET(gbrad_body).d);
    UseDS(P, ECMA, linklen_t, LINKLEN(gbrad_body)); Nbytes += BYTESIZE(P->LINKLEN(gbrad_body).d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_gbrad(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_gbrad *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(gbrad);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, gbrad)
  Call_CopyGet_TABLE(ECMA, d, 1, gbrad, pk1int, D, report_rrflag, Count, DATATYPE_INT4);
  Call_CopyGet_TABLE(ECMA, d, 2, gbrad, linkoffset_t, D, LINKOFFSET(gbrad_body), Count, DATATYPE_LINKOFFSET);
  Call_CopyGet_TABLE(ECMA, d, 3, gbrad, linklen_t, D, LINKLEN(gbrad_body), Count, DATATYPE_LINKLEN);
PostGetTable(d, double, gbrad)


PrePutTable(ECMA, d, double, gbrad)
  Call_CopyPut_TABLE(ECMA, d, 1, gbrad, pk1int, report_rrflag, D, Count, DATATYPE_INT4);
  Call_CopyPut_TABLE(ECMA, d, 2, gbrad, linkoffset_t, LINKOFFSET(gbrad_body), D, Count, DATATYPE_LINKOFFSET);
  Call_CopyPut_TABLE(ECMA, d, 3, gbrad, linklen_t, LINKLEN(gbrad_body), D, Count, DATATYPE_LINKLEN);
PostPutTable(d, double, gbrad)

PreLoadTable(ECMA, gbrad);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, report_rrflag);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(gbrad_body));
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(gbrad_body));
PostLoadTable(gbrad)

PreStoreTable(ECMA, gbrad);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, report_rrflag);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(gbrad_body));
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, linklen_t, DATATYPE_LINKLEN, LINKLEN(gbrad_body));
PostStoreTable(gbrad)

DefineLookupTable(gbrad)

PUBLIC void
ECMA_Dim_T_gbrad(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_gbrad *P = T;
  Call_LookupTable(gbrad, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_gbrad(void *T)
{
  TABLE_gbrad *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, LINKOFFSET(gbrad_body), Nbytes, Count);
  FreeDS(P, LINKLEN(gbrad_body), Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(gbrad,3);
}

DefineRemoveTable(ECMA, gbrad)

PUBLIC int
ECMA_Sql_T_gbrad(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_gbrad(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_gbrad *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_gbrad);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 3);
  InitDS(pk1int, DATATYPE_INT4, report_rrflag, gbrad, 1);
  InitDS(linkoffset_t, DATATYPE_LINKOFFSET, LINKOFFSET(gbrad_body), gbrad, 1);
  InitDS(linklen_t, DATATYPE_LINKLEN, LINKLEN(gbrad_body), gbrad, 1);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@gbrad"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 7;
    pfcom->wt = 3.001004;
    pfcom->tags = ECMA_Set_T_gbrad_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_gbrad_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_gbrad;
    pfcom->swapout = ECMA_Swapout_T_gbrad;
    pfcom->dim = ECMA_Dim_T_gbrad;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_gbrad;
    pfcom->remove = ECMA_Remove_T_gbrad;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_gbrad; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_gbrad; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_gbrad;
    pfcom->store = ECMA_Store_T_gbrad;
    pfcom->pack = ECMA_Pack_T_gbrad;
    pfcom->unpack = ECMA_Unpack_T_gbrad;
    pfcom->sql = ECMA_Sql_T_gbrad;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
