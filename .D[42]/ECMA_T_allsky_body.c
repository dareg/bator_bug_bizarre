#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_allsky_body 1
#include "ECMA.h"
...
PUBLIC int
ECMA_Pack_T_allsky_body(void *T)
{
  int Nbytes = 0;
  TABLE_allsky_body *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk9real, tbvalue); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk9real, tbvaluetl); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk9real, tbvaluead); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, pk1int, datum_tbflag); CHECK_PDS_ERROR(4);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_allsky_body(void *T)
{
  int Nbytes = 0;
  TABLE_allsky_body *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk9real, tbvalue); Nbytes += BYTESIZE(P->tbvalue.d);
    UseDS(P, ECMA, pk9real, tbvaluetl); Nbytes += BYTESIZE(P->tbvaluetl.d);
    UseDS(P, ECMA, pk9real, tbvaluead); Nbytes += BYTESIZE(P->tbvaluead.d);
    UseDS(P, ECMA, pk1int, datum_tbflag); Nbytes += BYTESIZE(P->datum_tbflag.d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_allsky_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_allsky_body *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(allsky_body);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, allsky_body)
  Call_CopyGet_TABLE(ECMA, d, 1, allsky_body, pk9real, D, tbvalue, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, allsky_body, pk9real, D, tbvaluetl, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 3, allsky_body, pk9real, D, tbvaluead, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 4, allsky_body, pk1int, D, datum_tbflag, Count, DATATYPE_INT4);
PostGetTable(d, double, allsky_body)


PrePutTable(ECMA, d, double, allsky_body)
  Call_CopyPut_TABLE(ECMA, d, 1, allsky_body, pk9real, tbvalue, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, allsky_body, pk9real, tbvaluetl, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 3, allsky_body, pk9real, tbvaluead, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 4, allsky_body, pk1int, datum_tbflag, D, Count, DATATYPE_INT4);
PostPutTable(d, double, allsky_body)

PreLoadTable(ECMA, allsky_body);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, tbvalue);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, tbvaluetl);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, tbvaluead);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, datum_tbflag);
PostLoadTable(allsky_body)

PreStoreTable(ECMA, allsky_body);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, tbvalue);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, tbvaluetl);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, tbvaluead);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk1int, DATATYPE_INT4, datum_tbflag);
PostStoreTable(allsky_body)

DefineLookupTable(allsky_body)

PUBLIC void
ECMA_Dim_T_allsky_body(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_allsky_body *P = T;
  Call_LookupTable(allsky_body, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_allsky_body(void *T)
{
  TABLE_allsky_body *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, tbvaluetl, Nbytes, Count);
  FreeDS(P, tbvaluead, Nbytes, Count);
  FreeDS(P, datum_tbflag, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(allsky_body,4);
}

DefineRemoveTable(ECMA, allsky_body)

PUBLIC int
ECMA_Sql_T_allsky_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_allsky_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_allsky_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_allsky_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 4);
  InitDS(pk9real, DATATYPE_REAL8, tbvalue, allsky_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, tbvaluetl, allsky_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, tbvaluead, allsky_body, 9);
  InitDS(pk1int, DATATYPE_INT4, datum_tbflag, allsky_body, 1);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@allsky_body"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 375;
    pfcom->wt = 358.000359;
    pfcom->tags = ECMA_Set_T_allsky_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_allsky_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_allsky_body;
    pfcom->swapout = ECMA_Swapout_T_allsky_body;
    pfcom->dim = ECMA_Dim_T_allsky_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_allsky_body;
    pfcom->remove = ECMA_Remove_T_allsky_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_allsky_body; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_allsky_body; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_allsky_body;
    pfcom->store = ECMA_Store_T_allsky_body;
    pfcom->pack = ECMA_Pack_T_allsky_body;
    pfcom->unpack = ECMA_Unpack_T_allsky_body;
    pfcom->sql = ECMA_Sql_T_allsky_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
