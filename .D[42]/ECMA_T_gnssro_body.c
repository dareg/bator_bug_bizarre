#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_gnssro_body 1
#include "ECMA.h"
...
PUBLIC int
ECMA_Pack_T_gnssro_body(void *T)
{
  int Nbytes = 0;
  TABLE_gnssro_body *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
    PDS = PackDS(P, ECMA, pk9real, obs_dndz); CHECK_PDS_ERROR(1);
    PDS = PackDS(P, ECMA, pk9real, obs_refractivity); CHECK_PDS_ERROR(2);
    PDS = PackDS(P, ECMA, pk9real, bg_dndz); CHECK_PDS_ERROR(3);
    PDS = PackDS(P, ECMA, pk9real, bg_refractivity); CHECK_PDS_ERROR(4);
    PDS = PackDS(P, ECMA, pk9real, bg_layerno); CHECK_PDS_ERROR(5);
    PDS = PackDS(P, ECMA, pk9real, obs_tvalue); CHECK_PDS_ERROR(6);
    PDS = PackDS(P, ECMA, pk9real, obs_zvalue); CHECK_PDS_ERROR(7);
    PDS = PackDS(P, ECMA, pk9real, bg_tvalue); CHECK_PDS_ERROR(8);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Unpack_T_gnssro_body(void *T)
{
  int Nbytes = 0;
  TABLE_gnssro_body *P = T;
  if (P->Is_loaded) {
    UseDS(P, ECMA, pk9real, obs_dndz); Nbytes += BYTESIZE(P->obs_dndz.d);
    UseDS(P, ECMA, pk9real, obs_refractivity); Nbytes += BYTESIZE(P->obs_refractivity.d);
    UseDS(P, ECMA, pk9real, bg_dndz); Nbytes += BYTESIZE(P->bg_dndz.d);
    UseDS(P, ECMA, pk9real, bg_refractivity); Nbytes += BYTESIZE(P->bg_refractivity.d);
    UseDS(P, ECMA, pk9real, bg_layerno); Nbytes += BYTESIZE(P->bg_layerno.d);
    UseDS(P, ECMA, pk9real, obs_tvalue); Nbytes += BYTESIZE(P->obs_tvalue.d);
    UseDS(P, ECMA, pk9real, obs_zvalue); Nbytes += BYTESIZE(P->obs_zvalue.d);
    UseDS(P, ECMA, pk9real, bg_tvalue); Nbytes += BYTESIZE(P->bg_tvalue.d);
  }
  return Nbytes;
}

PUBLIC int
ECMA_Sel_T_gnssro_body(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_gnssro_body *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(gnssro_body);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, gnssro_body)
  Call_CopyGet_TABLE(ECMA, d, 1, gnssro_body, pk9real, D, obs_dndz, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, gnssro_body, pk9real, D, obs_refractivity, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 3, gnssro_body, pk9real, D, bg_dndz, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 4, gnssro_body, pk9real, D, bg_refractivity, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 5, gnssro_body, pk9real, D, bg_layerno, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 6, gnssro_body, pk9real, D, obs_tvalue, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 7, gnssro_body, pk9real, D, obs_zvalue, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 8, gnssro_body, pk9real, D, bg_tvalue, Count, DATATYPE_REAL8);
PostGetTable(d, double, gnssro_body)


PrePutTable(ECMA, d, double, gnssro_body)
  Call_CopyPut_TABLE(ECMA, d, 1, gnssro_body, pk9real, obs_dndz, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, gnssro_body, pk9real, obs_refractivity, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 3, gnssro_body, pk9real, bg_dndz, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 4, gnssro_body, pk9real, bg_refractivity, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 5, gnssro_body, pk9real, bg_layerno, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 6, gnssro_body, pk9real, obs_tvalue, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 7, gnssro_body, pk9real, obs_zvalue, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 8, gnssro_body, pk9real, bg_tvalue, D, Count, DATATYPE_REAL8);
PostPutTable(d, double, gnssro_body)

PreLoadTable(ECMA, gnssro_body);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_dndz);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_refractivity);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_dndz);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_refractivity);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_layerno);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_tvalue);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_zvalue);
  Call_Read_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_tvalue);
PostLoadTable(gnssro_body)

PreStoreTable(ECMA, gnssro_body);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_dndz);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_refractivity);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_dndz);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_refractivity);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_layerno);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_tvalue);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, obs_zvalue);
  Call_Write_DS(ECMA, fp_idx, filename, Nbytes, pk9real, DATATYPE_REAL8, bg_tvalue);
PostStoreTable(gnssro_body)

DefineLookupTable(gnssro_body)

PUBLIC void
ECMA_Dim_T_gnssro_body(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_gnssro_body *P = T;
  Call_LookupTable(gnssro_body, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_gnssro_body(void *T)
{
  TABLE_gnssro_body *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  FreeDS(P, obs_dndz, Nbytes, Count);
  FreeDS(P, obs_refractivity, Nbytes, Count);
  FreeDS(P, bg_dndz, Nbytes, Count);
  FreeDS(P, bg_refractivity, Nbytes, Count);
  FreeDS(P, bg_layerno, Nbytes, Count);
  FreeDS(P, obs_tvalue, Nbytes, Count);
  FreeDS(P, obs_zvalue, Nbytes, Count);
  FreeDS(P, bg_tvalue, Nbytes, Count);
  P->Nrows = 0;
  P->Nalloc = 0;
  P->Is_loaded = 0;
  P->Swapped_out = P->Is_new ? 0 : 1;
  ODBMAC_TRACE_SWAPOUT(gnssro_body,8);
}

DefineRemoveTable(ECMA, gnssro_body)

PUBLIC int
ECMA_Sql_T_gnssro_body(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_gnssro_body(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_gnssro_body *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_gnssro_body);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 8);
  InitDS(pk9real, DATATYPE_REAL8, obs_dndz, gnssro_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, obs_refractivity, gnssro_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, bg_dndz, gnssro_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, bg_refractivity, gnssro_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, bg_layerno, gnssro_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, obs_tvalue, gnssro_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, obs_zvalue, gnssro_body, 9);
  InitDS(pk9real, DATATYPE_REAL8, bg_tvalue, gnssro_body, 9);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@gnssro_body"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 378;
    pfcom->wt = 358.000367;
    pfcom->tags = ECMA_Set_T_gnssro_body_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_gnssro_body_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_gnssro_body;
    pfcom->swapout = ECMA_Swapout_T_gnssro_body;
    pfcom->dim = ECMA_Dim_T_gnssro_body;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_gnssro_body;
    pfcom->remove = ECMA_Remove_T_gnssro_body;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_gnssro_body; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_gnssro_body; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_gnssro_body;
    pfcom->store = ECMA_Store_T_gnssro_body;
    pfcom->pack = ECMA_Pack_T_gnssro_body;
    pfcom->unpack = ECMA_Unpack_T_gnssro_body;
    pfcom->sql = ECMA_Sql_T_gnssro_body;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
