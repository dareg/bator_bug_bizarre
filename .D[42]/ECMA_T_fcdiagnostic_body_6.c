#undef ODB_MAINCODE
#undef IS_a_VIEW
#define IS_a_TABLE_fcdiagnostic_body_6 1
#include "ECMA.h"
...
PUBLIC int
ECMA_Pack_T_fcdiagnostic_body_6(void *T)
{
  int Nbytes = 0;
  TABLE_fcdiagnostic_body_6 *P = T;
  Packed_DS *PDS;
  if (P->Is_loaded) {
...
PUBLIC int
ECMA_Unpack_T_fcdiagnostic_body_6(void *T)
{
  int Nbytes = 0;
  TABLE_fcdiagnostic_body_6 *P = T;
  if (P->Is_loaded) {
...
PUBLIC int
ECMA_Sel_T_fcdiagnostic_body_6(void *T, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  TABLE_fcdiagnostic_body_6 *P = T;
  ODBMAC_TABLE_DELAYED_LOAD(fcdiagnostic_body_6);
  return P->Nrows;
}


PreGetTable(ECMA, d, double, fcdiagnostic_body_6)
  Call_CopyGet_TABLE(ECMA, d, 1, fcdiagnostic_body_6, pk9real, D, fc_depar, Count, DATATYPE_REAL8);
  Call_CopyGet_TABLE(ECMA, d, 2, fcdiagnostic_body_6, yyyymmdd, D, fc_step, Count, DATATYPE_YYYYMMDD);
PostGetTable(d, double, fcdiagnostic_body_6)


PrePutTable(ECMA, d, double, fcdiagnostic_body_6)
  Call_CopyPut_TABLE(ECMA, d, 1, fcdiagnostic_body_6, pk9real, fc_depar, D, Count, DATATYPE_REAL8);
  Call_CopyPut_TABLE(ECMA, d, 2, fcdiagnostic_body_6, yyyymmdd, fc_step, D, Count, DATATYPE_YYYYMMDD);
PostPutTable(d, double, fcdiagnostic_body_6)

PreLoadTable(ECMA, fcdiagnostic_body_6);
...
PostLoadTable(fcdiagnostic_body_6)

PreStoreTable(ECMA, fcdiagnostic_body_6);
...
PostStoreTable(fcdiagnostic_body_6)

DefineLookupTable(fcdiagnostic_body_6)

PUBLIC void
ECMA_Dim_T_fcdiagnostic_body_6(void *T, int *Nrows, int *Ncols,
  int *Nrowoffset, int ProcID)
{
  TABLE_fcdiagnostic_body_6 *P = T;
  Call_LookupTable(fcdiagnostic_body_6, P, Nrows, Ncols);
  if (Nrowoffset) *Nrowoffset = 0;
}

PUBLIC void
ECMA_Swapout_T_fcdiagnostic_body_6(void *T)
{
  TABLE_fcdiagnostic_body_6 *P = T;
  int Nbytes = 0;
  int Count = 0;
  ...
  ODBMAC_TRACE_SWAPOUT(fcdiagnostic_body_6,2);
}

DefineRemoveTable(ECMA, fcdiagnostic_body_6)

PUBLIC int
ECMA_Sql_T_fcdiagnostic_body_6(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_TABLESQL(); }

PUBLIC void *
ECMA_Init_T_fcdiagnostic_body_6(void *T, ODB_Pool *Pool, int Is_new, int IO_method, int it, int dummy)
{
  TABLE_fcdiagnostic_body_6 *P = T;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(ECMA_Init_T_fcdiagnostic_body_6);
  if (!P) ALLOC(P, 1);
  PreInitTable(P, 2);
  InitDS(pk9real, DATATYPE_REAL8, fc_depar, fcdiagnostic_body_6, 9);
  InitDS(yyyymmdd, DATATYPE_YYYYMMDD, fc_step, fcdiagnostic_body_6, 1);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "@fcdiagnostic_body_6"; pfcom->name = s; }
    pfcom->is_table = 1;
    ...
    pfcom->rank = 355;
    pfcom->wt = 358.000332;
    pfcom->tags = ECMA_Set_T_fcdiagnostic_body_6_TAG(&pfcom->ntag, &pfcom->nmem);
    pfcom->preptags = ECMA_Set_T_fcdiagnostic_body_6_PREPTAG(&pfcom->npreptag);
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = ECMA_Init_T_fcdiagnostic_body_6;
    pfcom->swapout = ECMA_Swapout_T_fcdiagnostic_body_6;
    pfcom->dim = ECMA_Dim_T_fcdiagnostic_body_6;
    pfcom->sortkeys = NULL;
    pfcom->update_info = NULL;
    pfcom->aggr_info = NULL;
    pfcom->getindex = NULL; /* N/A */
    pfcom->putindex = NULL; /* N/A */
    pfcom->select = ECMA_Sel_T_fcdiagnostic_body_6;
    pfcom->remove = ECMA_Remove_T_fcdiagnostic_body_6;
    pfcom->peinfo = NULL; /* N/A */
    pfcom->cancel = NULL;
    pfcom->dget = ECMA_dGet_T_fcdiagnostic_body_6; /* REAL(8) dbmgr */
    pfcom->dput = ECMA_dPut_T_fcdiagnostic_body_6; /* REAL(8) dbmgr */
    pfcom->load = ECMA_Load_T_fcdiagnostic_body_6;
    pfcom->store = ECMA_Store_T_fcdiagnostic_body_6;
    pfcom->pack = ECMA_Pack_T_fcdiagnostic_body_6;
    pfcom->unpack = ECMA_Unpack_T_fcdiagnostic_body_6;
    pfcom->sql = ECMA_Sql_T_fcdiagnostic_body_6;
    pfcom->ncols_aux = 0;
    pfcom->colaux = NULL;
...
/* *************** End of TABLE "fcdiagnostic_body_6" *************** */
