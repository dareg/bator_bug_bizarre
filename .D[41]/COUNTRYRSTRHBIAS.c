...
PUBLIC void 
COUNTRYRSTRHBIAS_print_flags_file(void)
{
  int rc = 0, io = -1;
  FILE *fp = NULL;
  cma_open_(&io, "COUNTRYRSTRHBIAS.flags", "w", &rc, strlen("COUNTRYRSTRHBIAS.flags"), strlen("w"));
...
Create_Funcs(ODB_Pool *pool, int is_new, int io_method, int it)
{
  ...
  ODBMAC_CREATE_TABLE(COUNTRYRSTRHBIAS, desc, 0, 1);
  ODBMAC_CREATE_TABLE(COUNTRYRSTRHBIAS, hdr, 1, 1);
  ODBMAC_CREATE_TABLE(COUNTRYRSTRHBIAS, body, 2, 1);
  DRHOOK_END(0);
  return nfuncs;
}
...
PUBLIC ODB_Funcs *
Anchor2COUNTRYRSTRHBIAS(void *V, ODB_Pool *pool, int *ntables, int it, int add_vars)
{
  ODB_Anchor_Funcs *func = V;
  ODB_Pool *p = pool;
  DRHOOK_START(Anchor2COUNTRYRSTRHBIAS);
  ...
  if (add_vars) {
    p->add_var(p->dbname, "$mdi", NULL, it, USD_mdi_COUNTRYRSTRHBIAS);
  } /* if (add_vars) */
  finish:
  DRHOOK_END(0);
  return NULL;
